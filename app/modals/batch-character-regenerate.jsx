...
function BatchCharacterRegenerateModal({ characterIds, characters, chapters, onClose, onConfirm }) {
  const selectedChars = characterIds.map(id => characters.find(c => c.id === id)).filter(Boolean);
  const [scope, setScope] = useState("all");
  const [reason, setReason] = useState("voice");
  const [note, setNote] = useState("");
  if (selectedChars.length === 0) return null;

  /* Calculate union of chapters where any selected character speaks */
  const chapterIds = chapters
    .filter(ch => selectedChars.some(c => ch.characters[c.id] && ch.characters[c.id] !== "skipped"))
    .map(ch => ch.id);
  const totalLines = selectedChars.reduce((s, c) => s + c.lines, 0);
      ...
      <div className="fixed inset-0 z-50 grid place-items-center p-6 pointer-events-none">
        <div className="bg-white rounded-3xl shadow-float w-full max-w-xl pointer-events-auto fade-in overflow-hidden max-h-[90vh] flex flex-col">
          ...
          <div className="px-6 py-4 border-b border-ink/10 flex items-center gap-3">
            <span className="w-10 h-10 rounded-full bg-peach/15 grid place-items-center text-magenta"><IconRefresh className="w-5 h-5"/></span>
            <div className="flex-1 min-w-0">
              <p className="text-[10px] uppercase tracking-widest text-ink/50 font-semibold">Batch regenerate</p>
              <h3 className="text-base font-bold text-ink truncate">{selectedChars.length} characters' lines</h3>
            </div>
          ...
          <div className="p-6 space-y-6 overflow-y-auto">
            ...
            <section>
              ...
              <div className="flex flex-wrap gap-2">
                {selectedChars.map(c => (
                  <span key={c.id} className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-canvas border border-ink/10">
            ...
            {/* Scope */}
            <section>
              <p className="text-[11px] uppercase tracking-wider text-ink/50 font-semibold mb-3">Scope</p>
              <div className="grid grid-cols-2 gap-2">
                <button onClick={()=>setScope("all")} className={`text-left p-3 rounded-2xl border transition-all ${scope === "all" ? "border-peach bg-peach/[0.06]" : "border-ink/10 hover:border-ink/20"}`}>
                  ...
                  <p className="text-xs text-ink/55 mt-0.5">{chapterIds.length} chapters · {totalLines} total lines</p>
                </button>
                <button onClick={()=>setScope("recent")} className={`text-left p-3 rounded-2xl border transition-all ${scope === "recent" ? "border-peach bg-peach/[0.06]" : "border-ink/10 hover:border-ink/20"}`}>
                  <p className="text-sm font-semibold text-ink">Only chapters after the last cast change</p>
                  <p className="text-xs text-ink/55 mt-0.5">Skips chapters voiced before the most recent edit.</p>
                </button>
              </div>
            </section>
            ...
            <div className="p-4 rounded-2xl bg-canvas border border-ink/10 flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-sm font-bold text-ink"><span className="tabular-nums">{totalLines}</span> lines · <span className="tabular-nums">{chapterIds.length}</span> chapters · ETA <span className="tabular-nums">{eta}</span></p>
              </div>
            </div>
          </div>
          ...
          <div className="px-6 py-4 border-t border-ink/10 flex items-center justify-end gap-3">
            ...
            <PrimaryButton variant="dark" onClick={()=>onConfirm({ characterIds, chapterIds, reason, note })}>Regenerate {selectedChars.length} characters</PrimaryButton>
          </div>
        </div>
      </div>
    </>
...
Object.assign(window, { BatchCharacterRegenerateModal });
