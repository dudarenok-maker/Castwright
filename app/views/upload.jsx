/* Upload — drop a file or paste markdown, hand off to the mock API.
   Calls api.uploadManuscript() and reports the UploadResponse upward via
   onUploaded(res). The Redux dispatch then advances the stage machine. */
...
function UploadView({ onUploaded }) {
  ...
  async function processText({ text, fileName, format }) {
    setError(null);
    setBusy(true);
    try {
      const res = await api.uploadManuscript({ text, fileName, format });
      onUploaded(res);
    } catch (e) {
  ...
  async function handleFile(file) {
    ...
    if (!okExt) {
      // EPUB/DOCX would need server-side conversion — surface that as a friendly error.
  ...
  async function handleSample() {
    await processText({ text: SAMPLE_MANUSCRIPT_MD, fileName: 'the-northern-star.md', format: 'md' });
  }
        ...
        {pasteOpen && (
          <div className="mt-4 bg-white rounded-3xl border border-ink/10 p-5">
            ...
            <div className="mt-3 flex justify-end">
              <PrimaryButton variant="dark" disabled={busy || !pastedText.trim()}
                onClick={()=>processText({ text: pastedText, fileName: 'pasted.md', format: 'md' })}>
                Upload pasted text
              </PrimaryButton>
            </div>
          </div>
