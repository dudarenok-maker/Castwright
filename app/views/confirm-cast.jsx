...
function ConfirmCastView({ characters, library, onConfirm, onReanalyse }) {
  /* decisions: { [charId]: "match" | "generate" } — only meaningful for matched characters */
...
function ConfirmCharacterCard({ character, voice, decision, onDecision }) {
  ...
  return (
    <article className={`bg-white rounded-3xl border shadow-card overflow-hidden transition-colors ${matched ? "border-purple-deep/15" : "border-ink/10"}`}>
      <div className="p-5 grid grid-cols-[auto_1fr_auto] items-start gap-5">
        <Avatar name={character.name} color={character.color} size={48}/>
        <div className="min-w-0">
          <div className="flex items-center gap-2 mb-1">
            <h3 className="text-lg font-bold text-ink truncate">{character.name}</h3>
            {matched && <Pill color="library">Matched · {Math.round(character.matchedFrom.confidence*100)}%</Pill>}
          </div>
...
function DecisionTile({ active, onClick, swatch, title, subtitle, badge, readonly }) {
  return (
...
Object.assign(window, { ConfirmCastView, ConfirmCharacterCard, DecisionTile });
