...
function PreviewListenerView({ chapters, characters, onExit, currentTrack, setCurrentTrack }) {
  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="bg-ink text-canvas border-b border-canvas/10 px-6 py-2.5 flex items-center justify-center gap-3 text-xs">
        ...
        <button onClick={onExit} className="ml-2 inline-flex items-center gap-1.5 text-canvas/80 hover:text-canvas font-medium">
          <IconArrowLeft className="w-3.5 h-3.5"/> Exit preview
        </button>
      </div>

      {/* Hero — gradient, centred, gentle */}
      <section className="relative overflow-hidden">
        ...
        <div className="relative max-w-4xl mx-auto px-6 pt-16 pb-20 text-center">
          <p className="text-[11px] uppercase tracking-[0.2em] text-white/70 font-semibold mb-6">An audiobook by Mike Dudarenok</p>
          <h1 className="font-serif text-5xl md:text-7xl font-bold text-white leading-[1.05]">The Northern Star</h1>
          <p className="font-serif italic text-white/80 mt-3 text-lg">A novel · Northern Coast Trilogy · Book Two</p>
          ...
          <div className="mt-10 flex flex-wrap items-center justify-center gap-3">
            <button onClick={()=>setCurrentTrack(chapters[0].id)} className="inline-flex items-center gap-3 rounded-full bg-canvas text-ink hover:bg-white pl-5 pr-7 py-3 text-sm font-bold transition-colors shadow-float">
              <span className="w-9 h-9 rounded-full bg-ink text-canvas grid place-items-center"><IconPlay className="w-4 h-4 ml-0.5"/></span>
      ...
      <section className="max-w-4xl mx-auto px-6 py-12">
        <div className="flex items-center justify-between mb-3">
          ...
          <span className="text-xs text-ink/50">First three chapters available · sign in for the rest</span>
        </div>
        <div className="bg-white rounded-3xl border border-ink/10 shadow-card divide-y divide-ink/5 overflow-hidden">
          {chapters.map((ch, i) => {
            const free = i < 3;
            const charsIn = Object.entries(ch.characters).filter(([id, st]) => st !== "skipped").map(([id]) => findChar(id)).filter(Boolean);
            const isPlaying = currentTrack === ch.id && free;
            return (
              <div key={ch.id} className={`grid grid-cols-[40px_60px_1fr_180px_100px] items-center gap-4 px-5 py-4 ${free ? "hover:bg-ink/[0.02]" : "opacity-60"}`}>
                <button onClick={()=>{ if (free) setCurrentTrack(currentTrack === ch.id ? null : ch.id); }} disabled={!free}
                  ...
                  {free ? (isPlaying ? <IconPause className="w-3.5 h-3.5"/> : <IconPlay className="w-3.5 h-3.5 ml-0.5"/>) : <IconLock className="w-3.5 h-3.5"/>}
                </button>
                ...
                <span className="text-sm tabular-nums text-ink/60 text-right">{free ? ch.duration : <Pill>Locked</Pill>}</span>
              </div>
            );
          })}
        </div>
      ...
      <section className="max-w-4xl mx-auto px-6 pb-20">
        <div className="rounded-3xl bg-ink text-canvas px-8 py-10 text-center">
          <p className="text-[10px] uppercase tracking-widest text-canvas/60 font-semibold mb-4">Created with</p>
          <p className="font-bold text-3xl tracking-tight">audiobook<span className="text-peach">.</span></p>
          <p className="text-canvas/70 mt-3 max-w-md mx-auto leading-relaxed">Turn your manuscript into a multi-voice audiobook in an afternoon. Voices generated from the prose, narrators reused across a series.</p>
          <div className="mt-6">
            <PrimaryButton variant="light">Start a project</PrimaryButton>
          </div>
        </div>
        <p className="text-center text-xs text-ink/40 mt-6">© 2026 Mike Dudarenok. Listening preview generated 9 May 2026.</p>
      </section>
    </div>
  );
}

Object.assign(window, { PreviewListenerView });
