...
function ListenView({ chapters, characters, library, currentTrack, setCurrentTrack, onSendApp, onRegenerate, onEnterPreview }) {
...
function ChapterListenRow({ chapter, charactersIn, isPlaying, onPlay, onRegenerate }) {
...
function Waveform({ progress, active }) {
  const bars = useMemo(() => {
    let s = 42; const out = [];
    for (let i = 0; i < 48; i++) { s = (s * 9301 + 49297) % 233280; out.push(0.25 + (s/233280) * 0.75); }
    return out;
  }, []);
  return (
    <div className="flex items-end gap-[2px] h-7">
      {bars.map((h, i) => {
        ...
        return <span key={i} className={`w-[3px] rounded-sm transition-colors ${active && filled ? "bg-magenta" : active ? "bg-ink/15" : "bg-ink/20"}`} style={{ height: `${h*100}%` }}/>;
      })}
    </div>
  );
}

function DownloadCard({ title, format, size, description, primary }) {
  return (
...
function MetaField({ label, value, type = "text" }) {
  return (
...
function ListenerAppCard({ app, onSend }) {
    ...
    <div className="grid grid-cols-[44px_1fr_120px_120px_140px_120px] items-center gap-4 px-5 py-3.5 text-sm hover:bg-ink/[0.02] transition-colors">
      ...
      <span className="min-w-0">
        ...
        {item.status === "in_progress" && (
          <div className="mt-1.5 h-1 rounded-full bg-ink/[0.06] overflow-hidden max-w-[280px]">
...
Object.assign(window, { ListenView, CoverArt, ChapterListenRow, DownloadCard, MetaField, ListenerApps, ListenerAppCard, ExportQueue, ExportQueueRow });
