...
function MiniPlayer({ chapter, onClose, onPrev, onNext, prevAvailable, nextAvailable }) {
  const [audio, setAudio] = useState({ durationSec: 0, peaks: [], url: null });
  const [currentSec, setCurrentSec] = useState(0);
  const [playing, setPlaying] = useState(true);
  const timerRef = useRef(null);
  const lastTickRef = useRef(0);

  /* Reset + fetch audio metadata on chapter change. Real backend returns a
     signed URL + duration + waveform peaks; we use them to render the
     scrubber + (later) the strip waveform. */
  useEffect(() => {
    if (!chapter) return;
    setCurrentSec(0);
    let cancelled = false;
  ...
  /* Wall-clock playhead. setInterval (not rAF) so it ticks even when the
     iframe is hidden — the prototype gets loaded into non-foregrounded
     panes during verification + side-by-side viewing. With a real
     backend this would mirror the <audio> element's currentTime. */
  useEffect(() => {
