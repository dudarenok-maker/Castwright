 ...
 * and the real counter-example, register row A29), so it lands in this
 ...
 * the ids it exists to protect (register row A29's `the-torment`/
 * `lightning-dave`).
 *
 ...
 * the narrator was substituted** (the actual A29 damage class this pass
 ...
 * (register row A30: `mayrin`, `coalfall`) while letting the one
 ...
 * (register row A29, `the-torment` itself) proved the opposite: a
 ...
 *  row A29's `the-torment`/`lightning-dave`: both have real rendered
 ...
 *  substituted (the actual A29 damage class). A round-1 version of this fix
 ...
 *  accepted, register row A30) while letting the already-fine `lady-alina`
...
export function planBookRepairs(input, deps) {
  ...
  for (const id of allIds) {
    ...
    if (matchedId) {
      ...
      //     GENUINELY WRONG — precisely register row A29's damage class,
      ...
      // accepted on 2026-08-05 (register row A30) — would have been
      ...
      // character either way"), which #2107 overturned: register row A29
 ...
 *  A30, `docs/testing/onbox-acceptance-register.md`), never by an automated
 *  test in this file. */
 ...
 *  on the exact summary line A30's precondition tells an operator to
 ...
 *  A30's own precondition tells them to trust — so this is pinned directly,
 ...
 *  correct. Register row A29 (`docs/testing/onbox-acceptance-register.md`)
 ...
 *  pre-lane aliases are exactly the ones this A30 repair workflow already
 *  visits.
 *
...
async function main() {
  ...
  // healthy workspace on the exact summary line A30's precondition tells an
  ...
  // saying a book WAS withheld — the exact line A30's own precondition
  ...
  /* #2128 — the one-shot back-fill stamp, for EVERY book scanned, not only
     ...
     are exactly the ones this A30 workflow already visits, so this is where
