...
//      (e.g. GEMINI_API_KEY) never leak into a worktree (#2345). Does NOT set
//      LOCAL_TTS_PORT: the TTS sidecar's own bind port and the server's
//      LOCAL_TTS_URL default both hardcode 9000, so setting it here would
//      make the sidecar bind a port the server never polls (#2348 review).
...
export function renderServerEnv({ slot, branch, ports }) {
  const lines = [
    ...
    `#`,
    `# Known limitation: the TTS sidecar is NOT isolated the same way — it`,
    `# still binds the hardcoded :9000 every checkout's server polls`,
    `# (spawn-sidecar.ts / LOCAL_TTS_URL), so only one worktree's sidecar can`,
    `# run at a time. LOCAL_TTS_PORT is deliberately omitted below: setting it`,
    `# would make the sidecar bind a different port than the server reads,`,
    `# breaking TTS rather than isolating it.`,
    `#`,
