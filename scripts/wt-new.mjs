...
//   5. Writes <worktree>/server/.env with PORT / WORKSPACE_DIR / LOCAL_TTS_PORT
//      for this slot — an ISOLATED workspace of its own, never the primary
//      checkout's (two servers on two branches can't safely share one
//      cast.json/state.json). Nothing is copied from the primary checkout's
//      server/.env, so secrets (e.g. GEMINI_API_KEY) never leak into a
//      worktree (#2345).
...
export function renderServerEnv({ slot, branch, ports }) {
  const lines = [
    `# Generated by scripts/wt-new.mjs — worktree slot ${slot} (branch ${branch}).`,
    `# Per-worktree server config: this worktree's server binds its OWN port`,
    `# and reads/writes its OWN workspace, so it never collides with — or`,
    `# corrupts — another worktree's (or the primary checkout's) server.`,
    `#`,
