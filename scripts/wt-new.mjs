...
// Extract slot number from the header comment of a .env.local file.
// The marker always appears on the first line: "# Generated by scripts/wt-new.mjs — worktree slot 2 (branch ...)"
// Reading only lines[0] is sound because renderEnvLocal() generates these files and always places the marker first.
// These files are never edited by hand or prepended to by external tools, so the first-line guarantee holds.
// Returns the slot number, or null if the marker is not found (malformed or missing .env.local).
export function extractSlotFromEnvLocal(envContent) {
  const lines = envContent.split(/\r?\n/);
  if (lines.length === 0) return null;
  const firstLine = lines[0];
  const match = /worktree slot (\d+)/.exec(firstLine);
...
// Scan the given worktrees and collect the slots claimed by their .env.local files.
// Returns an array of slot numbers (ascending, de-duplicated). Worktrees without
// .env.local are silently skipped — they claim no slot and don't block allocation,
// but also cannot be detected.
// This is a known and acceptable limit: EnterWorktree and Agent isolation: "worktree"
// never write .env.local, so those trees have no slot marker. Detecting them would require
// traversing the filesystem directly (expensive and fragile), so we simply note that
// unconfigured worktrees don't participate in the allocation.
//
// `worktreePaths` defaults to the live worktree list; pass an explicit array to
// point the scan at a fixture directory.
export function findClaimedSlots(worktreePaths = listWorktreePaths()) {
  const claimed = new Set();

  for (const treePath of worktreePaths) {
    const envPath = join(treePath, '.env.local');
    if (!existsSync(envPath)) {
      // No .env.local → no slot claimed. Silently skip.
      continue;
    }
    try {
      const content = readFileSync(envPath, 'utf8');
      const slot = extractSlotFromEnvLocal(content);
      if (slot !== null && typeof slot === 'number') {
        claimed.add(slot);
      }
    } catch {
      // Read error or parse error — silently skip.
      continue;
    }
  }

  return Array.from(claimed).sort((a, b) => a - b);
}
...
export async function main(argv) {
  ...
  // from existing worktrees' .env.local files. This avoids collision when
  // worktrees are torn down and recreated.
