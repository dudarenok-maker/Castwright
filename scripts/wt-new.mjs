...
//      LOCAL_TTS_PORT / PLAYWRIGHT_PORT for this slot. LOCAL_TTS_PORT is read
//      by the Node server to coordinate sidecar ownership (#2632).
...
export function renderEnvLocal({ slot, branch, ports }) {
  const lines = [
    ...
    `# LOCAL_TTS_PORT is read by the Node server to coordinate sidecar ownership (#2632).`,
...
export function renderServerEnv({ slot, branch, ports }) {
  const lines = [
    ...
    `#`,
    `# The TTS sidecar is now isolated via LOCAL_TTS_PORT (set in .env.local),`,
    `# so multiple worktree sidecars can run concurrently on different ports (#2632).`,
    `#`,
