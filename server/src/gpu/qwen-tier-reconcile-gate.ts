...
let provider:
  | ((keep: { keep06: boolean; keep17: boolean }, signal?: AbortSignal) => Promise<void>)
  | null = null;
...
export function setReconcileResidentQwenTiersProvider(
  fn: (keep: { keep06: boolean; keep17: boolean }, signal?: AbortSignal) => Promise<void>,
): void {
  provider = fn;
}

/** Calls the registered `reconcileResidentQwenTiers` and returns `true`, or
    returns `false` without doing anything when nothing has registered —
