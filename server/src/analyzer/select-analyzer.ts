/* Per-phase analyzer selection — plan 88 (pipelined two-model
   ...
   Precedence chain:
     ...
     3. saved Advanced Settings override (`analyzer.phase{0,1}.model`)
   ...
   Advanced Settings surface gives users a saved default they can
   override, while env stays the triage trump card.)
...
/** True when at least one phase knob resolves to a non-empty model —
    engaging the pipelined watermark seam. When neither does, the route
    layer runs the non-pipelined sequential phase gate. */
...
export function selectAnalyzerForPhase(opts: PerPhaseAnalyzerOptions): AnalyzerSelection {
  ...
  /* Priority 1 — explicit env. Ops needs the override for triage so
     this beats both opts.model and the saved Advanced Settings value. */
  ...
  /* Priority 3 — saved Advanced Settings override; empty falls through
