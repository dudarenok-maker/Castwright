...
describe('fs-33 — emotion survives the edits→cache→synth read path', () => {
  it('an emotion in manuscript-edits.json reaches the cache and selects the Qwen variant', async () => {
    ...
    writeFileSync(
      ...
      JSON.stringify({
        sentences: [
          ...
          { id: 2, chapterId: 1, characterId: 'sophie', text: '“Get down!”', emotion: 'angry' },
        ],
      }),
    );
    ...
    try {
      ...
      const variants = { angry: { name: 'qwen-v_sophie__angry' } };
      const picked = pickEmotionVariantVoice('qwen', variants, tagged?.emotion, 'qwen-v_sophie');
      expect(picked).toBe('qwen-v_sophie__angry');
    } finally {
  ...
  it('a missing variant falls back to the base voice (non-fatal)', () => {
    expect(pickEmotionVariantVoice('qwen', {}, 'angry', 'qwen-v_sophie')).toBe('qwen-v_sophie');
