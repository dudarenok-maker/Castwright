...
/** Resolve a `characterId` coming from manuscript attribution or a frozen
    ...
    its `supersededBy`/`rejected` fields), not a bare `supersededBy` map with
    `rejected` threaded separately (#2040 Task 17 fix round 1). The two used
    to be independent parameters, which let five of this function's six call
    sites pass `supersededBy` alone and silently default `rejected` to `[]`
    — a rejection that updated the banner and the analyzer's future matching
    but did nothing at synth/QA/splice/revision time, the one place it was
    supposed to matter (the same shared-consumer-contract defect shape as
    Wave 2's Critical on this issue). Taking one object makes that
    unrepresentable: every caller that loads `CastIdHistory` at all now
    passes the same object to both fields at once. See each call site for
    how it obtains the object (`loadCastIdHistory(bookDir)` locally, or
    threaded through as a parameter from a caller that already loaded it).

    `rejected` (#2040 Task 17, spec §4.6) is checked AFTER `exact` but ahead
    of the other three tiers — see the `rejected` field's own doc comment on
    `CastIdHistory` (`store/cast-id-history.ts`) for the fix-round-1 history:
    checking it before `exact` reintroduced #2040's original bug for any
    rejected id a later analysis reclaims as a genuine live cast row (common,
    not an edge case — the orphaned id is very often the character's own
    name). A live exact match always wins over a stale rejection. */
export function buildCastResolver<T extends CastRecord>(
  cast: readonly T[],
  history: Pick<CastIdHistory, 'supersededBy' | 'rejected'> = { supersededBy: {} },
