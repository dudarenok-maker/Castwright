// Guards the `overrides: { "adm-zip": ">=0.6.1" }` block in server/package.json.
//
...
//
// 0.6.1 fixes GHSA-7q85-xj36-vmfc / CVE-2026-77301 (published 2026-09-18): an
// entry's output buffer is allocated from the declared uncompressed size
// before validating it against the actual data, so a tiny crafted ZIP can
// force a multi-gigabyte allocation — a remote DoS for any service reading
// entries from untrusted ZIPs, which describes every EPUB upload here.
...
describe('adm-zip override', () => {
  it('resolves adm-zip at or above the patched 0.6.1', () => {
    const { version } = require('adm-zip/package.json') as { version: string };
    expect(compareVersions(version, '0.6.1')).toBeGreaterThanOrEqual(0);
  });
  ...
  it('keeps the override declared, since epub2 alone would pull a vulnerable 0.5.x', () => {
    ...
    // `>=0.6.1`, deliberately NOT `^0.6.1`: adm-zip has only ever published 0.x
    // releases, so a caret would cap at <0.7.0 and silently hold the tree back
    // when the next fix lands as 0.7.0.
    expect(pkg.overrides?.['adm-zip']).toBe('>=0.6.1');
  });
