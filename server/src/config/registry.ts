...
export const KNOBS: ConfigKnob[] = [
  ...
  // ── analyzer-chunking ─────────────────────────────────────────────────────
  {
    ...
    help: 'Maximum characters per stage-2 attribution chunk before the chapter is pre-emptively split. This value is the ceiling for BOTH families: local (context-family) engines derive min(analyzer.stage2.localInputFraction × Ollama num_ctx × ~2 chars/token, this ceiling); cloud (Gemini) engines derive min(this ceiling, the body sized from analyzer.gemini.maxInputTokensPerRequest).',
    type: 'integer',
  ...
  },
  {
    ...
    help: 'Fraction of local num_ctx reserved for stage-2 INPUT (rest is output — per-sentence JSON, which scales with section size). Lower it for a verbose local model whose output overflows the window (qwen3.5:4b / gemma4-e4b). Default 0.3. Feeds the min(fraction × num_ctx × ~2 chars/token, analyzer.stage2.chunkCharBudget) formula above; local engines only — Gemini ignores this knob.',
  ...
  },
  {
    ...
    help: 'Maximum characters per stage-1 cast-detection chunk before the chapter is split. Local (context-family) engines derive the effective budget as min(analyzer.stage1.localInputFraction × Ollama num_ctx × ~2 chars/token, this ceiling), so a large or non-Latin chapter can never overflow the context window. Cloud (Gemini) engines ignore this knob entirely: their body is sized instead from analyzer.gemini.maxInputTokensPerRequest alone, with no further ceiling (stage 1 is the one pass that has none — see that knob\'s own help).',
    type: 'integer',
  ...
  },
  {
    ...
    help: 'Fraction of local num_ctx reserved for stage-1 INPUT (rest is prompt+output). Lower it for a verbose local model that overflows the window. Default 0.7. Feeds the min(fraction × num_ctx × ~2 chars/token, analyzer.stage1.chunkCharBudget) formula above; local engines only — Gemini ignores this knob.',
    type: 'number', min: 0.1, max: 0.9, step: 0.05,
    default: 0.7,
    apply: 'live', risk: 'medium',
  },
