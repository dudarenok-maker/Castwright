...
describe('CoquiInstallBootstrap', () => {
  ...
  describe('install runs inside the sidecar hold (#2192 / #3039)', () => {
    ...
    it("an installer failure still releases the hold, still runs the ORT restore, and is the job's error", async () => {
      ...
      /* Current (buggy) behavior — see the "reported (current behavior)"
         ...
         misleadingly claims Coqui landed. The installer's own failure
         text is still present, embedded in that sentence. */
      expect(b.getJob(job.id)?.error).toMatch(/exited with code 1.*pip failed/);
      expect(b.getJob(job.id)?.error).toMatch(/^Coqui XTTS v2 installed, but restoring the GPU ONNX runtime afterwards failed/);
    });
  ...
  /* Like WhisperInstallBootstrap (and unlike QwenInstallBootstrap #3043 S1),
     CoquiInstallBootstrap's `run()` does NOT distinguish an installer failure
     from a restore failure in the reported message: whenever an
     ...
     outer branch always renders it through the single "Coqui XTTS v2
     installed, but restoring the GPU ONNX runtime afterwards failed: …"
     template — which misreports an installer failure as an
     installed-but-runtime-broken state, and (when the restore ALSO fails)
     drops the restore's own error entirely, only console.warn-ing it. These
     tests pin the ACTUAL current behavior (a pre-existing bug, out of this
