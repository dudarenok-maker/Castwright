...
describe('resolveRequired (shared by bootstrap-venv + apply.ts)', () => {
  it('defaults to the nvidia overlay (no profile arg = today): stamps nvidia, hashes overlay-then-base-then-speakerQa', () => {
    ...
    const speakerQa = readFileSync(join(SIDECAR_DIR, 'requirements', 'speaker-qa.txt'), 'utf8');
    expect(r.reqHash).toBe(computeReqHash([overlay, base, speakerQa]));
  ...
  it('.gitattributes pins requirements/*.txt to LF, so reqHash is checkout-independent (#2586)', () => {
    ...
    // 'pip-in-place' migration for every existing venv (#2586).
    //
    // Asserting only the CR-byte absence below (materialisation) is a tautology on any LF-native
    // box: it's true whether or not the .gitattributes rule exists, so on `ubuntu-latest` — every
    // `verify.yml` runner — this would pass identically with the pin deleted (#2588 pass-2
    // review). Asserting the RULE ITSELF is the platform-independent half: it reddens on deletion
    // everywhere, not just on a CRLF-materialising box.
    const gitattributes = readFileSync(join(SIDECAR_DIR, '..', '..', '.gitattributes'), 'utf8');
    const pinsRequirementsToLf = gitattributes
      .split('\n')
      .some((line) => /^server\/tts-sidecar\/requirements\/\*\.txt\s+text\s+eol=lf\s*$/.test(line.trim()));
    expect(pinsRequirementsToLf, '.gitattributes must pin server/tts-sidecar/requirements/*.txt to text eol=lf').toBe(
      true,
    );

    // Second assertion: the pin is actually IN EFFECT on this checkout. This is the only one of
    // the two that catches the stale-checkout case (a clone that existed before the pin was
    // added — git's text=auto clean filter treats an on-disk CRLF file as unchanged from its LF
    // blob, so a later pull/checkout is a no-op and the file stays CRLF; see .gitattributes'
    // comment on the rule above). Enumerated via readdirSync rather than a hardcoded list so a
    // future fifth overlay (already covered by the glob above) can't silently drop out of this
    // check the way speaker-qa.txt did (#2588 pass-2 review).
    const files = readdirSync(join(SIDECAR_DIR, 'requirements')).filter((f) => f.endsWith('.txt'));
  ...
  it('maps apple (and any unknown) to the mac-safe nvidia overlay, but stamps the given profile', () => {
    ...
    expect(r.reqHash).toBe(computeReqHash([nvidia, base, speakerQa]));
  });

  describe('speaker-qa.txt is hashed (issue #2534\'s exact failure mode, found in #2588 pass-2 review)', () => {
    it('a speaker-qa.txt content change flips reqHash and triggers pip-in-place', () => {
      // All three overlays `-r speaker-qa.txt` (nvidia-cuda.txt, cpu.txt, amd-rocm.txt), so it is
      // pip-installed alongside every profile — but resolveRequired() used to hash only
      // [overlay, base], leaving speaker-qa.txt's real pins (speechbrain, huggingface_hub)
      // invisible to reqHash. A version bump there left reqHash unchanged and decideVenvAction
      // wrongly reported 'noop' for every existing venv — #2534 verbatim, in the one requirements
      // file the #2534 guard above doesn't cover.
      const overlay = readFileSync(join(SIDECAR_DIR, 'requirements', 'nvidia-cuda.txt'), 'utf8');
      const base = readFileSync(join(SIDECAR_DIR, 'requirements', 'base.txt'), 'utf8');
      const speakerQa = readFileSync(join(SIDECAR_DIR, 'requirements', 'speaker-qa.txt'), 'utf8');
      const bumpedSpeakerQa = `${speakerQa}\n# reqHash regression probe: a speaker-qa.txt-only edit\n`;

      const currentHash = computeReqHash([overlay, base, speakerQa]);
      const bumpedHash = computeReqHash([overlay, base, bumpedSpeakerQa]);
      expect(bumpedHash).not.toBe(currentHash);

      const required = resolveRequired(SIDECAR_DIR, 'nvidia');
      expect(required.reqHash).toBe(currentHash);

      const staleStamp = { pythonTag: required.pythonTag, profile: required.profile, reqHash: bumpedHash };
  ...
  describe('side-28 ORT pin re-stamp (issue #2534): venv with stale reqHash must reinstall', () => {
    it('onnxruntime-gpu pin bump must be coupled to reqHash via requirements text change', async () => {
      ...
      const normalizedSpeakerQa = speakerQa.replace(/\r\n/g, '\n');
      ...
      const oldHash = computeReqHash([oldNvidiaContent, normalizedBase, normalizedSpeakerQa]);
      const currentHash = computeReqHash([normalizedNvidia, normalizedBase, normalizedSpeakerQa]);
