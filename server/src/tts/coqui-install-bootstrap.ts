...
export class CoquiInstallBootstrap {
  ...
  private async run(job: CoquiInstallJob): Promise<void> {
    ...
    if (ort.restoreError) {
      console.warn(`[coqui-install] onnxruntime after install: restore FAILED — ${ort.restoreError.message}`);
    } else {
      console.log(`[coqui-install] onnxruntime after install: ${ort.restoreOutcome}`);
    }
        ...
        error:
          ...
          (ort.restoreError
            ? `Restoring the GPU ONNX runtime afterwards also failed: ${ort.restoreError.message} ` +
              'The CPU Coqui may run until it is repaired — with the app closed, run ' +
    ...
    if (ort.restoreError) {
      ...
      this.transition(job, 'error', {
        error:
          `Coqui XTTS v2 installed, but restoring the GPU ONNX runtime afterwards failed: ${ort.restoreError.message} ` +
          'The CPU Coqui may run until it is repaired — with the app closed, run ' +
          'server/tts-sidecar/scripts/install-ort.mjs against the sidecar venv python.',
      });
      return;
  ...
  private spawnInstaller(job: CoquiInstallJob): Promise<void> {
    ...
    return runChild(
      ...
      [script, ...this.installArgs],
      {
        ...
        failure: (code, detail) => `install-coqui.mjs exited with code ${code}.${detail ? ` ${detail}` : ''}`,
      },
      { engineLabel: 'install-coqui', childIdleTimeoutMs: this.childIdleTimeoutMs },
    );
  }
  ...
  private restoreOrtInSidecarVenv(): Promise<OrtRestoreOutcome> {
    ...
    return restoreOrtRuntime({
      ...
      log: (m) => console.log(`[coqui-install] ${m}`),
    });
  }
