...
export class CoquiInstallBootstrap {
  ...
  private async run(job: CoquiInstallJob): Promise<void> {
    ...
    /* An installer failure propagates out of the hold (the hold still
       ...
       must say so — thrown, it would read as a failed Coqui install.
    ...
    const ort = await this.holdSidecarFn<{ outcome: OrtRestoreOutcome } | { failure: Error }>(async () => {
      ...
      try {
        ...
        if (installerError) {
          console.warn(`[coqui-install] ORT restore also failed: ${err instanceof Error ? err.message : String(err)}`);
          return { failure: installerError };
        }
    ...
    if ('failure' in ort) {
      this.transition(job, 'error', {
        error:
          `Coqui XTTS v2 installed, but restoring the GPU ONNX runtime afterwards failed: ${ort.failure.message} ` +
    ...
    console.log(`[coqui-install] onnxruntime after install: ${ort.outcome}`);
...
/** Default holdSidecarFn — see CoquiInstallOptions.holdSidecarFn. */
