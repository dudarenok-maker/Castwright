...
beforeAll(async () => {
  ...
  keeperBookId = seed(workspaceRoot, AUTHOR, SERIES, 'Keeper of the Lost Cities', {
    ...
    characters: [
      ...
      { id: 'sophie',   name: 'Sophie', voiceId: 'v_sophie', aliases: ['Sophie Foster'], gender: 'female', ageRange: 'teen' },
...
describe('GET /api/books/:bookId/series-roster', () => {
  ...
  it('preserves voiceId, aliases, gender, ageRange on each entry', async () => {
    ...
    expect(res.status).toBe(200);
    const sophie = (res.body.characters as Array<{ name: string; voiceId?: string; aliases?: string[]; gender?: string; ageRange?: string; bookId: string; bookTitle: string }>)
