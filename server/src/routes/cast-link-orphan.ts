/* POST /api/books/:bookId/cast/:characterId/link-orphan-match  (#2238)
   ...
   2. Reserved fold-bucket ids (`unknown-male`, `unknown-female`,
      `fold-minor-cast.ts`'s `MALE_BUCKET_ID`/`FEMALE_BUCKET_ID`) are never
      auto-recorded book-wide by the repair script (plan 122,
      `fold-minor-cast.ts:349-354`) because the bucket stands in for
      MULTIPLE unrelated background characters sharing one voice slot —
      aliasing a real, addressable orphaned id permanently onto a shared
      bucket is not a reconciliation, it is a lossy merge the user almost
      certainly doesn't intend. Refused here (400) as defence in depth; the
      UI also disables the control with a visible reason so the refusal is
      never the user's first signal.
   3. `withCastLock`. This route never calls `writeJsonAtomic(castJsonPath(`
      — it reads cast.json ONCE, purely to validate `characterId` names a
      live character (a 404 otherwise), and that decision is never paired
      with a write in the same request. CLAUDE.md's cast-lock rule ("lock
      the innermost read-through-write") governs a read that a write
      DEPENDS on; there is no read-through-write cycle here to lock —
      `retireCharacterId` is the only write this route performs, and it owns
...
/** Decision 2 above — the reserved minor-cast fold buckets. Mirrors
    `voice-override-linked.ts`'s own local `BUCKET_IDS` convention (a small,
    file-local `Set` rather than importing a shared one) except this file
    DOES import the two ids from their single source of truth
    (`fold-minor-cast.ts`) rather than re-typing the literals — there is no
    third bucket (`narrator`) relevant here, so a bespoke two-entry set is
    clearer than reusing a set shaped for a different check. */
...
castLinkOrphanRouter.post(
  ...
  async (req: Request, res: Response<LinkOrphanMatchResponse | { error: string }>) => {
    ...
    /* Decision 2 — refuse a reserved fold bucket outright rather than
       silently recording a lossy alias. */
    if (RESERVED_FOLD_BUCKET_IDS.has(characterId)) {
      return res.status(400).json({
        error:
    ...
    console.log(`[cast-link-orphan] book=${bookId} linked "${orphanedId}" -> ${characterId}`);
