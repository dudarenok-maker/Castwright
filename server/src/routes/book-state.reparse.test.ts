...
describe('reparse handler — reuse/voice carryover (srv-13)', () => {
        ...
        characters: [
          {
            ...
            voiceId: 'sophie',
            voiceState: 'reused',
            matchedFrom: { bookId: 'b0', characterId: 'sophie', confidence: 0.91 },
  ...
  it('refreshes the carryover from the CURRENT cast (no resurrection of a removed link)', async () => {
    ...
    writeFileSync(
      carryoverPath(),
      JSON.stringify({ characters: [{ id: 'sophie', voiceState: 'reused', voiceId: 'sophie' }] }),
    );
    // …but the user has since unlinked Sophie in the live cast.
    writeFileSync(
      castPath(),
      JSON.stringify({ characters: [{ id: 'sophie', name: 'Sophie', voiceState: 'generated' }] }),
    );
