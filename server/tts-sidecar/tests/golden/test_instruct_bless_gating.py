...
def _write_baseline(tmp_path: Path, *, tolerances: dict | None) -> Path:
...
def test_bless_writes_tolerances_on_a_never_before_blessed_baseline(monkeypatch, tmp_path) -> None:
    """First bless -- no committed tolerances to protect yet, so no flag is
    needed even though `rtf_max` (1.31) differs from nothing."""
    path = _write_baseline(tmp_path, tolerances=None)
...
def test_bless_refuses_and_leaves_the_file_untouched_when_rtf_max_would_move(
    ...
    path = _write_baseline(tmp_path, tolerances=dict(BASE_TOLERANCES))
    monkeypatch.setattr(instruct, "BASELINE_PATH", path)
    monkeypatch.delenv("GOLDEN_REBLESS_THRESHOLDS", raising=False)
