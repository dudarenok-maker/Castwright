...
- CI & gating: local pre-commit/pre-push hooks are instant (ESLint on staged files, guards, and a scope-gated sidecar check — see below); GitHub Actions' verify.yml is the required, enforcing status check on every PR, opt-out (not opt-in) since the repo went public.
...
- Precommit & prepush lifecycle (ops-2997, 2026-09):
  - prepare runs husky; pre-commit runs `node scripts/hooks/pre-commit-lint.mjs` (ESLint over staged JS/TS files only, one process, no vitest pool); pre-push runs its guards plus `node scripts/verify-cache.mjs --steps test:sidecar --scope-branch` (scope-gated to `server/tts-sidecar/**`, near-zero on a push where that step is out of scope or already cached), and is skipped entirely for a docs-only push. Cloud verify.yml is the enforcing gate; no local hook runs a test battery anymore.
- **Committing is instant. A sidecar-touching push is not — launch that one detached and poll.**
  - A hook may never spawn a process pool (docs/superpowers/specs/2026-09-05-commit-gate-rebalance-design.md). pre-commit is one ESLint process over the staged files; pre-push is a few guard scripts plus a single scope-gated `test:sidecar` pytest run. The old "5–7 minute commit" is gone: a foreground `git commit` now finishes sub-second.
  - **What is NOT gone is the 30-second single-command cap.** The Copilot CLI kills a single command at 30 seconds, and `test:sidecar` costs **~6.85 minutes** when it runs. A foreground `git push` in that case is not slow, it is *killed* — and retrying it never helps.
  - **The rule, stated for every agent lane and not just this one: launch `git push` detached and poll whenever the branch diff touches `server/tts-sidecar/**`.** Everything else — every `git commit`, and any push whose branch diff does not touch that path — may run in the foreground.
  - **Evaluate that mechanically, do not predict.** `git diff --name-only main...HEAD -- 'server/tts-sidecar/**'` — non-empty means detached. It is `main...HEAD`, NOT `origin/main...HEAD`: the hook's own `branchDiffFiles` (`scripts/verify-cache.mjs`) merge-bases against the LOCAL `main`, and with a stale local `main` the two disagree in the dangerous direction — this test says "foreground" while the hook puts `test:sidecar` in scope. Three successive revisions of this guidance tried instead to enumerate which diffs were slow, and all three under-listed in the same direction: the agent read its own change as exempt, took the foreground path into the kill, and reached for `--no-verify`. Two cases the path test cannot see, so relaunch detached rather than retrying if a foreground push is still alive at ~25 s: a root `package.json`/`package-lock.json`/`.github/actions/**` change (`computeShared` in `scripts/verify-cache.mjs` puts every step in scope), and a cold verify cache (an in-scope `test:sidecar` executes instead of reporting `[cached]`).
  - **Launch it detached**, redirect its output into a scratch dir whose name is **freshly generated per attempt** (a GUID suffix — never a fixed `%TEMP%\push.log`, or you poll another run's sentinel and report a push you never made), append an `EXIT=` sentinel as the last line, and poll with short commands: check the **process first**, the sentinel second — while the process is alive, any `EXIT=` you can see belongs to an earlier attempt. Each poll returns instantly, so the cap stops mattering. Polling is active waiting and is correct; what is wrong is ending your turn while it runs. Budget ~7 minutes; settle any disagreement against the repo itself (`git status -sb`), because a pid can be reused and a log line can be stale while a ref either moved or did not.
  - **Never `--no-verify`.** This is the exact failure that tempts it, and reaching for it is the tell: it means you have diagnosed a *slow* command when what you actually have is a *capped* one. On 2026-08-19 an agent did precisely this and produced correct, well-tested code that never left the box. See CLAUDE.md "Do not use `--no-verify` to bypass".
