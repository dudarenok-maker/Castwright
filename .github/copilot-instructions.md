...
- Precommit & prepush lifecycle (ops-2997, 2026-09):
  - prepare runs husky; pre-commit runs `node scripts/hooks/pre-commit-lint.mjs` (ESLint over staged JS/TS files only, one process, no vitest pool); pre-push runs its guards plus `node scripts/verify-cache.mjs --steps test:sidecar --scope-branch` (scope-gated to `server/tts-sidecar/**`, zero cost on most pushes), and is skipped entirely for a docs-only push. Cloud verify.yml is the enforcing gate; no local hook runs a test battery anymore.
- **Committing is instant now — no detached/poll dance needed.**
  - A hook may never spawn a process pool (docs/superpowers/specs/2026-09-05-commit-gate-rebalance-design.md). pre-commit is one ESLint process over the staged files; pre-push is a few guard scripts plus, only when `server/tts-sidecar/**` is in the branch diff, one pytest run. Neither runs `verify:fast:scoped`/`verify:fast:branch`'s old batteries, so the "5-7 minute commit"/"30-second CLI kill" problem this section used to describe no longer applies — a foreground `git commit`/`git push` finishes in the hooks' own budget (sub-second for pre-commit; up to ~7 min for pre-push only on a sidecar-touching push).
  - **Never `--no-verify`.** Still true regardless of how fast the hooks are — see CLAUDE.md "Do not use `--no-verify` to bypass".
- Tests organization:
