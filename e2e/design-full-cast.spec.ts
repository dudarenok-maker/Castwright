...
/**
 * "Design full cast" — bulk Qwen voice design from the cast view, surfaced as
 * the third top-bar status pill (alongside Analysis + Generation).
 *
 ...
 * ttsModelKey), then clicks "Design full cast": every character is
 * "Needs voice" on a Qwen project with no designed overrides, so the bulk job
 * runs (mock api.startCastDesign emits a designed voice per character). The
 * status pill shows "Designing", the rows flip to "Designed" as each completes,
 * and the button disappears once the cast is fully designed.
 *
 * Why browser-level: the button → designAllRequested → cast-design middleware →
 * mock SSE → slice → top-bar pill + live row flips crosses the router / redux /
 * layout seams the unit tests (src/store/cast-design-*.test.ts,
 * src/views/cast.test.tsx, src/components/top-bar.test.tsx) cover in isolation.
 */
...
test.describe('cast view → Design full cast', () => {
  test('designs every needs-voice character, shows the pill, flips rows to Designed', async ({
    ...
    /* Rows flip to "Designed" as each character_designed lands, and the button
       disappears once every character has a voice (needs-voice count → 0). */
    await expect(designBtn).toHaveCount(0, { timeout: 15_000 });
