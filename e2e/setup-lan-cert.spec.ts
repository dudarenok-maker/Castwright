...
test('first-run wizard surfaces the LAN-access cert step and can regenerate', async ({ page }) => {
  ...
  // environment→ffmpeg→analysis→voice→defaults→library→lanCert = 6 Next clicks
  // (Next is never gated; fe-49 split Models into analysis + voice; the Library
  // step was inserted after Defaults, pushing lanCert from step 6 to step 7).
  for (let i = 0; i < 6; i++) {
