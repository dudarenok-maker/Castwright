...
for (const scene of SCENES) {
  ...
  test(`capture ${scene.id}`, async ({ page }, testInfo) => {
    ...
    /* Full-page screenshots scroll-and-stitch multiple viewport-sized strips
       together. top-bar.tsx's `<header className="sticky top-0 …">` stays
       pinned to whatever viewport slice was active when Playwright captured
       it, so it gets composited into the stitched image at that scroll
       offset instead of once at the true top — a duplicated/misplaced header
       floating mid-page on any fullPage scene tall enough to need more than
       one strip. Forcing it to `position: static` just for the fullPage shot
       renders it once, in its real DOM position (the top of the page). */
    if (fullPage) {
