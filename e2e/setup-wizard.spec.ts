...
test('first-run wizard renders with step UI when not ready', async ({ page }) => {
  ...
  // Step 1 shows "Step 1 of 6" progress indicator
...
test('wizard reaches the last step and shows the finish button', async ({ page }) => {
  ...
  // Advance through steps 1 → 2 → 3 → 4 → 5 (Next disappears on step 6)
  for (let i = 0; i < 5; i++) {
    await expect(next).toBeVisible();
    await next.click();
  }

  // On step 6 the wizard's Next is gone; StepFinish owns the finish button
...
test('Tier-1 smoke test runs and renders audio (mock)', async ({ page }) => {
  ...
  // Advance through steps 1 → 2 → 3 → 4 → 5 → 6 (Finish) via Next ×5.
  // On step 6 the wizard's Next is gone; StepFinish owns the finish button.
  const next = page.getByRole('button', { name: /^next$/i });
  for (let i = 0; i < 5; i++) {
    await next.click();
...
test('re-entry opens on the summary board and drills into the wizard', async ({ page }) => {
  ...
  await expect(page.getByText(/step 1 of 6/i)).toHaveCount(0);
