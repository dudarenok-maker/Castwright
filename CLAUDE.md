...
4. **Update the two release-notes documents, in this PR.** Append an entry to `docs/release-notes-next.md` (technical register, PR-refed — see [CONTRIBUTING.md "Release notes"](CONTRIBUTING.md#release-notes)) AND a matching user-facing, brand-voice line to the in-progress version section at the top of `RELEASE_NOTES.md`. Land both PR-by-PR, not reconstructed from git history at cut time — that's the whole point of this step. **First PR landing after a cut** (neither file has been reset yet — `docs/release-notes-next.md`'s `release-notes-next-version:` marker is not ahead of `package.json`'s current version, since nothing bumps it forward automatically): this PR resets both — bump the marker to the next version (increment minor, this project's usual cadence, unless the team is doing a patch release) and clear `docs/release-notes-next.md`'s stale body from the last release, then open a fresh top section in `RELEASE_NOTES.md` headed with that same new version — before appending your entry to each. Otherwise both files already have an open draft to append to. Skip only when the change has no shippable delta (pure docs/process, CI-only, internal chore with no user- or operator-visible effect) — say so explicitly rather than silently omitting.
