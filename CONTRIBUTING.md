...
```
[allow] docs/release-notes-next.md honoured 2 literal(s): "CAFÉ™", "groß—"
```

— which is also exactly why the example marker lines in this section are safe to keep here: the gate never reads CONTRIBUTING.md, so documenting the syntax here can never arm one. "Reaches the file" is a real qualifier, not throat-clearing: the CLI checks `RELEASE_NOTES.md` before `docs/release-notes-next.md` and stops at the first failure, so a marker armed in the second file goes unechoed on a run where the first already failed — publication is blocked either way, so this can't reintroduce a silent arming, but it does mean the echo is not literally unconditional.

Reach for a marker only when you have satisfied yourself the text is legitimate — the ordinary answer to a mojibake failure is to re-encode the file, not to allow the span. **A marker is legal only in `docs/release-notes-next.md` (#1985)**: that file is cleared by hand in the first PR after a cut, which happens to take any marker in it along with the rest of the body, so a marker there self-expires. `RELEASE_NOTES.md` is cumulative and never reset, so the gate **refuses any marker it finds there outright** — the run fails, naming the file, rather than silently honouring or merely warning about it (an ignored marker would otherwise read to the author as "my marker worked"). The way out for a genuine false positive in `RELEASE_NOTES.md` is the same as for any other unwanted failure: re-encode the offending text. `bump-version.mjs --force` additionally downgrades the whole mojibake check to a warning for that release, but only when the failure is hit through `bump-version.mjs` itself — `release.yml`'s own "Guard — committed release notes are real" step invokes this gate's CLI directly with no bypass of its own, so re-encoding is the only way out there. A green gate is also not a proof of cleanliness: a lossy double-encode (`Â` + NBSP flattened to `Â` + a plain space) does not decode back and so is never reported.
