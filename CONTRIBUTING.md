...
**A literal must extend beyond the span it excuses** — quote the whole surrounding word (`CAFÉ™`), never the flagged characters on their own. A marker naming just `É™` suppresses nothing at all, not even its own copy in the marker line, and the failure output says so rather than letting you think it worked: a literal is excused at every occurrence, so accepting a bare span would blind the file to it wholesale, and a flagged span standing alone between spaces is by construction a real mangle rather than the accent-beside-punctuation false positive the marker exists for. Widening the suggestion is not a substitute for the rule — the dominant corrupted shape is a mangled dash, arrow, ellipsis or emoji sitting between spaces, which is already its own whole word, so there is nothing to widen to. The gate therefore prints a paste-able marker line **only when a valid literal genuinely exists** around the span; otherwise it tells you the span has no surrounding context and points you at re-encoding. Two shapes are ignored outright, on purpose: a marker inside a fenced code block (so documenting this syntax in a gated file cannot arm it), and a marker that is not complete on one line.

Reach for a marker only when you have satisfied yourself the text is legitimate — the ordinary answer to a mojibake failure is to re-encode the file, not to allow the span — and note that **nothing expires a marker for you**. `docs/release-notes-next.md` is cleared by hand in the first PR after a cut, which happens to take any marker in it along with the rest of the body; `RELEASE_NOTES.md` is cumulative and never reset, so a marker added there stays live and keeps that literal excused for every future release until someone deliberately deletes it. A green gate is also not a proof of cleanliness: a lossy double-encode (`Â` + NBSP flattened to `Â` + a plain space) does not decode back and so is never reported.
