...
- _What:_ In-app voice-sample **capture** (record/upload) with quality guidance + re-take. Explicit **consent on the record** (names the person + permitted use); cloning blocked without it. **Clone + cast** via Qwen (reference-clip clone, the default engine) first, then XTTS zero-shot reference as the second engine; held consistent across book + series. **Voice-library split:** cloned voices get their own `#/voices` section with provenance + consent, **excluded** from the cross-book "offer it back" matcher so a person's voice is never reused into a stranger's book. **Local-only**; export explicit.
- _Benefit:_ the most personal, gift-able feature the product can offer — and the strongest consumer hook.
- _Status:_ **Wave 1 delivered** (2026-07-24, branch `feat/frontend-voice-library-wave1`) — the book-independent voice-library store, the three-section `#/voices` restructure (My voices | In use | Catalogue), and standalone designed-voice authoring (create / redesign-with-compare / promote / assign); folds in `fs-12` as fully delivered. **Wave 3a delivered** (ingest/consent/recorder, behind the since-removed `voices.library.enabled` flag). **Wave 3b1 delivered** — the first user-visible clone on the Qwen engine (capture → consent → derive → advisory ECAPA fidelity check → persist, wired via a "Clone a voice" wizard), plus the first never-silent-substitution fix for Qwen (see `docs/features/267-fs38-wave3-voice-clone.md`). **Wave 3b2 delivered** — the resolver/lifecycle half of the never-substitute invariant: a per-chapter pre-pass classifies every in-chapter cloned voice Healthy/Repairable/Broken before any synth call, transparently re-deriving a Repairable one and hard-failing a Broken one instead of ever rerouting; revoke/delete now share one total artifact-erasure routine; §2.3 designed-voice clip-persist + orphan self-heal shipped alongside (see `docs/features/268-fs38-wave3b2-resolver.md`). **Remaining:** Catalogue rebuild, 3c (XTTS clone), polish. See `docs/features/194-voice-cloning.md` and the Wave 3 spec (`docs/superpowers/specs/2026-07-25-fs38-wave3-clone-pipeline-design.md`).
