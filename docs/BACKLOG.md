...
### 3. Global queue modal for chapter generation (cross-book, reorderable, regen-safe)

Source: net-new (2026-05-23). User-flagged during the queue-refactor planning round; subsumes the former Should #1 ("Generation SSE survives Node hot-reload during dev") because the SSE reconnect / idempotent server-side resume ack is the same surface touched by the queue dispatcher rewrite. Full Phase-1 exploration + Round 1 wave decomposition captured in `~/.claude/plans/a-large-piece-of-refactored-bird.md` — move into `docs/features/NN-global-queue-modal.md` when Round 1 starts.

- _What:_ Promote the implicit "chapter queue" (today: subset of chapters where `state ∈ {queued, in_progress}` in `src/store/chapters-slice.ts`, with all queueing logic living inside the generation-stream middleware) into a first-class, persisted, cross-book queue surfaced by a global "View queue" modal mounted in `src/components/layout.tsx`. Five coupled pieces. **(a) New `queue-slice`** — explicit FIFO of entries shaped `{ id, bookId, chapterId, scope: 'this' | 'forward' | 'character', characterId?, addedAt, status: 'queued' | 'in_progress' | 'paused' | 'done' | 'failed', order: number }`. Persisted to a **single workspace-level** `<workspace>/.queue.json` (one file holds the cross-book queue, chosen for first-class freedom to mix-and-match order across books — e.g. "regenerate these 2 chapters of Book 1, then generate these 5 chapters of Book 5, then regenerate all of Book 6") via the existing `writeJsonAtomic` contract from `server/src/workspace/state-io.ts`; `GET /api/queue` reads the file directly. A `forward` scope (regenerate chapter N + all subsequent) **expands at enqueue time** into one entry per affected chapter so each chapter in the forward range can be reordered individually. Book-delete hook prunes stale entries by `bookId` from the workspace queue file in the same transaction that drops the book directory. Reorderable via drag-handles on desktop + tap "Move up / Move down" pills on touch (touch-equivalence rule from CLAUDE.md plan 81); the currently-`in_progress` entry is pinned and non-draggable. **(b) Generation dispatcher refactor** — replace the hard-interrupt regen path at `src/store/generation-stream-middleware.ts:325-347` with an enqueue-only flow. The middleware drains FIFO; the in-flight entry runs to completion before the next entry starts. Regenerate-during-regenerate becomes "append to queue" instead of "close handle + restart." Cross-book draining honours the per-book SSE contract (one handle per book) — books with queued work alternate at chapter boundaries based on `queue.order` regardless of which book is currently viewed. The slice's `pendingRegen` + `regenEpoch` + `paused` fields are stripped; `paused` migrates onto the queue slice as a queue-global flag. **(c) Global queue modal** — new `src/modals/queue-modal.tsx` mounted in Layout, responsive per CLAUDE.md mobile protocol (full-screen on `<640px`, dialog on `≥640px`). Lists every queue entry with book title + chapter title + status + per-entry progress + ETA. Click an entry → router pushes `#/books/<bookId>/generate?chapter=<chapterId>` and the Generate view scrolls the chapter row into view (the URL grammar already supports this; the Generate view today does not act on `currentChapterId` — adding the consumer is part of this work). Modal hosts the **Resume / Pause** control relocated from `src/views/generation.tsx:587-607` — pause becomes queue-global, not per-book-active-handle. **(d) Wire every regenerate site to enqueue** — all 10 existing trigger sites dispatch a new `queueActions.enqueue` thunk that appends to the queue and pops a `notifications-slice` toast ("Added to queue · n entries pending") with a "View queue" link. Sites: per-chapter "Regenerate" in `src/views/generation.tsx`, per-chapter "Regenerate" in `src/components/listen/listen-player-region.tsx`, failed-chapter "Retry" in `src/views/generation.tsx`, header "Regenerate" (all done) in `src/views/generation.tsx`, engine-drift "Regenerate all" banner in `src/views/generation.tsx`, cast-view selection-pill "Regenerate" in `src/views/cast.tsx`, drift-report per-chapter "Regenerate this chapter" in `src/modals/drift-report.tsx`, drift-report auto-queue path in `src/components/layout.tsx:1031-1055`, voice-edit `StaleAudioBanner` "Regenerate now" in `src/components/stale-audio-banner.tsx`, profile-drawer per-character "Regenerate" in `src/modals/profile-drawer.tsx`. Generate view's per-chapter button gains a paired "Add to queue" + "View queue" affordance replacing the existing single-action button. **(e) SSE survival + idempotent resume (absorbs former Should #1)** — `realStreamGeneration` in `src/lib/api.ts:2763-2828` auto-reconnects when EventSource ends while the queue has in-flight work; server-side `server/src/routes/generation.ts` emits a `resume_from` ack first (snapshot of the active queue entry's progress + already-completed chapters within that entry) so the reconnect doesn't replay. Same seam covers `tsx watch` hot-reload during dev AND production crash recovery (server bounce, Node OOM). Cross-tab queue mutations broadcast through the existing `src/store/broadcast-middleware.ts` (new `QUEUE_BROADCAST_ACTIONS` set; per-entry progress ticks stay non-broadcast like per-chapter ticks today, single-user-per-workspace contract intact).
- _Acceptance:_ With Books A + B both having queued work, enqueue chapter 5 of Book A → switch to Book B → enqueue chapter 3 of Book B → "View queue" modal shows both entries with `A.ch5` above `B.ch3`. Pause from the modal → both books stop at their next chapter boundary. Resume → `A.ch5` completes, then `B.ch3` starts. While `A.ch5` is in-flight, enqueue `A.ch7` from any view (per-chapter button, listen view, drift modal) → `A.ch7` appears at the bottom of the queue, `A.ch5`'s in-flight progress is preserved (no drop). Drag `A.ch7` above `B.ch3` in the modal → next dispatch after `A.ch5` is `A.ch7`. The in-flight entry is non-draggable and shows a visible pin. Click `A.ch5` in the modal → router lands on `#/books/<A>/generate?chapter=5` and the Generate view scrolls the chapter row into view. Restart the Node server during a generation (`tsx watch` triggered by editing any `server/src/` file) → frontend reconnects within ~3 s, `resume_from` ack restores the progress bar without replaying completed chapters, and the queue keeps draining; "Worker has gone quiet" banner does NOT appear. Reload the page mid-queue → queue restores from the workspace-level `<workspace>/.queue.json`, the in-flight entry resumes its SSE, no work is lost. Voice-drift fix scenario: from `DriftReportModal`, batch-regenerate 3 chapters across 2 books → 3 queue entries land in order, all 3 drain sequentially across books without dropping any in-flight work. Toast fires on each entry completion with `dedupeKey: queue-complete-${entryId}`. Phone viewport (`<640px`) renders the modal full-screen with bottom-sheet reorder gestures; tablet / desktop render as dialog with drag handles. Touch targets ≥44×44 px per WCAG 2.5.5. New paired tests: Vitest pins for `queue-slice` reducers + selectors + persistence; Vitest pins for the middleware dispatcher (FIFO order, in-flight pinning, regen-as-enqueue, cross-book alternation, reconnect-with-`resume_from`); existing chapter-row Vitest specs updated to dispatch through the new enqueue thunk; new Playwright e2e (`e2e/queue-modal.spec.ts`) drives enqueue → reorder → pause → resume → reconnect across the three responsive projects (chromium + mobile-chrome + tablet-chrome) per CLAUDE.md mobile protocol; existing `e2e/responsive/coverage.spec.ts` extended with the queue modal case so it auto-runs at every viewport.
- _Key files:_ **Queue + dispatcher** — new `src/store/queue-slice.ts` (slice + actions + selectors + persist-config carve-out); new `src/store/queue-thunks.ts` (`enqueue` + `reorder` + `cancel` thunks consolidating the 10 existing regenerate dispatches; `enqueue` with `scope: 'forward'` expands at enqueue time into one entry per affected chapter so the slice never carries a "forward range" as a single entry); rewrite of `src/store/generation-stream-middleware.ts:152-459` (replace `pendingRegen` consumption + the hard-interrupt at lines 325-347 with FIFO drain reading from `queue-slice`; keep the reverse-local-analyzer guard at lines 284-320 intact); `src/store/chapters-slice.ts:34-101` (strip `pendingRegen` + `regenEpoch` + `paused`; the per-chapter `state` field stays the way the UI renders the per-chapter status); `src/store/broadcast-middleware.ts` (add `QUEUE_BROADCAST_ACTIONS`); new `server/src/workspace/queue-io.ts` (atomic read / write of the single workspace-level `<workspace>/.queue.json`); new `server/src/routes/queue.ts` (`GET /api/queue` reads the workspace file, `POST /api/queue/enqueue`, `POST /api/queue/reorder` for cross-book ordering, `DELETE /api/queue/:entryId` for cancel); book-delete hook in the existing book-deletion route (locate via `grep -nP 'delete.*book|book.*delete' server/src/routes/*.ts`) prunes any queue entries whose `bookId` matches the deleted book in the same write transaction. **Modal + UI** — new `src/modals/queue-modal.tsx` (responsive, modeled on `src/modals/profile-drawer.tsx`'s breakpoint behaviour at lines 246-407); new `src/components/queue-entry-row.tsx` (drag-to-reorder + tap pills, modeled on the PointerEvent handler at `src/views/manuscript.tsx:377-451`); `src/components/layout.tsx` (mount queue modal; relocate Resume / Pause from generation.tsx); `src/views/generation.tsx` (delete Resume / Pause at lines 587-607; add "View queue" + "Add to queue" buttons; add `id={\`chapter-${chapter.id}\`}` to ChapterRow div near line 1006 + scroll consumer reacting to `currentChapterId` from `useAppSelector`); `src/components/top-bar.tsx` (queue count chip opening the modal). **Regenerate trigger rewiring** — `src/views/generation.tsx` ChapterRow regen + retry buttons; `src/components/listen/listen-player-region.tsx:316`; `src/views/cast.tsx:752` (cast selection-pill regen); `src/modals/drift-report.tsx:33,87` + `src/components/layout.tsx:1020-1055` (both `DriftReportModal` paths — onRegenerateChapter and auto-queue); `src/components/stale-audio-banner.tsx:27-47`; `src/modals/profile-drawer.tsx:1221-1223`; `src/components/regenerate-modal.tsx` + `src/components/character-regenerate-modal.tsx` + `src/components/batch-character-regenerate-modal.tsx` (each modal's `onConfirm` dispatches `queueActions.enqueue` instead of `chaptersActions.regenerate*`). **SSE survival (absorbed former Should #1)** — `src/lib/api.ts:2763-2828` (auto-reconnect on EventSource end while queue has in-flight work); `src/store/generation-stream-middleware.ts` (reconnect-driven handle reopening, distinct from queue-drain reopening); `server/src/routes/generation.ts:1-78` (emit `resume_from` ack with completed-chapter snapshot for the active queue entry on every new subscriber; reuses the existing `chapter_complete` catch-up pattern at line 19). **OpenAPI** — `openapi.yaml` (new `QueueEntry` schema + queue endpoints + `resume_from` SSE event shape; regenerate `src/lib/api-types.ts` via `npm run openapi:types`). Wave decomposition into 6 parallel-agent branches + branch names + rebase notes + per-wave verify steps in `~/.claude/plans/a-large-piece-of-refactored-bird.md`; move into the design doc when Round 1 opens.
- _Depends on:_ none structural. Reuses the existing per-book SSE contract (`server/src/routes/generation.ts` — one handle per book, catch-up on subscribe), the `writeJsonAtomic` + rotation contract from `server/src/workspace/state-io.ts`, the `notifications-slice` `pushToast` shape, the responsive modal pattern from `src/modals/profile-drawer.tsx`, the PointerEvent drag pattern from `src/views/manuscript.tsx:377-451`, the `broadcast-middleware.ts` cross-tab sync seam, the per-engine `overrideTtsVoices` cast map (drift detection unchanged), and the existing `pendingRegen` consumption path as the implementation reference for the FIFO drain. Pairs structurally with Must #1 (in-app upgrade) — `queue.json` needs a schema-version entry in the migration family Must #1 introduces; this slots into Must #1's `server/src/workspace/{cast,manuscript-edits,revisions,...}-migrate.ts` set as a sibling `queue-migrate.ts`. **Subsumes the former Should #1** ("Generation SSE survives Node hot-reload during dev") in piece (e); the standalone Should #1 entry is removed from this BACKLOG in the same docs PR that adds Must #3.
- _Benefit (user / architectural):_ closes the two cited bugs in one structural change. (1) **Regenerate-during-regenerate no longer drops in-flight work** — today's `closeHandle()` at middleware line 346 abandons the partial chapter audio when the user clicks Regenerate on a different chapter; the FIFO replaces this with append-and-wait. (2) **Cross-book voice-drift fixes stop racing** — batch regenerate across 3 books today fires 3 separate `pendingRegen` dispatches that fight over the single-book `currentBookId` guard; the cross-book queue with explicit ordering removes the race entirely. **Architecturally:** decouples scheduling from book-active state — the queue is the source of truth, not the chapter-slice's implicit ordering, which lays the seam for future "Schedule overnight" / "Generate when GPU is idle" / "Pause this book, prioritise that one" affordances. The SSE-survival piece (absorbed former Should #1) clears the "Worker has gone quiet" stall banner that hits during every `tsx watch` restart in dev, and gives production users a crash-recovery story without losing the visible progress thread. The concurrent-multibook invariant from the standing memory becomes a tested property of the queue dispatcher instead of a fragile cross-slice handshake. Foundation also enables Could #13 (within-chapter sentence parallelism) — the queue dispatcher is the natural place to gate K-wide synth dispatch per entry.
