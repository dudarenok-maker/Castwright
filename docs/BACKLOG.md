...
- _What:_ Bring up AMD GPUs for the TTS sidecar. **Phase 1 shipped** (the public-beta ship gate): the Python 3.11→3.12 transition + NVIDIA-latest torch + a layered-requirements structure + a detect-and-reinstall alpha path, with all AMD code paths **dormant + unit-tested** so Phase 2 is a flip not a rewrite (pure `accelerator-profile.mjs` resolver + `venv-migration.mjs` decision core + `python-tag.txt`/`ensure-python312.mjs`; NVIDIA install set unchanged; `apply.ts` refuses a 3.11→3.12 in-place upgrade). **Phase 2 (this item) is now BUILT DORMANT** (branch `feat/sidecar-amd-gpu-phase2`, Waves A–G): the `amd-rocm.txt`/`cpu.txt` overlays + ROCm wheels + resolver-driven installers, profile→overlay wiring (stamp carry-forward → no forced migration), the sidecar runtime consuming the injected profile, the `/health` rocm/directml enum, the cached Kokoro DirectML self-test, the `ACCELERATOR` knob + first-run picker + `gpu-acceleration-unavailable` FailureCode, and a job-coordinated profile-switch route. The seamless in-place rebuild was descoped (P1's detect-and-reinstall handles a profile switch). **Only the on-AMD acceptance (Wave H) remains** — requires an **AMD-owning tester** (a partial DirectML check may be attemptable on the author's Radeon 780M iGPU).
