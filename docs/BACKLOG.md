...
### 32. Series-level cast in the manuscript speaker / segment-reassign pickers

Source: net-new (2026-05-21). User-reported during low-confidence triage on a series-Book-2 manuscript: a recurring series character (e.g. "Councillor Linnet") that exists in the series-level roster but was missed by the analyzer for the current book is not pickable in the per-sentence / per-segment reassign menus — only the local book's cast is listed. Workaround today is to leave the manuscript view, open the ProfileDrawer for the misattributed character, manually link to the series-roster entry via the existing "From prior books in this series" optgroup, then come back and reassign — three context switches to fix one line.

- _What:_ Extend the two speaker pickers in `src/views/manuscript.tsx` — the `SegmentRow` segment-level dropdown (around lines 998-1019) and the `SegmentInspector` segment + per-sentence pickers (around lines 1176-1208 and 1225-1237) — to also list characters from prior books in the same series, fetched via the existing `api.getSeriesRoster(bookId)` (`src/lib/api.ts:260-269`) that already powers `ProfileDrawer`'s linking optgroup at `src/components/layout.tsx:187-214` + `:1000-1010`. Group the series-roster entries under a separator labelled "From prior books in this series" with a per-row book-title subtitle so disambiguation is obvious ("Councillor Linnet · Northern Coast Trilogy · Book 1"). When the user picks a series-roster entry, dispatch a new `cast/addFromSeriesRoster` thunk (`src/store/cast-slice.ts`) that creates a local cast member with `matchedFrom: { bookId, characterId, bookTitle, confidence }` populated from the roster entry and the prior `voiceId` preserved, THEN dispatch the existing `setSentenceCharacter` / `setSentencesCharacter` reducer with the newly-created local id. The materialise-then-assign sequence matches the precedent the ProfileDrawer already established.
- _Acceptance:_ Open a Book-2 manuscript whose analyzer missed a recurring series character. In the manuscript view, click any sentence misattributed to a local character → the segment dropdown AND the SegmentInspector per-sentence picker each show the local cast on top, then a "From prior books in this series" group below populated from `getSeriesRoster()`. Picking a series-roster entry adds the character to the book's local cast (visible in the cast view with `matchedFrom` set + the prior voice still bound) AND reassigns the sentence in one click — no context switch out of the manuscript view. Existing per-book picker behaviour for sentences whose right speaker IS already in the local cast stays byte-identical. New Vitest covers the `addFromSeriesRoster` thunk's cast.json write shape; new Playwright spec under `e2e/manuscript-reassign-from-series.spec.ts` covers the materialise-then-assign flow end-to-end.
- _Key files:_ `src/views/manuscript.tsx` (both pickers: SegmentRow dropdown ~998-1019, SegmentInspector pickers ~1176-1208 + ~1225-1237); `src/store/cast-slice.ts` (new `addFromSeriesRoster` thunk that mirrors the ProfileDrawer's manual-link pattern); `src/components/layout.tsx` (the `getSeriesRoster()` cache already lives here at `:187-214` — extend the context the manuscript view consumes); `src/lib/api.ts:260-269` (`SeriesRosterEntry` type — likely no shape change needed); new Vitest spec for the slice thunk; new `e2e/manuscript-reassign-from-series.spec.ts`. Update the regression plan that owns the manuscript-view reassign flow (likely `docs/features/archive/40-manuscript-segments.md` or the active plan that supersedes it — confirm at pick-up time).
- _Depends on:_ none — the `getSeriesRoster()` API + `SeriesRosterEntry` type + `Character.matchedFrom` field already ship today. Pairs naturally with #33 (the most common reason to fast-jump to a low-confidence sentence is to fix exactly this kind of misattribution).
- _Benefit (user):_ closes the recurring-series-character workflow gap that's visible the moment a series character isn't pulled into a new book's analysed cast. Today the fix is three context switches (manuscript → profile drawer → cast → back); after this lands the same fix is one click in the picker the user is already looking at. Particularly load-bearing for trilogies / long series where the analyzer's per-book cast detection can't see the broader roster.

### 33. Fast navigation between low-confidence attributions in the manuscript view

Source: net-new (2026-05-21). User-reported during the same low-confidence triage session that surfaced #32. The header already shows a "X low-confidence" count (`src/views/manuscript.tsx:464-467` filters `chapterSentences.filter((s) => s.confidence != null && s.confidence < 0.75)`) but there's no affordance to jump to those sentences — user has to scroll-by-eye through the chapter looking for the orange "Low confidence" pill rendered at `src/views/manuscript.tsx:965-966`.

- _What:_ Promote the existing "X low-confidence" stat in the manuscript header from a passive counter to an active navigator: render it as a pill with ▲ / ▼ controls (next / previous), each click scrolls the next low-confidence sentence into view AND opens the `SegmentInspector` on it so the user can reassign without another click. Bind J / K (or ↓ / ↑ with the manuscript view focused) as keyboard shortcuts mirroring the pill. Wrap-around behaviour: pressing next on the last low-confidence sentence cycles to the first. Disable / grey-out the pill when the count is 0. Optional stretch (call out in the plan, defer if it inflates scope): a density gutter alongside the scrollbar showing low-confidence positions across the chapter with click-to-jump.
- _Acceptance:_ Open a chapter with K low-confidence sentences (the canonical Marlow-story fixture at `C:\Users\dudar\Downloads\the Coalfall Commission.txt` chapter 1 should reliably surface several). The header pill shows "K low-confidence ▲ ▼". Clicking ▼ (or pressing J) scrolls the next low-confidence sentence into view AND opens the `SegmentInspector` selected on it. Pressing ▼ K times wraps back to the first. K=0 renders the pill greyed/disabled. After reassigning a sentence (its confidence becomes null, or it transitions above the 0.75 threshold) the counter and the cycle decrement in lockstep — ▼ from a now-fixed sentence jumps to the next still-low-confidence one. New Vitest covers the "find next low-confidence sentence index given current index" selector with wrap-around + post-reassign decrement; new Playwright spec under `e2e/manuscript-jump-low-confidence.spec.ts` covers the keyboard + click jump-then-reassign flow end-to-end.
- _Key files:_ `src/views/manuscript.tsx` (header pill at ~464-467 → active control; scroll-into-view via the existing inspector scroll-to-selected pattern at `:1092+`); `src/store/manuscript-slice.ts` or a new selector file (low-confidence index selector — derive from existing `confidence` field at the 0.75 threshold already used at `:464,965`); new Vitest spec for the selector (wrap-around, K=0, post-reassign re-derivation); new `e2e/manuscript-jump-low-confidence.spec.ts`. No new redux shape needed — derivable from existing sentence array.
- _Depends on:_ none. Pairs naturally with #32 — the most common reason to jump to a low-confidence sentence is to fix exactly the misattribution #32 makes one-click instead of three context switches. Shipping #32 + #33 together closes the full triage loop (find → reassign) to keyboard-shortcut + one-click per misattribution.
- _Benefit (user):_ today the user scrolls a 300-sentence chapter looking for orange "Low confidence" pills by eye — every misattribution costs ~5-15 s of scroll. After this lands, full low-confidence triage on a chapter is K key presses regardless of chapter length. The pairing with #32 turns the "fix a misattributed recurring series character" loop from a 30-60 s workflow into ~3 s.

### 34. Typeahead search in the manuscript speaker / segment-reassign pickers

Source: net-new (2026-05-21). Surfaced during the same low-confidence triage session that produced #32 + #33: as the cast list grows — especially once #32 appends series-roster entries — the per-sentence reassign picker overflows the viewport without a way to filter. User scrolls a long dropdown to find a single character.

- _What:_ Add a typeahead search input at the top of both speaker pickers in `src/views/manuscript.tsx` — the `SegmentRow` dropdown (~lines 998-1019) and the `SegmentInspector` segment + per-sentence pickers (~1176-1208 and ~1225-1237). Filter the rendered character list as the user types, case-insensitive substring match against the character's `name` (and `aliases` if populated — confirm at pick-up). Once #32 ships, the same search input also filters the "From prior books in this series" group below. Auto-focus the search input when the picker opens. Arrow keys navigate the filtered list; Enter picks the highlighted entry; Esc closes the picker. Cap the rendered list height with internal scroll (~12 rows visible at once) so the picker never extends past the viewport regardless of total roster size. Likely the right factoring is to extract the picker into a shared `<CharacterSearchPicker>` component under `src/components/` since the segment + per-sentence pickers share most of the markup — confirm at pick-up.
- _Acceptance:_ Open a book whose cast (local + series after #32) has >12 characters. Click any sentence's speaker pill → the picker opens with a search input focused. Type "ali" → list filters to just "Linnet" / "Alicia" / etc. Arrow-down + Enter assigns the highlighted character. Esc closes the picker. Picker height never exceeds ~12 rows × row-height; internal scroll handles longer lists at any viewport. Existing one-click behaviour for short casts is unchanged (the search input sits at the top of the menu; users can still mouse-click any visible row without typing). New Vitest covers the search-filter + keyboard nav (arrow keys, Enter, Esc, focus-on-open); e2e coverage extends the #32 spec (`e2e/manuscript-reassign-from-series.spec.ts`) to type-then-pick rather than scroll-then-pick.
- _Key files:_ `src/views/manuscript.tsx` (both pickers: SegmentRow dropdown ~998-1019, SegmentInspector pickers ~1176-1208 + ~1225-1237); new `src/components/character-search-picker.tsx` (extracted shared picker) — or inline in both call sites if the duplication is small enough; new Vitest spec for the picker's filter + keyboard nav. E2e covered by extending `e2e/manuscript-reassign-from-series.spec.ts` (from #32) to exercise the typeahead path.
- _Depends on:_ none structural — search is independent of #32 / #33. But pairs strongly with #32 (search is what makes the longer post-#32 list still usable on mobile / tablet) and #33 (search shrinks picker height so the inspector-after-jump UX doesn't clip on short chapters or small viewports). Shipping all three together is the natural unit — single regression plan, one shared component, one shared e2e spec.
- _Benefit (user):_ keeps the reassign picker usable as casts grow. Today a 20-character cast is rare but workable; series-Book-N casts can easily hit 30-40 once #32 lands, and the mobile-tablet viewports from plan 81 make picker overflow worse on small screens. With typeahead, finding any character is one or two keystrokes regardless of cast size or viewport.
