...
3. ⛔ *(gated on `srv-32`/`srv-35`)* `app-3` ([#543](https://github.com/dudarenok-maker/AudioBook-Generator/issues/543)) — Delta sync engine (per-book pull via the manifest's per-chapter `urlSuffix` — **never hardcode `.mp3`**; **range-resume from the partial `.tmp`** (no restart at byte 0) + integrity; **atomic `.tmp`→rename** swap, defer if playing; **Android foreground service** so big downloads survive backgrounding; evict via active-ID diff). _Benefit:_ no full-book resync; downloads resume + finish in the background. _Depends:_ `srv-35`.
