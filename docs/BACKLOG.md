...
> _GitHub issues for these `app-*` / `srv-32` / `srv-34` / `srv-35` / `srv-33` items are
> filed after the user's review of plan 188; until then each row links the plan. IDs +
> positions are final. (Two 2026-06-06 reviews — secondary + cross-item coherence — are
> folded into the plan; see its "Secondary review" + "Relationships" sections.)_
...
3. `app-3` — Delta sync engine (per-book pull via the manifest's per-chapter `urlSuffix` — **never hardcode `.mp3`**; **range-resume from the partial `.tmp`** (no restart at byte 0) + integrity; **atomic `.tmp`→rename** swap, defer if playing; **Android foreground service** so big downloads survive backgrounding; evict via active-ID diff). _Benefit:_ no full-book resync; downloads resume + finish in the background. _Depends:_ `srv-35`.
4. `app-4` — Offline library store (per-chapter audio + **actual codec extension** + cover **thumbnails** (~250×250, also for lock-screen) + storage accounting + auto-eviction). _Benefit:_ listen anywhere; cache never fills the phone; lists stay smooth.
5. `app-5` — Native player (background, lock-screen, Bluetooth, **per-book state**; media keys default to **seek ±30/±15 s** not chapter-skip; **autosave ~5–10 s** survives an OS kill; **lock-screen artwork = local thumbnail** not full-res, so the media service can't OOM). _Benefit:_ table-stakes UX that survives backgrounding + car controls.
