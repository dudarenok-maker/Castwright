...
- _Benefit (user / architectural):_ removes the manual upgrade rite (download zip → extract → `npm ci` → restart) every alpha tester walks through every release; replaces it with a single click in the Account tab, with auto-backup-before-migrate as the data-integrity contract. Surfaces the version delta + release notes inline so testers always know what changed. Atomic rollback path when an upgrade goes sideways means the user never wakes up to a half-applied state. Architecturally: establishes a per-file schema-migration seam across the rest of the workspace (today only `state.json` has its own) so every future non-additive shape change has a tested backup-then-migrate path instead of a one-shot ad-hoc script. (Note: `fs-2`'s `language` field shipped as an _additive_ field and needed none of this — the coordinator is plumbing for _future_ non-additive changes, not fs-2.) Foundation work also enables future BACKLOG items `ops-1` (Windows installer) and `ops-2` (Docker image) to share the same `RELEASE_NOTES.md` + `/api/info` plumbing.
- _Status:_ **implemented** on `feat/server-in-app-upgrade` (delivered as the **versioned-directory** layout, not the original in-place swap — see [docs/features/171-in-app-upgrade.md](features/171-in-app-upgrade.md)); pending the v1.6.0 release cut. Remove this row when the PR merges.
