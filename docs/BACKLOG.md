...
#### `fs-38` — warn at cast time when a cloned voice cannot derive on the engine it will route to ([#1980](https://github.com/dudarenok-maker/Castwright/issues/1980))

- _What:_ A cloned voice can be assigned to a character whose routed engine can render it, while the OTHER engine's artifact is terminally un-derivable (e.g. a Qwen derive needs a non-empty `master.transcript`; a clip ingested without one can never derive, and the resulting 400 persists as `status: 'failed'`, which nothing in `server/src` can clear). `/assign` (per #1933) writes BOTH engine slots for every cloned entry, so that character now carries a slot backed by a dead artifact. #1933 adds an assign-time `warning`, but that advisory only fires at the moment of assignment. **The likeliest path into the bad state does not pass through assign at all**: a book-level engine switch silently moves every default-riding character onto the other engine, long after any assign happened. The user then discovers the problem as a hard chapter failure mid-render.
- _Benefit:_ *Benefit (user):* moves the discovery of an unrenderable cast from chapters deep into a render, back to cast time, where it can still be fixed cheaply. Requested directly by the repo owner while deciding #1933: hard-failing at render "leaves it to hard fail not letting them know earlier at cast time to allow to fix." Blocks/relates: #1933
_Full detail + acceptance:_ [#1980](https://github.com/dudarenok-maker/Castwright/issues/1980).
...
- _What:_ Follow-up to **fe-39** (#799). fe-39 added `group-active:` touch press-feedback mirrors to the *enumerated inner-span* decorative controls, but deliberately scoped **out** the sibling **container-level** hover effects on those same `group` elements. This item mirrors the highest-value one: the "Add book" tile's **outer-tile** hover tint. Today `src/components/library/library-grid.tsx` (the `NewBookCard` button, `data-tour-id="new-book-btn"`) carries a container-level hover on the outer tile: ``` hover:border-peach hover:bg-peach/4 ``` fe-39 mirrored only the *inner circle* span (`group-active:bg-peach …`), leaving the outer tile's border/background tint inert on touch. This item adds the matching `active:` mirror to the outer tile so the whole control — not just its inner badge — flashes on press: ``` active:border-peach active:bg-peach/4 ``` (These are plain `active:` variants on the element that carries the `hover:` — not `group-active:` — because the outer tile is the interactive `<button>` itself, so `:active` applies to it directly.) Optionally, the two other fe-39 groups have lower-value sibling hovers that could be mirrored in the same pass if desired, but they are **not** required for this item: continue-listening **card button** — `hover:shadow-float hover:border-ink/20` (`continue-listening-rail.tsx`) wizard **Review row button** — `hover:bg-ink/[0.03]` (`setup-wizard.tsx`)
- _Benefit:_ _(no Benefit section found in #1798 — fix the issue body, see the parseability audit)_
