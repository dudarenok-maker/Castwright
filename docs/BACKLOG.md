...
### 44. Make the workspace queue authoritative for reconcile-driven runs (plan 110 Part B)

Source: plan 110 Part A ship (2026-05-24). Part A made the queue modal / chip *reflect* a live run when the workspace queue is empty (read-side overlay derived from `chapters.activeStream`). But the underlying gap remains: the PRIMARY generation path — first generation after analysis, and resume-on-reopen — opens its SSE via the `generation-stream-middleware` reconcile (`hasWork(chapters)`) and writes NO `.queue.json` entry, so those runs have no REAL reorderable/cancelable entries.

- _What:_ When reconcile auto-opens a resume/first-run stream (after `runner.open(...)` on the no-handle branch in `generation-stream-middleware.ts`), silent-enqueue the viewed book's non-excluded `in_progress`+`queued` chapters (deterministic ids `resume-<bookId>-<chapterId>`, always `scope: 'this'`, dedupe against existing entries) via a new `silent` option on `enqueueQueueEntries` (skip toast, swallow 409). Remove entries completion-driven: on each `chapter_complete` tick in `generation-stream-runner.ts` `handleTick`, DELETE the matching `scope:'this'` (bookId, chapterId) entry (plus an idle-sweep belt-and-braces). Widen `StreamRunnerStore` with an optional `queue?`.
- _The trap (must be covered by a regression test):_ naively enqueuing makes the dispatcher re-drive already-done chapters into an infinite regen loop when the reconcile stream ends (its `inFlightEntryId` is null). The completion-driven dequeue runs while `activeStream` is non-null (dispatcher parked), so done chapters' entries are gone before the dispatcher's post-clear tick. Add the canonical no-loop integration test (both middlewares + runner): reconcile opens → enqueues → all chapters complete → idle → clearActiveStream ⇒ no regenerate fires, rows stay `done`, queue ends empty.
- _Key files:_ `src/store/queue-thunks.ts` (silent enqueue), `src/store/generation-stream-middleware.ts` (enqueue-on-open), `src/store/generation-stream-runner.ts` (dequeue-on-complete + idle sweep). Frontend only — existing `POST /api/queue/enqueue` + `DELETE /api/queue/:entryId` suffice (`cancel()` is idempotent, `enqueue()` 409s on dup ids). `queue-dispatcher-middleware.ts` stays untouched.
- _Depends on:_ plan 110 Part A shipped. Restores plan 102's stated invariant ("all generation flows through the queue; the modal reflects the truth") for the reconcile path.
- _Benefit (user / architectural):_ pausing/reordering/cancelling a resumed run from the modal works on real entries (today they're synthetic, read-only). Architecturally collapses the two SSE-open paths back onto one authoritative `.queue.json` record. On `stable`, flip plan 110 to stable + archive it.
