...
- **One-command LAN bootstrap** — `mkcert` drops a local root CA on every dev box;
...
- **Portable book bundle** — `GET /export/portable` streams a single `.zip`
  (state + manuscript + audio + cover + change-log + MANIFEST); `POST
  /import/portable` accepts it with rename / replace / skip conflict modes (plan 75).
...
- **Drift modal collapses ~300 events into ~6–18 cards** — one card per `(book ×
  character × snapshot)` instead of one per event (~7,200 DOM nodes → ~200), with
