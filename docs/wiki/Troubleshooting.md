...
- **To authorize an actual phone/tablet:** from a tab that's already loopback-authorized (`localhost`), go to **Admin → LAN access**, name the device, and click **Authorize a device** — this shows a pairing QR. Scan it with that device's camera and it authorizes itself. Note: the card only shows the QR image, not the code as text, so there's currently no way to authorize a second desktop browser tab this way without a camera to scan through — `localhost` is the practical option for same-machine testing.
