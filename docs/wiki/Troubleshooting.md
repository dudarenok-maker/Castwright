...
### The app loads at castwright.local but the library won't — "This browser is no longer authorized"

**What you saw:** `https://castwright.local` (or the raw LAN IP, with or without `:8443`) loads the app shell fine, but the library panel shows "Couldn't load your library — This browser is no longer authorized for Castwright on your network," followed by a pointer to the fix, without a page reload. As of v1.15.0 that pointer comes from the server itself and names the port this install **actually bound**, so the whole line reads "This browser is no longer authorized for Castwright on your network. Missing or invalid LAN access token. Start pairing from `https://localhost:<port>` or `https://castwright.local` on the computer running Castwright." (Before v1.15.0 the panel showed only the raw server text, `Library scan failed (401): Missing or invalid LAN access token.`, with no way out.)

**Why:** LAN access is guarded by a `LAN_AUTH_TOKEN`, so every non-loopback request is required to pair first — this applies to the friendly `castwright.local`/`castwright.dev.local` hostnames and any raw LAN IP exactly the same as a phone or tablet, **even when you're typing that address into a browser on the desktop machine itself.** As of v1.13.0 that token is **auto-generated on first boot** whenever LAN is on (which is the production default), so this pairing requirement is expected, not a sign you misconfigured anything. Only `https://localhost:8443` is exempt (it's recognised as loopback and skips pairing). Since v1.15.0, an authorization also lasts up to a year rather than a month by default, so this dead end should come up far less often — see below for the click that gets you back in when it does.

**What to do:**

- **Fastest, same machine only:** open `https://localhost:8443/#/` instead — no pairing needed, same server and data.
- **Fix it from `localhost` in one click:** from a tab that's already loopback-authorized, go to **Admin → LAN access** and click **Authorize this browser**. No device name, no QR, no camera — it re-links *this computer's* `castwright.local` browser directly and drops you back on the library, authorized. This is the click the 401 panel's own pointer text is describing. It only works when the server can confirm `castwright.local` is actually reachable (the mDNS responder and the `:443` forwarder both up — the same prerequisite the pairing-link button below already needed); if it isn't, the button explains that and falls back to the QR flow instead of failing silently.
- **To authorize an actual phone/tablet:** from that same loopback tab, name the device and click **Authorize a device** — this shows a pairing QR. Scan it with that device's camera and it authorizes itself. For testing from a different desktop browser tab on the same machine, the LAN Access card also displays a one-click link — "Open pairing link on castwright.local" — right next to the QR whenever the friendly hostname is confirmed reachable; clicking it opens a new tab and skips straight to the "Authorize this browser?" confirmation without needing a camera. This link only appears once the server confirms the mDNS responder and the `:443` forwarder are both up and working (falling back to QR-only if they're not yet reachable, such as under `dev:lan` where this link is unavailable). `localhost` remains the quickest same-machine option and doesn't require any of this setup.
