...
**What you saw:** `https://castwright.local` (or the raw LAN IP, with or without `:8443`) loads the app shell fine, but the library panel shows "Couldn't load your library — This browser is no longer authorized for Castwright on your network," followed by a one-line pointer to the fix (the panel used to show the raw server text, `Library scan failed (401): Missing or invalid LAN access token.` — it now names the problem and the way out directly, in place of it, without a page reload).
