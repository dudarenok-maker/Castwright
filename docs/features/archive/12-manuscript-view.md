...
- **Detected list sorts by current-chapter activity.** Characters in the Detected card are ordered by line count in the currently-selected chapter (descending); roster order is the stable tiebreaker. Characters with zero lines in the chapter fall to the bottom and render at `opacity-60` so they're still reachable for cross-chapter reassignment but don't compete with the chapter's actual speakers for vertical space. When no chapter is selected, `counts` falls back to a global tally, which surfaces the busiest speakers across the whole book. The active filter pill always wins over the dim — clicking a silent character to filter the manuscript to it makes the row loud + colored.
...
- **The slice's "empty" state is `[]`, never the app's built-in demo fixture (#1356).** `manuscript-slice.ts`'s initial state and `reset()` both default `sentences` to `[]`, and `ManuscriptView`'s fallback for a momentarily-null `sentences` prop is a stable `EMPTY_SENTENCES` constant — not the Halloran-ship demo fixture from `src/data/sentences.ts`. Server-side, `PUT /:bookId/state` (`slice: 'manuscript'`) shape-validates the patch against `sentenceSchema` and 409s a save that would wipe an already-analysed manuscript (existing on-disk `sentences.length > 0`) down to zero — the combination that let a stale/mid-hydration client state permanently bake the demo fixture into a real book's `manuscript-edits.json`.
...
9. **Large-book sidebar** — load a 500-chapter book (e.g. the new bulk fixture, 509 chapters / 30 cast). The Detected card is visible at the bottom of the sidebar without scrolling. Mouse-wheel over the Chapters card scrolls the chapter list in place; the cast card does not move. Type `epilogue` into `Filter chapters…` and the list collapses to matching rows. Clear the filter and all rows return. Click Next from chapter 200 and the active row is scrolled into the chapter scroll region on its own. Click any chapter and watch the Detected list re-order so the chapter's busiest speakers (Narrator + a handful of recurring characters) float to the top; silent characters fall to the bottom at reduced opacity but stay clickable.
