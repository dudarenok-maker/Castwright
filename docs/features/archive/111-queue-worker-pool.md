...
## Update — 2026-05-26: force-remove escape hatch for orphaned in_progress entries

The reconcile loop (`queue-dispatcher-middleware.ts` STEP 1) only completes entries in its **in-memory** `inFlight` map. That map does not survive a page reload, so an entry marked `in_progress` on disk before a reload (e.g. a chapter that failed, then the tab reloaded mid-run) becomes an **orphan**: the dispatcher won't reconcile it (not in the fresh map) and won't re-claim it (STEP 2 only claims `queued`). It then can't be deleted — the `DELETE` route 409s on `in_progress` — so orphans accumulate and the queue wedges. Only a server restart cleared them (`resetOrphanedQueueEntries` resets `in_progress`→`queued` on boot).

Fix: a **force-remove** escape hatch. `cancel(file, id, { force })` and `DELETE /api/queue/:entryId?force=true` drop even an `in_progress` entry (a still-live stream just idles into a no-op reconcile against the now-gone id); the queue modal renders a "Remove" control on every in-flight row (`queue-entry-<id>-force-remove`) wired to `cancelQueueEntry(id, { force: true })`. The normal cancel button still 409-guards `in_progress` (pause-first) for genuinely-running entries. Mock queue (`src/mocks/mock-queue.ts`) mirrors `?force=true`. Auto-reclaiming orphans client-side was deliberately NOT done (it would race a sibling tab genuinely running the chapter — the concurrent-multibook invariant); manual force-remove + the server boot-reset cover it. Coverage: `queue-io.test.ts`, `queue.test.ts`, `queue-thunks.test.ts`, `queue-modal.test.tsx`, e2e `queue-modal.spec.ts`. Shipped via `fix/frontend-queue-force-remove`.

## Update — 2026-05-26: failed chapters PERSIST as "Failed" in the queue + Retry

Previously the dispatcher's reconcile (STEP 1) done-pruned EVERY entry whose chapter's stream closed — success and failure alike (`/complete` always sent `done`). A failed chapter therefore vanished from the queue with no trace; the only signal was the changelog/per-chapter row. Now a failed chapter LINGERS in the queue as `failed` with its `errorReason`, and offers a Retry.

Mechanism — the failure outcome is threaded **frontend-side** (the dispatcher is the queue authority; the server stays out of `.queue.json`):
- The runner (`generation-stream-runner.ts`) records each `chapter_failed` reason in a `chapterFailures` map keyed `${bookId}::${chapterId}`, set **unconditionally** (a cross-book chapter's entry must be markable even though its rows aren't in the viewed slice). The map outlives the `idle` close that follows. `takeChapterFailure(bookId, chapterId)` returns + clears it (one-shot, so a later success can't read a stale failure).
- The dispatcher's reconcile calls `takeChapterFailure`; on a hit it `completeQueueEntry(id, { outcome: 'failed', errorReason })` (entry → `failed`, lingers) and crucially does NOT add the id to the no-reclaim `completed` set, so a Retry can re-claim it; on a miss it done-prunes as before.
- `completeEntry` already supported the `failed` outcome; `POST /api/queue/:id/complete` now reads `{ outcome, errorReason }` (default `done` for back-compat). New `retry(file, id)` mutator (`failed` → `queued`, clears `errorReason`/`progress`) + `POST /api/queue/:id/retry`; thunk `retryQueueEntry`. The queue modal renders a Retry control (`queue-entry-<id>-retry`, `IconRefresh`) on `failed` rows. Mock queue mirrors the `/complete` outcome body + `/retry`.

Coverage: `queue-io.test.ts` (retry mutator), `queue.test.ts` (`/complete` failed + `/retry`), `generation-stream-runner.test.ts` (`takeChapterFailure` one-shot, survives close, cross-book), `queue-dispatcher-middleware.test.ts` (fail→linger→retry→re-claim integration), `queue-thunks.test.ts`, `queue-modal.test.tsx`, e2e `queue-modal.spec.ts`. Shipped via `feat/frontend-queue-failed-retry`.
