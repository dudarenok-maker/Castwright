...
- **`'forward'` scope expands to non-excluded chapters only.** The modal's forward set is computed by `forwardRegenChapters(chapters, anchorId)` (`src/store/chapters-slice.ts`) = the anchor plus every later chapter with `!c.excluded`. Excluded front/back-matter (Dedication, Copyright, CONTENTS) is never enqueued — feeding it to generation produces empty no-content queue entries — and the modal's forward count + ETA (`forwardCount` / `forwardDurationSec`) read off the same filtered set, so the displayed scope can't diverge from what actually queues. Mirrors the `!c.excluded` predicate used by `regenerateChapterIds` and `enqueueOnWork`.
