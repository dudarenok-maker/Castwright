...
- **`parseAndValidate` rescues four classes of malformed model output** (shared with the Ollama analyzer via `repairUnescapedQuotes` / `trimTrailingProse` / `repairStructuralPunctuation` exports from `gemini.ts`):
  ...
  3. Trailing prose after the outer closing brace (`trimTrailingProse`) — model finished its JSON and then continued writing free-form commentary. Ch44 pos 37588 shape in the Coalfall Commission regression.
