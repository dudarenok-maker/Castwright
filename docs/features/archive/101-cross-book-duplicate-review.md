...
- **Mock-mode parity.** `api.linkPrior` + new `api.notLinkedTo` shipped in `src/lib/api.ts`'s mock branch with realistic latency + symmetric local-store mutation. Mock fixture: the Hollow Tide books 1+2 share a Wren/Wren-Foster unlinked pair to trigger the chip on first load.
...
- **Vitest unit (`src/views/voices.test.tsx`)** — new `describe('LibraryView cross-book duplicate review (plan 101)')`:
  - Asserts `⚠ N duplicate candidate(s)` pill appears on `af_aoede` family with Wren/Wren-Foster fixture.
...
Run in mock mode (`VITE_USE_MOCKS=true`) — the the Hollow Tide #1+#2 fixture seeds the unlinked Wren/Wren-Foster pair.
...
- **Shipped:** 2026-05-25 via branch `fix/frontend-duplicate-review-hydration` (PR pending). Plan stays `status: stable` in `archive/`.
