...
- **Symptom:** The same "duplicate keeps coming back" report, but sharper: on the global `#/voices` tab the ⚠ pill vanished when the user clicked "Same character — link them" (toast: success), yet a **page reload** brought it straight back. It looked like the link/merge never persisted. The user's real case: *Terik* (Exile) ↔ *Councillor Terik* (Everblaze) on `kokoro/am_onyx`, where Everblaze's "Councillor Terik" already carries "Terik" as an (analyzer-generated) alias on disk.
