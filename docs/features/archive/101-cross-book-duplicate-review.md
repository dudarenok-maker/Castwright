...
- **Detecting cross-base-voice duplicates.** If the user hand-tuned Wren to `af_alloy` in The Ebb while keeping `af_aoede` in The Tidewatcher’s Oath, the predicate's same-family rule will miss them. Out of scope — covered by the existing same-base-voice surface.
