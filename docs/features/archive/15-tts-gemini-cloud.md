...
- Output is MP3-encoded and cached identically to the local sidecar: `server/audio/voices/{voiceId}-{modelKey}-<paramHash>.mp3`. Cache key includes the model key so different Gemini models don't collide.
...
1. **Configure key, switch engine to Gemini, click Preview** → `POST /api/voices/<voiceId>/sample` with `modelKey: 'gemini-2.5-flash'`. Within a few seconds, audio plays. MP3 cached on disk.
...
6. **Chapter generation with Gemini** — start a generation run with `modelKey: 'gemini-2.5-flash'`; chapters synthesise via Gemini and write MP3s to disk; rate-limit pauses surface as chapter-failed ticks with a retry-after hint where available.
