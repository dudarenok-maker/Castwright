...
> Status: stable — shipped 2026-06-13 (five waves + post-ship UX polish, all CI-green). Real-hardware on-box acceptance (Mac/Linux Kokoro install, venv bootstrap, etc.) was de-scoped from a tracked residual on 2026-06-13: the install paths ride along the beta install runs, and any failure that surfaces there gets filed as a fresh bug. Backlog item `fs-21` / issue #752 closed accordingly. The acceptance matrix below is retained as the beta-run checklist.
