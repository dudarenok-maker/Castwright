...
- **Server:** a new optional `Voice.sampled` flag (OpenAPI `Voice` schema; mirrors `generated`). `aggregateVoices` (`server/src/routes/voices.ts`) stamps it for the `engine=qwen` query by checking the voice-sample cache (`listVoiceSampleFiles`, new in `server/src/tts/voice-sample-cache.ts`) for a `<scope>-qwen3-tts-0.6b-*.mp3` file, where `scope = voiceId ?? char-<characterId>` — the same scope the player + design route cache under (`sampleScopeFor`). Anchored on the single Qwen synth model key so a `sophie` scope can't false-match a `sophie-foster` file. Cross-book OR-aggregated like `generated`; preset path untouched (scan runs only for `engine=qwen`).
