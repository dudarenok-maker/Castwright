...
Shipped 2026-05-28 · `c46b850` (feat(scripts): gate release tag creation on a green cross-OS run). `bump-version.mjs` blocks tag creation on a red cross-OS run; `pickWorkflowRun` selection + `--skip-cross-os` escape hatch covered by the bump-version unit tests.
