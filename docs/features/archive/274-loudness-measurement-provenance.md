...
Shipped 2026-07-29 on `feat/server-plan-274-loudness-provenance`, closing #1922 and
#1923. Decision 1 = A′ (grandfather legacy sidecars) was implemented as locked;
commits (T1 onward): `3e5d662a` (T1/T2), `7071ed78` (T4), `33170757` (T5),
`9fc17ef0` (T6), `71e06f9e` (T7), `48654c4d` (T8).

Behaviour delta vs. the plan: none — T1–T10 landed exactly as designed. Confirmed
during implementation: T9's golden-assembly suite stayed green with **zero**
re-blessings (§1.7's prediction held); T6's pre-existing frontend fixture corpus
(`loudness-report.test.tsx`, `listen-player-region.test.tsx`) stayed green
**unedited**, which is the acceptance evidence §3.1 predicted for the grandfather
clause. One incidental defect found and fixed while implementing T6 (not called out
in the plan): `loudness-report.tsx`'s Measured/Drift table cells gated on
`lufs.twoPass` directly rather than on `classifyDrift`'s `bucket`, so a
`measurementSource: 'loudnorm'` row's Status pill would have read "No measurement"
while the adjacent cells kept printing the fabricated LUFS figure — fixed in the
same commit, covered by a paired test.
