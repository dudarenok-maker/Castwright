...
- Playwright e2e (`e2e/voice-library.spec.ts` / a Coqui cast/audition spec,
  Task 30, headline case added Task 41) — **covers both the assign-time
  rejection path AND a successful Coqui assign.** `_mockAssignGuardError`
  (`src/lib/api.ts`) was re-mirrored against the real `/assign` route's
  current guard order (revoked → not-ready → wrong-*non-clone-capable*-
  engine) — it rejects only an engine that is neither Qwen nor Coqui, so a
  cloned voice with a ready `xtts` slot now assigns cleanly to a
  Coqui-routed character in the mock. Step 7 drives exactly that: it selects
  Coqui as the character's engine, assigns `lib-cloned-demo` (the fixture
  Task 29 gave a ready `xtts` slot specifically for this), and asserts the
  assign succeeds with no error surfaced — the headline "assigns on Coqui
  and succeeds" case. This bullet itself used to claim the opposite (no
  Coqui-success coverage existed) — that was exactly this wave's own
  signature defect class ("an artifact asserts what its source no longer
  says") reproduced inside its own plan; corrected here rather than left
  standing. A real browser-level walkthrough on live hardware (not the mock)
  is still what Section E's E-01 (on-box) exists to close.
...
- **(a) All three of the plan's originally-not-landed tasks have since
  ...
  - **Task 10a landed** (`493d6b8f` + a same-day fix round) — the
    ...
    [#1899](https://github.com/dudarenok-maker/Castwright/issues/1899), fixed
    on this branch — closes when this PR merges.
...
- **(g) ~~`voiceSubstitutedFrom` is not carried through a re-record merge~~
  CLOSED.** `[ADV-H6]` — the per-line diagnostic field that records when a
  rendered line used a substituted voice did not survive the re-record/merge
  path that folds a repaired line back into a chapter's segment list.
  Root cause was produced-but-dropped (shape (a), not (b)):
  `synthesiseChapter` (`server/src/tts/synthesise-chapter.ts`) always
  computed the field correctly per segment, but it was dropped at three
  separate points downstream — both repair routes' inline `synth` closures
  (`server/src/routes/chapter-splice.ts`, `chapter-qa-repair.ts`, the latter
  also in its local `best` take-tracker) never read it off the fresh
  segment, and `buildSynthReplacements`'s `freshVerdict` construction
  (`server/src/audio/build-synth-replacement.ts`) never carried it into the
  `SegmentReplacement` the splice engine merges (`server/src/audio/
  splice-chapter.ts`). Fixed by threading it through all four sites —
  unconditionally, unlike `voiceName`/`baseVoiceName`'s omit-when-absent
  rule, since a re-record always has a definite substitution answer and an
  omitted key would let a clean take's merge preserve a stale prior
...
- **(h) The manual cast-link consent bypass (copy direction) is closed;
  ...
  fixed: `cast-link-prior.ts:193-197` now carries a
  `sourceIsCloned`/`targetIsCloned` fail-safe guard (`characterHasClonedSlot`)
  refusing to denormalise a cloned slot across a manual link in either
  direction, and `series-reuse-link.ts`'s own denormalise call (via
  `resolveReusedVoiceFields`, `hydrate-reused-voice.ts:107/147/179`) now
  guards both the starting character and the terminal chain source the same
  way. Adjacent to but distinct from Task 6a's force-to-Qwen fix (that closed
  a different interaction on the same file). Filed as
  [#1885](https://github.com/dudarenok-maker/Castwright/issues/1885), fixed
  on this branch — closes when this PR merges.
  **The separate, destructive-direction gap in the same file is now closed
  too**: `series-reuse-link.ts`'s `clearStaleLink` used to unconditionally
  delete `overrideTtsVoices`/`voiceStyle`/`ttsEngine`/`voiceUuid` on any
  `voiceState: 'reused'` character when its link went stale, with no
  ...
  `voiceState`) was silently erased the next time `pruneStaleReuseLinks` found
  the link stale, and the character reverted to an ordinary designed voice.
  `clearStaleLink` now carries the same `characterHasClonedSlot` guard: it
  still clears the dead link's `matchedFrom`/`matchFactors`, but preserves a
  cloned character's voice fields instead of erasing them.
- **(i) `spawn-sidecar.ts`'s `XTTS_VOICES_DIR`/`QWEN_VOICES_DIR` now route
  ...
  [#1890](https://github.com/dudarenok-maker/Castwright/issues/1890), fixed
  on this branch — closes when this PR merges.
- **(j) A legacy qwen slot with a `libraryUuid` and no `provenance` field
  ...
  close. Filed as [#1891](https://github.com/dudarenok-maker/Castwright/issues/1891),
  fixed on this branch — closes when this PR merges.
...
- **(n) `voice-library-panel.tsx`'s fire-and-forget `assignVoice` dispatch
  ...
  [#1896](https://github.com/dudarenok-maker/Castwright/issues/1896), fixed
  on this branch — closes when this PR merges.
...
- **(q) The assign readiness gate is Qwen-only and over-blocks a
  Coqui-routed assign.** `POST /:voiceUuid/assign` refuses a cloned entry
  whose **Qwen** slot isn't `ready`, even when the character would actually
  route to Coqui and that slot already is. Fail-closed — nothing unsafe
  ships, it just refuses some assigns it could safely allow. Fixing it
  correctly means checking the *routed* engine's readiness rather than
  Qwen's unconditionally, which needs the routed engine resolved before the
  gate runs (currently computed later, once the cast is read) — a per-engine
  readiness rule, not a one-line fix. Filed as
...
### Post-ship: #1967 — cloned-voice derive on Coqui failed on static-FFmpeg boxes

Found by the first fs-38 Wave 3 on-box acceptance run against this wave's own Section E (2026-07-31), on `main` @ `b5479e9c`, blocking all nine of that section's items. `clone_voice`'s derive (`main.py:2500-2508`) writes the decoded reference PCM to a temp WAV and hands XTTS the *path*; XTTS's own `load_audio` (`TTS/tts/models/xtts.py`) then re-decodes that WAV through torchaudio's loader, which on torchaudio >= 2.9 dispatches unconditionally to torchcodec — and torchcodec needs FFmpeg's **shared** libraries. The normal Windows install (`winget install Gyan.FFmpeg`, and the one this repo's own docs steer deployers to) is a static build with no `avcodec-*.dll` at all, so every cloned-voice derive on Coqui failed there, surfaced as `derive-failed` wrapped in copy that told the user to "Re-enable **Qwen**" after a **Coqui** failure.

This is exactly the divergence recorded in the correction note on [`2026-07-04-fs38-voices-library-design.md:196`](../superpowers/specs/2026-07-04-fs38-voices-library-design.md): that design specified loading `master.wav` via `soundfile`, with a guardrail test forbidding `torchaudio.load`; Wave 3c instead shipped the path handoff above, and the named guardrail (`test_audio_io_invariant.py`) could not see the divergence because it only scans the sidecar's own top-level source, not a call made from inside the installed `TTS` package.

**Fixed** in [#1967](https://github.com/dudarenok-maker/Castwright/issues/1967) by patching XTTS's reference loader out for the duration of the derive — a new `server/tts-sidecar/xtts_audio_io.py` decodes the WAV with the stdlib `wave` module + NumPy instead, swapped into `TTS.tts.models.xtts` by a scoped context manager inside `_synth_lock` — rather than by switching to `soundfile` as originally specified. `install-coqui.mjs` also gained a post-install verification step that fails the Coqui install outright if a future `coqui-tts` release moves or reshapes the loader this patches. Full design: [`2026-07-31-xtts-clone-torchcodec-ffmpeg-design.md`](../superpowers/specs/2026-07-31-xtts-clone-torchcodec-ffmpeg-design.md); implementation plan: [`2026-07-31-xtts-clone-torchcodec-ffmpeg.md`](../superpowers/plans/2026-07-31-xtts-clone-torchcodec-ffmpeg.md).

This PR also re-derived and re-triaged this wave's `Re-enable Coqui` copy site: the on-box run's [#1936 grep](https://github.com/dudarenok-maker/Castwright/pull/1936) citation for one at `:144` above resolves, on re-derivation, to the `coqui_weights_present` remote-sidecar bug this wave itself fixed (Task 20) — a pre-existing, unrelated "Re-enable Coqui" symptom that occurred when a remote/LAN sidecar's weights were present but a local disk stat said otherwise. It is historical record of that bug, not a stale copy site, and is left unchanged.
