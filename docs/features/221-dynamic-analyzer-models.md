...
- **Technical:** the analyzer dropdown lists the **installed** Ollama tags (from `/api/ollama/health`'s `models`), labelled from curated `MODEL_OPTIONS` where one matches — so it only offers models you can actually run (an Ollama-down box shows no local options, just the always-present Gemini group). The single canonical install list (`DEFAULT_ALLOWED_MODELS`) is surfaced to the frontend as `pullable` and drives the Model Manager pull list, replacing the duplicated frontend mirror. _(Earlier this plan shipped a curated-∪-live union in the dropdown; reversed to installed-only — see invariant 1.)_
...
- **Invariants preserved:**
  ...
  - Gemini fallback (`selectAnalyzer`) unchanged: analyzer engine = local + Ollama down + key set + Cloud fallback is on → announced Gemini fallback.
...
1. **Dropdown = installed-only; pull list = curated allowlist.** Analyzer pickers render `buildModelOptionGroups(buildLocalModelOptions(account.localAnalyzerModels))`, where `buildLocalModelOptions` returns **only installed live tags** (curated label/hint applied where a curated id matches; an installed-but-uncurated tag shows bare). A curated model that isn't pulled does **not** appear in the dropdown — it is offered for install in the Model Manager pull list (server `pullable`). The picker is never blank because the Gemini group always renders alongside. `buildLocalModelOptions([])` → `[]`. **(Reversed from the original curated-∪-live union — the dropdown now lists only runnable models; the pull list remains the install surface.)**
...
8. **Analysing-view warm/residency target the run model.** The analysing view's auto-warm, Load pill state, and `isAnalyzerReady` gate derive residency from `ollamaHealth.resident` matched against the run's `effectiveModelIds` — NOT `ollamaHealth.modelResident` (which is the *configured default*'s residency) and NOT a no-arg `loadAnalyzer()`. A run on a non-default model (e.g. a per-run override or a per-phase pick) must warm/report on the model it actually executes, never re-warm the configured default behind the user's back.
...
5. **Gemini-fallback caveat.** analyzer engine = local + Ollama down + `GEMINI_API_KEY` set + Cloud fallback is on → analysis completes via Gemini even though the picker showed local options (pre-existing `selectAnalyzer` behavior; see the new Help topic `picked-local-but-ran-on-gemini`).
