...
- **Invariants preserved:**
  ...
  - srv-12 orphan-reset: the worker flips to `awaiting_confirm` (serialized) BEFORE the stream closes, so `resetEntryToQueued` (in_progress-only) no-ops on the parked entry. Since #3029 the client's `close(key)` on park runs synchronously (see below), so the server's `res.on('close')` orphan-recovery branch can genuinely fire mid-park if the chapter-boundary sidecar-recycle await holds the loop open — it stays safe only because `resetEntryToQueued` still requires `status === 'in_progress'`, and the entry is already `awaiting_confirm` by then.
...
- The dispatcher claim filter (`queue-dispatcher-middleware.ts`) claims only `status === 'queued'`; the reconcile skips `/complete` for a parked entry. **#1284 fix (2026-07-04):** this check reads `runner.takeChapterAwaitingConfirm(bookId, chapterId)` (recorded off the `chapter_awaiting_fallback_confirm` tick), NOT the queue slice's `status` field — nothing dispatches a queue-snapshot update off that tick, so the Redux copy isn't guaranteed to have caught up to `awaiting_confirm` by the time the reconcile runs. Reading the stale `status` field there raced the reconcile into `/complete`-ing the parked entry (a server no-op, but the local `completed` Set poisoned it permanently — it never actually leaves the live queue, so nothing ever pruned it back out, and a later user confirm could never get it re-claimed). **#3029 fix:** the park now closes the stream handle synchronously (`close(key)` on the `chapter_awaiting_fallback_confirm` tick) instead of waiting for the subsequent `idle` tick — the reconcile runs off that close's `clearActiveStream` dispatch, not off an `idle` tick, so a `fallbackConfirmed` re-dispatch is re-claimed immediately instead of sitting `queued` until an unrelated action woke the dispatcher.
...
- Vitest unit (`src/store/queue-dispatcher-middleware.test.ts`) — reconcile does NOT `/complete` a parked entry (stays re-claimable) using the REAL SSE tick ordering (a park tick closes the stream synchronously; no interleaved queue-snapshot dispatch); a parked-then-confirmed entry IS re-claimed (a second stream opens) even though the queue-slice status lags the park (#1284 regression); `fallbackConfirmed` is threaded into the stream open for a confirmed entry; a parked-then-confirmed entry is re-claimed even when no subsequent `idle` tick ever arrives (#3029 regression).
