...
- The dispatcher claim filter (`queue-dispatcher-middleware.ts`) claims only `status === 'queued'`; the reconcile skips `/complete` for a parked entry. **#1284 fix (2026-07-04):** this check reads `runner.takeChapterAwaitingConfirm(bookId, chapterId)` (recorded off the `chapter_awaiting_fallback_confirm` tick), NOT the queue slice's `status` field — nothing dispatches a queue-snapshot update off that tick, so the Redux copy isn't guaranteed to have caught up to `awaiting_confirm` by the time the reconcile runs off the following `idle` tick. Reading the stale `status` field there raced the reconcile into `/complete`-ing the parked entry (a server no-op, but the local `completed` Set poisoned it permanently — it never actually leaves the live queue, so nothing ever pruned it back out, and a later user confirm could never get it re-claimed).
...
- Vitest unit (`src/store/queue-dispatcher-middleware.test.ts`) — reconcile does NOT `/complete` a parked entry (stays re-claimable) using the REAL SSE tick ordering (park tick then idle, no interleaved queue-snapshot dispatch); a parked-then-confirmed entry IS re-claimed (a second stream opens) even though the queue-slice status lags the park (#1284 regression); `fallbackConfirmed` is threaded into the stream open for a confirmed entry.
