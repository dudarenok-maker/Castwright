...
- **Subset-retry route sticky** — *shipped in D1.* `POST /:id/analysis/chapters` now registers an `AnalysisJob` with `kind: 'subset'` + `subsetChapterIds` in a second in-flight map (`inFlightSubsetByManuscript`, keyed by `manuscriptId`). The analyzer work runs in a detached `runSubsetAnalyzerJob` so a client disconnect doesn't abort it; a concurrent re-POST attaches as a second subscriber via the shared `replayCatchUp` helper, same shape as the main route's subscribe-vs-start dispatch. `/pause` aborts both main and subset for the manuscript. `snapshotInFlightAnalysis` returns the subset job's state when one is live (the user just kicked the retry; the pill should reflect what they're watching), falling back to main when only main is live. The cold-boot file gains `kind` + `subsetChapterIds` so the AnalysisPill renders "Retrying N chapters" copy (D2). Subset terminal success deliberately does NOT `deleteAnalysisState` so any sibling main snapshot survives. Pinned by `server/src/routes/analysis.test.ts` ("sticky subset retry — second in-flight slot").
