...
3. **`server/.env` write is idempotent** (`buildEnvContents` returns `null` when `server/.env` exists), so Update / re-install preserve a user's `WORKSPACE_DIR`. **A fresh `.env` defaults `WORKSPACE_DIR` to `~/Castwright`** (discoverable, survives an app uninstall) rather than the buried `<appRoot>/workspace` — via the pure `chooseFreshWorkspaceDir({appDir, homedir, workspaceExists})`, guarded two ways: a **directory-existence** check keeps an already-present `<appRoot>/workspace` (so a user whose `.env` was deleted/clobbered isn't orphaned onto `~/Castwright`), and an unusable home dir (the CLI's `mkdirSync` writability probe throwing) falls back to `<appRoot>/workspace` so a fresh install can never be blocked from booting. Existing installs are untouched (fresh-install-only). See [docs/superpowers/specs/2026-07-15-first-run-library-location-design.md](../superpowers/specs/2026-07-15-first-run-library-location-design.md).
...
- `pinokio-scripts/lib/resolve-release.test.js` (6), `write-env.test.js` (7 — `defaultLibraryDir`/`chooseFreshWorkspaceDir` guards + `buildEnvContents`), `menu.test.js` (5 — both running states + the Reset-confirm guard) — `npm run test:pinokio` (node:test), in `test:all` + the `verify-cache` `test:pinokio` step (scope `pinokio-scripts/**`). `structure.test.js`'s STATES covers the `running/url:null` startup window so the "every state yields a default" invariant guards it.
