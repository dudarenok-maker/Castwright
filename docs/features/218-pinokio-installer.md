...
2a. **Every `pinokio/*.js` script sets `path: '..'` on every `shell.run`/`fs.rm` step.** Pinokio resolves a `shell.run`'s cwd — and any `fs.rm` target — relative to the **currently running script's own directory**, not the app root. Since these scripts live in `pinokio/` (one level below `package.json`/`server/`), every step needs the explicit `path: '..'` override, or it silently operates one directory too deep (confirmed on-box 2026-07-11: `conda: { path: 'env' }` resolved to `pinokio/env` instead of `<app>/env`, and would have made `resolve-release.js`'s own `existsSync('pinokio/start.js')` guard always fail once reached). `script.start`'s `uri` is unaffected — sibling-script resolution is correctly relative to the caller's own directory.
