...
- **Scope note:** an in-flight chapter at the moment of sidecar death still fails its synth (the existing `withTtsRetry` window is shorter than a Qwen cold reload) and is recovered by the queue re-dispatch + srv-16's audio-exists skip — i.e. the *run* self-heals, not necessarily the exact in-flight chapter without a blip.
