...
7. **The profile drawer's `engineChoice` seed is always an engine the picker renders an option for** (`src/modals/profile-drawer.tsx`) — clamped against `pickerEngines` (the offered `kokoro`/`qwen`/`coqui` set ∩ `eligibleTtsEngines`), not against raw `eligibleTtsEngines`. A reused character carries its previous book's `ttsEngine` on disk, and since fs-60 a non-English book no longer hard-locks to Qwen, so an unclamped seed puts the controlled `<select>` on a value with no matching option: it *displays* "Default (…)" while Save writes the stale engine the user never chose. Clamping on eligibility alone is not enough — `gemini` is language-eligible yet has no option row (#1534).
...
- Vitest frontend (`src/modals/profile-drawer.test.tsx`) — `lockedToQwen` now derives from `eligibleTtsEngines.length === 1 && eligibleTtsEngines[0] === 'qwen'`; plus invariant 7's seed clamp, asserted through **Save** (`onSave` must not carry a stale `kokoro`/`gemini` on a ru book) rather than through `select.value`. The DOM cannot see this bug: with no matching option jsdom applies the HTML "selectedness" reset and auto-selects the first option, so `select.value` reads `'default'` either way — only the state/DOM desync behind it is observable.
