...
- Any button that can fire while a knob is being edited (`OverrideRow`'s own Revert, `SettingsSection`'s "Reset section", `AdvancedView`'s "Reset all") carries `onMouseDown={(e) => e.preventDefault()}`. Without it, the button's mousedown blurs whatever input currently has focus first — committing an uncommitted edit at the exact moment the button is clicked, racing a fresh `saveOverride` against the `resetKnob`/`resetGroup`/`resetAllConfig` it triggers (no ordering guard in `config-slice.ts`'s `fulfilled` reducers — whichever response lands last wins).
