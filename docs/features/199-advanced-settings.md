...
- Any button that can steal focus while a knob is mid-edit (`OverrideRow`'s own Revert, `SettingsSection`'s "Reset section", `AdvancedView`'s "Reset all") makes its blurring input **abandon** rather than commit its edit, checked via `shouldAbandonOnBlur` two ways at once — both are load-bearing, don't drop either:
  - **Mouse/pointer**: the button calls the exported `beginConfigAction()` (`override-row.tsx`) from `onMouseDown` — always as `() => beginConfigAction(...)`, NEVER `onMouseDown={beginConfigAction}` directly, since React passes the DOM event as the first argument and `beginConfigAction` would then treat that event object as its `scopedInput` param and silently no-op. It sets a module-level flag and imperatively calls `.blur()` on the target (see row-scoping below), forcing the currently-focused knob input to blur *before* the button's own `onClick` runs. This can't rely on the browser's own click-focuses-the-button behavior: WebKit (desktop Safari, and every iOS browser, which is WebKit under the hood) doesn't move focus to a `<button>` on click/tap the way Chromium/Firefox do, so relying on that would silently fail there. **This repo's e2e suite is Chromium-only** (no WebKit project in `playwright.config.ts`), so a WebKit-only regression here is invisible to CI — verify manually on Safari/iOS per the mobile testing protocol if touching this mechanism again.
  ...
  - **Row-scoping**: `OverrideRow`'s own Revert button passes its own `inputRef.current` to `beginConfigAction(scopedInput)`, so clicking it only ever abandons THIS row's own input — clicking Row B's Revert must not silently discard an unrelated Row A's in-progress edit just because Row A happens to be what's currently focused. `beginConfigAction` treats `undefined` (no argument — Reset section/Reset all, which don't own a single row to scope to) as "fall back to whatever's globally focused," and an explicit `null` (a boolean/enum/device row's `inputRef`, which is never attached) as "nothing to abandon" — the `scopedInput === undefined` check distinguishes the two; don't conflate them.
- `KnobControl`'s commit handlers funnel through a shared `commitEdit` that attaches a `.catch()` to `onChange`'s return value (when it returns one — `advanced.tsx` wires it to `dispatch(saveOverride(...)).unwrap()`) and resets the draft back to the last-known `value.effective` if the save is rejected — guarded **two ways**, both needed:
  - `editingRef.current`: don't clobber a re-edit that's still mid-typing.
  - `generationRef`, a single counter bumped BOTH inside `commitEdit` (a new local commit) AND inside the resync effect whenever `value.effective` itself changes (an external Revert/Reset on this same field, another tab's edit, anything). A rejection only reverts the draft if the generation is still exactly what it was when that commit started. Two earlier, narrower guards were tried and superseded — the final design needs both cases a single counter now covers:
    - A counter bumped only by local commits doesn't cover a same-field Revert/Reset landing while the original save is still in flight — no new local edit happens in that case, so that counter alone stays unchanged and a later rejection would still stomp the already-correct, already-reset value with the stale pre-edit one.
    - `editingRef` alone doesn't cover a re-edit that's ALREADY been committed (blurred, its own save dispatched or even already succeeded) by the time an earlier commit's rejection arrives late — it's back to `false` again once that second edit is also blurred.
...
- **`SettingsSection`'s "Reset section" and `AdvancedView`'s "Reset all" abandon whatever's globally focused, not specifically a field within their own scope** — unlike `OverrideRow`'s own Revert (which now scopes to its own row via `inputRef`, see "Row-scoping" above), these two page/section-level buttons still fall back to `document.activeElement` with no per-row ref to check against, since they can legitimately affect many rows at once and don't own a single row to scope to. Clicking "Reset all" while an UNRELATED field elsewhere on the page is mid-edit (unblurred) abandons that field's edit, even if it isn't in the group/page being reset. Scoping this properly would mean either of these buttons tracking every child row's input ref (a bigger refactor — `SettingsSection` doesn't currently track its children's refs at all) — deferred as a narrower edge case (requires editing one field, then acting on a Reset button without ever blurring the first) relative to that refactor's cost.
