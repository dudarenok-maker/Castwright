...
- Any button that can steal focus while a knob is mid-edit (`OverrideRow`'s own Revert, `SettingsSection`'s "Reset section", `AdvancedView`'s "Reset all") calls the exported `beginConfigAction()` (`override-row.tsx`) from `onMouseDown`. It sets a module-level flag and imperatively calls `document.activeElement.blur()`, forcing whichever knob input is currently focused to blur (and, via `consumeAbandonNextBlur()`, **abandon** rather than commit its edit) *before* the button's own `onClick` runs. Two earlier approaches were tried and rejected — don't reintroduce either:
  - `onMouseDown={(e) => e.preventDefault()}` to block the blur outright: this also blocks the `editing` transition the draft-resync effect needs, so the abandoned edit silently re-committed (recreating the override the click had just reverted) on the row's next real blur.
  - Checking the blur event's `relatedTarget` against a `data-config-action` marker (relying on the browser's own click-focuses-the-button behavior to fire that blur): unreliable, because WebKit (desktop Safari, and every iOS browser, which is WebKit under the hood) doesn't move focus to a `<button>` on click/tap the way Chromium/Firefox do — so no blur (and no `relatedTarget`) ever fired there, reproducing the same silent-recommit bug on WebKit specifically. **This repo's e2e suite is Chromium-only** (no WebKit project in `playwright.config.ts`), so a WebKit-only regression like this is invisible to CI — verify manually on Safari/iOS per the mobile testing protocol if touching this mechanism again. The imperative `.blur()` call sidesteps the issue entirely since it doesn't depend on focus ever reaching the button.
- `KnobControl`'s commit handlers funnel through a shared `commitEdit` that attaches a `.catch()` to `onChange`'s return value (when it returns one — `advanced.tsx` wires it to `dispatch(saveOverride(...)).unwrap()`) and resets the draft back to the last-known `value.effective` if the save is rejected — **guarded by `editingRef.current`**, same as the resync effect: if the user has since refocused the same field and started a fresh edit before the earlier save's rejection arrives, the stale rejection must not clobber the in-progress edit. Without the `.catch()` at all, a rejected save left the draft frozen on the unsaved value forever (config-slice's `saveOverride.rejected` case leaves `s.values` untouched, so the draft-resync effect has nothing to resync to), and `AdvancedView` only surfaces the rose error banner pre-hydration (`!hydrated && status === 'error'`) — so a rejection after the view is already hydrated (the normal case for any in-place edit) produced zero visible feedback otherwise.
