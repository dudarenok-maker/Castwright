...
- [49 — Release packages on git-tag push](49-release-package.md) — `scripts/bump-version.mjs` + `.github/workflows/release.yml` produce a downloadable `audiobook-generator-vX.Y.Z.zip` on GitHub Releases whenever a `v*.*.*` tag is pushed. Cross-platform (Windows + macOS + Linux). Adds the production launch path (`npm run start:prod` → `express.static(dist)` on :8080) and an in-app Gemini API key field. **Status: active.**
