...
- `OLLAMA_URL` env / user-settings `ollamaUrl` defaults to `http://localhost:11434`. `OLLAMA_MODEL` env / user-settings `ollamaModel` defaults to `qwen3.5:9b`. Resolved at request time (no restart needed after a settings save).
- `OllamaAnalyzer.runStage*` calls `POST /api/chat` with `format: 'json'`, `stream: true`, `keep_alive: '5m'`, `think: false`, `options.num_ctx: 8192`. These constants are load-bearing for the 8 GB VRAM budget — change with care.
...
1. **Happy path (local)** — `cd server && npm run dev`. Upload canonical manuscript `C:\Users\dudar\Downloads\the Coalfall Commission.txt` → run analysis. SSE label reads `Engine: Ollama (qwen3.5:9b)`, chunks arrive every 1–3 s, schema-valid output. `nvidia-smi` shows ~6.6 GB resident in `ollama_llama_server.exe` for ~5 min after completion, then released by the keep-alive timer.
...
- JSON-schema-mode structured output: v1 uses `format:'json'` (string-only constraint). Upgrade path (if first-attempt validation rates sag): add `zod-to-json-schema`, pass per-stage schemas through `format:<JSON Schema>` to Ollama 0.5+.
