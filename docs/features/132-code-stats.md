...
- **New seams:**
  ...
  - A `<!-- CODE-STATS:START -->` / `<!-- CODE-STATS:END -->` marker block in `docs/project-narrative.md` Appendix B that the script rewrites idempotently.
...
4. The `refreshCodeStats()` step in `bump-version.mjs` is **non-fatal**: it skips cleanly when `scripts/code-stats.mjs` is absent (fixture repo) or when tokei errors (not installed), and `docs/project-narrative.md` is only staged when it exists.
...
3. `npm run stats -- --write` → updates only the CODE-STATS block in `docs/project-narrative.md`; `git diff` shows that block alone; re-running prints `no change — stats already current.`
