...
> Status: stable
> Key files: `src/views/generation.tsx`, `src/store/chapters-slice.ts` (`applyGenerationTick`, `regenerateChapter`/`regenerateCharacter`/`batchRegenerateCharacters`), `src/lib/api.ts` (`realStreamGeneration`), `server/src/routes/generation.ts`, `server/src/routes/chapter-audio.ts`, `server/src/tts/synthesise-chapter.ts`
> URL surface: `#/books/:bookId/generate`
> OpenAPI ops: `POST /api/books/:bookId/generation` (SSE stream), `GET /api/books/:bookId/chapters/:chapterId/audio` (segments JSON), `GET /api/books/:bookId/chapters/:chapterId/audio.wav` (range-supporting WAV)
> Paired tests: `src/store/chapters-slice.test.ts`

## What this covers

Streams chapter audio generation via Server-Sent-Events. The client opens a long-running POST with `{ modelKey, chapterIds?, force? }`; the server iterates chapters, synthesises each character's same-speaker groups via the chosen TTS provider, emits ticks for progress / assembling / completion / failure / idle, and on `chapter_complete` writes a WAV + segments JSON. The client maps ticks into per-chapter/per-character state transitions in the chapters slice, plays the resulting WAV via the MiniPlayer, and forwards a `pendingRegen` spec on the next stream open when the user regenerates.
...
- `GenerationTick` union: `'progress' | 'chapter_assembling' | 'chapter_complete' | 'chapter_failed' | 'idle'`. Payload fields per type:
  ...
  - `chapter_assembling` → `chapterId` (required); `totalGroups?`, `durationSec?` carry the run summary about to land on disk. Fired between the last group and the WAV write.
...
4. **Chapter complete** — chapter card shows "complete" badge; WAV exists on disk under `books/<Author>/<Series>/<Title>/.audiobook/audio/<chapter-slug>.wav`. The next queued chapter starts automatically the moment the server emits its own `progress` tick for it; nothing on the client gates the promotion.
5. **Preview playback** — click Preview on a done row → `setCurrentTrack(chapterId)` dispatched; MiniPlayer mounts a real `<audio>` element backed by `GET /api/books/:bookId/chapters/:chapterId/audio.wav` (range-supported, so seek works) and renders the segment colours from `GET .../audio` (JSON ChapterAudio).
