...
- **Invariants preserved:**
  - Narrator detection stays keyed on id, never display name — changing the display name breaks none of it. `analyzer/narrator-identity.ts` (`NARRATOR_CHARACTER_IDS`, renamed from `NARRATOR_IDS` by #1895/PR #2009) is now the single server-side source for the `'narrator'`/`'char-narrator'` id pair, consumed by `routes/voices.ts`, `routes/voice-style.ts`, `tts/voice-mapping.ts`, `store/merge-analysis-cast.ts`, and `tts/synthesise-chapter.ts`; the frontend twin `src/lib/narrator-ids.ts` (`NARRATOR_CHARACTER_IDS`) is consumed by `lib/principal-cast.ts`, `lib/tts-voice-mapping.ts`, and `modals/rebaseline-modal.tsx`. `analyzer/roster-coverage.ts`, `analyzer/dialogue-structure/name-matcher.ts`, and `modals/profile-drawer.tsx` key on a standalone `'narrator'` id literal (not centralised, not `'char-narrator'`-aware) — unaffected by this feature or by #1895.
...
1. Narrator ids stay exactly `'narrator'` / `'char-narrator'` — `narrator-identity.ts:NARRATOR_CHARACTER_IDS` (`server/src/analyzer/narrator-identity.ts`, renamed from `NARRATOR_IDS` by #1895/PR #2009) keys on both; every other narrator check in the codebase keys on id first, never on display name.
