...
3. `encodePcmToAudio({ format: 'mp3' })` emits a buffer byte-identical to the pre-plan-72 default (same libmp3lame VBR V2 args). The MP3 regression test in `server/src/tts/mp3.test.ts` locks this — any change to `buildMp3FfmpegArgs` MUST keep the test green.
...
Canonical end-to-end manuscript for full-pipeline regression: `C:\Users\dudar\Downloads\the Coalfall Commission.txt` (do not commit — copyrighted).
