...
- **Invariants preserved:**
  ...
  - No change to merge-commit shape: PRs still produce `Merge pull request #N from <head-ref>` commits on `main`. Disabling squash/rebase merge buttons in the GitHub UI prevents accidental policy drift via the wrong button click — the _option that's left_ matches the existing pattern.
...
4. **`deleteBranchOnMerge` MUST stay enabled at the repo level.** Once a branch is merged, `git branch --list 'feat/server-*'` should list _open work only_ — leaving merged branches around defeats the at-a-glance scope discovery from plan 38's branching model.
