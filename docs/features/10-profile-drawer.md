...
- **Model-voice override picker** (`ModelVoiceOverridePicker` in `src/modals/profile-drawer.tsx`) — sits beneath the Preview voice block and lets the user pin a specific base voice for this character, bypassing the attribute-driven match.
  - **Engine-tabbed**: one tab per engine present in `state.voices.baseVoices` (Coqui, Kokoro, optionally Gemini). The active tab determines which engine's catalog the `<select>` exposes; switching tabs preserves the OTHER engines' selections. Tabs render a small `.bg-magenta` dot when that engine has a non-null slot, and the project's currently-active engine carries a small "Active" label so the user knows which slot actually drives synthesis right now.
  - The `<select>` round-trips through string values: `AUTO` or `${engine}|${name}`. The selected value for the active tab is read from `voice.overrideTtsVoices?.[engineTab]`; legacy `voice.overrideTtsVoice` is folded in by `mapOverridesToBaseVoiceMap` so older Voice payloads still render correctly during the migration window. When the active tab matches the auto-resolved engine the AUTO option reads "Auto — currently &lt;Engine&gt; · &lt;Name&gt;"; for other tabs it reads "Auto for &lt;Engine&gt; — attribute-driven".
