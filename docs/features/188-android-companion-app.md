...
| `app-10` | #589 (merge `c51f71e`, scaffolding) + #553 (merge loopback proxy implementation) | stream-over-LAN instant play (companion). **Was blocked 2026-06** by the app-pinned-CA-vs-platform-player TLS gap — `just_audio` streams via the native player (OS trust store), which rejects the mkcert CA. **Now delivered 2026-07** via an in-app loopback proxy that re-serves CA-pinned HTTPS bytes as plaintext to the player over `127.0.0.1` (no OS cert install, token never leaves the app). Component set: `LoopbackProxy` (HTTP → proxy task), `pinnedRangeStream` (CA-pinned range fetcher), `network_security_config.xml` (permit localhost plaintext), `AudioEngine.errorStream` seam (stream-failure fallback), paired Dart tests from Tasks 1-8. Spec [`2026-07-12-app-10-lan-loopback-proxy-design.md`](../superpowers/specs/2026-07-12-app-10-lan-loopback-proxy-design.md), plan [`2026-07-12-app-10-lan-loopback-proxy.md`](../superpowers/plans/2026-07-12-app-10-lan-loopback-proxy.md). |
...
- **Benefit (user):** zero-wait preview before committing a download. **Depends on:**
  `app-2`, `app-5`.
- **Status (2026-07-12): DELIVERED via loopback proxy (#553).** Scaffolding shipped inert in PR #589 — the pure pieces existed and were unit-tested (`resolvePlaybackSource`, `AppSettings.streamOverLan` default-off toggle, `AudioEngine.setStreamUrl`, the settings switch) but nothing consumed `lanStream` because of the app-pinned-CA-vs-platform-player TLS gap: the native player (`just_audio` → ExoPlayer/AVPlayer) streams over the OS trust store, which rejects the mkcert CA, so `https://<lan>:8443` audio fails TLS. **Unblock designed + delivered** via an in-app loopback proxy (Tasks 1-8) that re-serves CA-pinned HTTPS bytes as plaintext to the player over `127.0.0.1` — no OS cert install, token never leaves the app. Component set: `LoopbackProxy` (HTTP server), `pinnedRangeStream` (CA-pinned range fetcher), `network_security_config.xml` (permit localhost plaintext), `AudioEngine.errorStream` seam (stream failure callback), paired Dart tests. Spec [`2026-07-12-app-10-lan-loopback-proxy-design.md`](../superpowers/specs/2026-07-12-app-10-lan-loopback-proxy-design.md), plan [`2026-07-12-app-10-lan-loopback-proxy.md`](../superpowers/plans/2026-07-12-app-10-lan-loopback-proxy.md).
- **Follow-ups (#1579, shipped):** (1) each stream load carries a generation id (`_streamGen`/`_streamingGen`) so a late teardown error from an OUTGOING stream arriving mid-switch is recognised as superseded and ignored, not misrouted as the incoming chapter's failure; (2) `ApiClient.dispose()` force-closes the three owned pinned `HttpClient`s (send/range/stream), called from `CompanionRuntime.dispose()` so a re-pair doesn't orphan the old connection pools. Regression: superseded-error case in `player_controller_test.dart` + new `api_client_dispose_test.dart`.
- **On-device acceptance (owed post-merge):** real Android phone on home Wi-Fi, undownloaded
  chapter, "Stream over LAN" on → tap play → instant start; seek mid-chapter; lock-screen
  transport controls; background survival via the media foreground service; confirm NO OS
  cert-install prompt. Then toggle off / leave Wi-Fi → tap the same chapter → "download to
  play" message (no stall).
