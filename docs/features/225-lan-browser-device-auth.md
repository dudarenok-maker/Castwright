...
- **Migration:** `device-tokens.json` schema 1→2 adds `expiresAt`; there is **no** grandfather migration — a legacy `schema: 1` record (no `expiresAt`) is rejected. Since #2149 this rejection happens at **load time** (`invalidDeviceField`, mirroring `findValidDevice`'s own `expiresAt` rule one step earlier), not only at auth time — so the record is dropped from `cache` entirely and also disappears from `GET /api/devices`, not merely from authentication. That device re-pairs once. Forward-only; reversible by not setting `LAN_AUTH_TOKEN` (guard no-ops).
...
1. **`extractToken` reads the cookie first** (`server/src/lan-auth.ts`, `extractToken` → `readCwLanCookie`): the `__Host-cw_lan` cookie is checked before Bearer / `X-Lan-Token` / `?token=`. `readCwLanCookie` wraps `parseCookie` in try/catch (runs on every `/api` request — an unguarded throw would 500 the API).
2. **Expiry + revocation are enforced at auth time in `findValidDevice`, and (since #2149) every trusted field is ALSO validated at load time** (`server/src/workspace/device-tokens.ts`): `findValidDevice` rejects `revoked`, `expiresAt === undefined`, an `expiresAt` whose `Date.parse` is not finite, `now > exp`, or a non-string `tokenHash`. **The `Number.isFinite` arm is load-bearing, not defensive** (#2144): `Date.parse` returns `NaN` for `null`/`""`/garbage and every comparison against `NaN` is `false`, so a bare `now > Date.parse(...)` let those records authenticate forever. Do not "simplify" it back. `isValidDeviceToken` stays single-arg and calls it with the default clock. The client cookie `Max-Age` is not authoritative. **`loadSync`'s `invalidDeviceField` (#2149) enforces the same rules one step earlier**, at the load boundary: a record with a non-string/empty `tokenHash`, an absent-or-unparseable `expiresAt`, a non-boolean `revoked`, an unparseable `createdAt`, or a non-object shape is dropped (with a `console.warn` naming the record + field) before it ever reaches `cache`. `findValidDevice`'s own checks remain — defence-in-depth, not a replacement, for any future path that populates `cache` without going through `loadSync`.
...
6. **All token-minting paths are loopback-gated** (`devices.ts`: `POST /api/devices` admin mint + `POST /api/devices/pair-session`) — a stolen browser cookie cannot mint a fresh durable token that survives revocation. **Revocation is loopback-gated too** (`DELETE /api/devices/:id`, #2269) — the same stolen cookie must not be able to lock the legitimate owner out by revoking their own device. This is strictly *narrower* than minting a pairing session: `mayStartPairingSession` also admits an already-paired device arriving via the friendly hostname (`castwright.local`, peer `127.0.0.2` through the `:443` forwarder), but revoke does not — so revoke works only from the direct `https://localhost:<port>` address, never `castwright.local` or the bare `:443` forwarder, even for the host's own browser. `isLoopbackHost()` (client-side, hostname-only) cannot see the forwarder, so the Admin card's Revoke button can render and then 403 when reached that way; `revoke()` in `lan-access-card.tsx` translates that 403 into copy naming the direct-port URL.
...
- `server/src/csrf-origin.test.ts` — allowed/loopback origin pass, foreign origin 403, no-Origin+no-Referer 403, header-token bypass, parser-agreement (a cookie `parseCookie` accepts still gates).
- `server/src/routes/devices.test.ts` — `pair-session` loopback-only URL payload + 409; admin mint 403 from non-loopback; revoke (`DELETE /api/devices/:id`) 403 from non-loopback with the record still live afterwards, a loopback-caller sanity pass, and the real friendly-hostname/`:443`-forwarder case (peer `127.0.0.2`, `Host: castwright.local`, driven directly against `devicesRouter` since supertest can't present that peer IP) asserting both the 403 status and body (#2269); label cap.
