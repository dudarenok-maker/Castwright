...
**Re-pinned to `80be2f1d` on 2026-09-16.**
- **The rebase:** the design branch was rebased onto `origin/main` = `80be2f1d` (PR #3192's merge); HEAD `b54eefb5`.
- **Citations:** every code citation in the spec and master plan was re-derived there by content (`git show 80be2f1d:<path>`). A citation into a file unchanged between `46e62a34` and `80be2f1d` keeps its line.
- **Moved symbols:** `getResolvedOllamaUrl` / `getResolvedOllamaModel` now live in `server/src/config/ollama-resolved.ts` (`:21`, `:35-39`).
- **Allowlist:** `server/madge-cycles-allowlist.json` is unchanged between the two commits.
- **History:** the text below is the history of the earlier pin, kept as written. The anchor table gains an `80be2f1d` column.

**Audited against `f3bad701` on 2026-09-17 — the pin stays `80be2f1d`.** Main advanced by 23 commits (PRs #3236, #3240, #3243, #3244) while the plan sat awaiting approval. The delta was audited rather than re-pinned, because no citation broke.
- **Files main changed that the plan actually cites:** `src/views/analysing.tsx` (+ its test), `src/components/analysing/phase-card.tsx`, `phase-model-chip.tsx` (+ its test), `src/components/layout.tsx` (+ test), `src/components/top-bar.tsx` (+ test), and `CLAUDE.md`. Every other changed file is uncited — including `src/lib/analysis-phase-state.ts`, `sticky-analysis-bar.tsx`, `library-grid.tsx` and `book-library.test.tsx`.
- **No citation broke.** #3244's edits are additive, and every cited line checked still resolves to its quoted symbol at its cited line (`top-bar.tsx:233`, `phase-card.tsx:354`, `phase-model-chip.tsx:91-96`, `analysing.tsx:358`/`372`/`1435`). Ordinary line drift below an insertion point stays covered by the per-citation "re-grep first / re-derive at ship time" instruction.
- **`CLAUDE.md` is untouched in substance:** the only changed sentence renames the workflow wiring `npm run check:onbox-register` (`onbox-register-check.yml` → a step in `verify.yml`'s `lint-and-checks` job). No plan citation quotes it.
- **One design-level edit WAS owed, and is applied.** PR #3244 added `isNotAFailureHaltCode` (`src/lib/analysis-phase-state.ts:11`) and a neutral `needs-action` third render state (`PhaseRenderState`, `:4`) for `cast_incomplete` and `stage1_shrink_refused`. Wave 2's `haltFixes`-from-snapshot fallback was gated on "`error` is null but the stream is halted", which is true of those two codes too, so it now carries an explicit `!isNotAFailureHaltCode(...)` exclusion (w2, the `src/views/analysing.tsx` bullet).
- **The store contract is unaffected:** #3244 derives `needs-action` per component from `haltCode` and never persists it, so `analysis-slice.ts`'s `setHalted` still carries `{ manuscriptId, code, message }` and the plan's added `fixes` / `haltFixes` collide with nothing.
...
| What | At `46e62a34` (history) | At `80be2f1d` (re-derived by content, 2026-09-16) |
|---|---|---|
| `readUserSettings` / whole-file schema fallback / `getCachedUserSettings` / `writeUserSettings` | `user-settings.ts:436` / `:522-525` / `:537` / `:592` | `user-settings.ts:492` / `:579-582` / `:594` / `:659` |
| Malformed-file path (backup recovery, else defaults + `corrupt: true`) | `user-settings.ts:479-498` | `user-settings.ts:535-554` |
| Boot warm, fire-and-forget, wrapped in its own try/catch | `index.ts:188` (`bootWarmUserSettings`, `:135-144`) | unchanged |
| `endJob` | `analysis.ts:3063` | `analysis.ts:3058` |
| Job terminal classify + `endJob` call | `analysis.ts:6444-6450` (subset `:8042`) | `analysis.ts:6448-6454` (subset `:8092`) |
| Phase-0 per-chapter content-block rethrow | `analysis.ts:4550` (subset `:7149`) | `analysis.ts:4554` (subset `:7199`) |
| Subset Phase-0 per-chapter catch | `analysis.ts:7144-7155` | `analysis.ts:7194-7205` |
| Phase-1 pool `launchNext` + its catch | `analysis.ts:5680-5691` (catch `:5685-5689`) | `analysis.ts:5684-5695` (catch `:5689-5693`) |
| Silence warning / heartbeat threshold | `analysis.ts:4417-4423` / `:1184` | `analysis.ts:4421-4427` / `:1179` |
| Stage-1 chunk-budget call | `analysis.ts:4496` (subset `:7065`) | `analysis.ts:4500` (subset `:7115`) |
| `structureBudget` declaration and share | `analysis.ts:3692`, `:5462` (subset `:6805`, `:7361`) | `analysis.ts:3695`, `:5466` (subset `:6855`, `:7411`) |
| Non-story classifiers | `analysis.ts:5814-5836` (subset `:7540-7566`), swallowing catch `:5831-5833` | `analysis.ts:5818-5840` (subset `:7590-7616`), swallowing catch `:5835-5837` |
| `unloadResidentOllama` / `usesLocalAnalyzer` | `analysis.ts:3235` / `:3716` | `analysis.ts:3230` / `:3720` |
| `persistTerminalSnapshot` | `analysis.ts:2875` | `analysis.ts:2870` |
| Script review: content-block capture / catch / terminal event / budget call / warm / keep-alive pin | `script-review.ts:924` / `:944-951` / `:972-987` / `:840` / `:748` / `:798` | `script-review.ts:924` / `:944-951` / `:972-986` / `:840` / `:748` / `:798` |
| Gemini escalation analyzer (stays Gemini) | `analysis.ts:2182` | `analysis.ts:2177` |
| `FORBIDDEN_KEYS` set / its last entry `'tourCompletedAt'` / `stripForbiddenKeys` | `user-settings.ts:643-661` / `:660` (`]);` at `:661`) / `:663-671` | `user-settings.ts:710-728` / `:727` (`]);` at `:728`) / `:730-738` |
| Mock settings whitelist: `mockPutUserSettings`, its destructure, its mirrored object | `src/lib/api.ts:7320` / `:7324-7337` / `:7341-7354` | `src/lib/api.ts:7382` / `:7396-7406` / `:7410-7420` (the function now first rejects #3192's retired fields, `:7384-7393`) |
| Migration hook (#3192) and settings PUT order | — | `user-settings.ts:570-571` (`migrateLegacyEagerLoadFields`, then `migrateLegacyAnalyzerModelFields`); `routes/user-settings.ts:42-47` `RETIRED_ANALYZER_FIELDS`, rejected at `:122-130` before `writeUserSettings` `:131` |
| Ollama resolvers (moved file in #3192) | `user-settings.ts:960-965` (`getResolvedOllamaModel`) | `config/ollama-resolved.ts:21` (`getResolvedOllamaUrl`), `:35-39` (`getResolvedOllamaModel`) |
