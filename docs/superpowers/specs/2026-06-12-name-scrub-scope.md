...
| 2a | **Code comments** (the Hollow Tide as examples) | `roster-coverage.ts`, `voice-match.ts`, `text-match.ts`, `series-cast-scan.ts`, `drift-report.tsx` | Low — no logic depends, pure reword |
| 2b | **Analyzer PROMPT examples** | `server/src/routes/analysis.ts` (~L1140–1147: the Hollow Tide names inside the LLM prompt template literals) | **Med** — these are prompt *content* the analyzer sends, likely pinned by `gemini.test.ts`/prompt tests. NOT a cosmetic reword. |
| 3 | **Frontend mock data** | `src/mocks/` (incl. `canned-data.ts`), `src/data/`, the mock branch of `src/lib/api.ts` (e.g. `mock-book-The Drowning Bell` book entries — *data*, not comments) | Med — drives mock-mode UI, marketing capture, frontend tests asserting names |
...
| 6 | **Multilingual fixtures** | `e2e/language-detection.spec.ts`, `162-fs2-multilanguage.md` + non-English (Russian) the Hollow Tide content | **Blocked** — Coalfall is English-only; **no owned non-English text exists** (see Open items) |
...
2. **Analyzer prompt examples (Cat 2b)** — reword the Hollow Tide→Coalfall *inside* the
...
5. **Docs — pointer + prose together (Cat 1 + Cat 5)** — one pass per doc file
   (the manuscript pointer and the Hollow Tide prose co-occur in 19 files; CLAUDE.md
...
- **DECISION — multilingual source (Cat 6).** Coalfall is English-only and no
  ...
  synthetic non-English snippet (no the Hollow Tide, no Coalfall); (c) descope multilingual
...
- **DECISION — historicity convention.** Many Cat 2a comments and Cat 5 archived
  docs record *real past runs* against Bonus Marlow (observed char lists, char
...
- **Cardinality** — the Hollow Tide fixtures may name more distinct characters than
  Coalfall's 14 (e.g. a fixture with Wren+Marlow+Oduvan+Maerin+Linnet+narrator =
...
- **Mapping completeness** — enumerate **every** the Hollow Tide name in use (not just the
  top 8: also Maerin, Linnet, Sela, Lessom, Dame Linnet, etc.) before renaming.
