...
| D7 | **`completionPct`** = consumed listenable-seconds ÷ total listenable-seconds (for the progress bars only). **`finished` = resume position at/near the end of the *final chapter*** (position-based, decoupled from `completionPct`, live-derived, not latched) | Avoids the "99% of a 10-hour book = 6 min left, prematurely finished + dropped from the rail" bug. Durations from the chapter `duration` string (see S1 below). |
...
2. Validate: `sessionId` non-empty string; **`days.length ≤ 366`** (bound the write — S4); each `date` an ISO `YYYY-MM-DD` within `[fixed past floor, now + skew window]` — reject far-future *and* absurd-past dates (S5); `seconds` finite, `0 ≤ seconds ≤ 86400` (S4 — can't accrue more than a day of wall-clock in a day per session).
3. **Under the per-book mutex (D12):** read current file, upsert each `(date, sessionId)` slot to **`max(existing, incoming)`** — never blind overwrite, so a delayed/out-of-order retry can't lower a higher stored absolute (S2; safe because a session's per-`(book,date)` absolute is monotonic). `writeJsonAtomic`.
4. **Wire → storage mapping:** the wire body carries *one* session's contribution (`{ sessionId, days[] }`); the server folds each `(date, sessionId)` into that date's `sessions[]` in storage (`date → sessions[]`). The shapes differ by design (S6) — the wire is one session, storage merges across sessions.
5. A batched `days` array lets the companion flush several offline days in one call.
...
- **Durations** (S1): the persisted chapter shape in `state.json` has **no plain `durationSec`** — it carries `duration?: string` (formatted, e.g. `"12:34"`) plus an optional PCM-exact `audioQa.durationSec` (only when QA ran). `completionPct` parses the readily-available `duration` string (whole-second precision is fine for a %); use `audioQa.durationSec`/segments/srv-32 only if precision ever matters. `completionPct = (Σ durations before resume chapter + currentSec) ÷ total listenable duration`; divide-by-zero → `0%` (first-run guard). Unrendered chapters have no duration, so a still-generating book's denominator is the rendered chapters only (documented mid-generation caveat).
- **`finished`** (S3) is position-based: the resume bookmark is in the **final *listenable* chapter** — the last non-`excluded`, non-`held` chapter that has audio (P1) — and `currentSec ≥ finalChapterDuration − max(30s, 2%)` (P2). Decoupled from `completionPct`. Used identically for `booksFinished` and the rail exclusion.
...
- In-progress only: **excludes `finished`** (the S3 end-of-final-chapter predicate — so a long book with minutes left still appears) and books under the **`>5s` noise floor**; sorted by resume `updatedAt` desc.
...
`src/lib/api.ts` gains `getLibraryStats`, `getContinueListening`, and `putListenStats`, with **both real and mock implementations** — the mock (`src/mocks/canned-data.ts`) is mandatory (P4), since the frontend e2e runs in `VITE_USE_MOCKS` mode and the rail/stats specs are dead without it. All writes (web + companion) flow through the single Node server process, so the in-memory per-book mutex (D12) fully covers concurrency — no cross-process race (P5).
...
New `Stage` kind `stats` (the `Stage` union already carries ~13 top-level kinds — this is the routine pattern). The plan must touch **all** the router seams (P3): the `Stage` union in `types.ts`, `parseHash` (inbound) *and* `stageToHash` (outbound, → `#/stats`) in `router.ts`, the `App.tsx` render switch, the `a11y.test.tsx` core-views list, and a new case in `e2e/responsive/coverage.spec.ts` (per the "adding a new view" convention). Entry point: a link from Account / library.
...
- **Finished may diverge from the companion (D-M3):** the dashboard's `finished` is position-derived (end of final chapter); the companion's local `setChapterFinished` flag is *not* synced (report-only).
- **Trusted client inputs within the LAN boundary:** a client supplies its own `seconds` and local `date`; a wrong client clock pollutes a day (bounded by the past-floor/future-skew validation, S5), a buggy client could inflate hours (bounded by the per-entry `seconds ≤ 86400` cap, S4). Acceptable for a LAN personal tool; stated as a boundary.
- **Simultaneous multi-device playback double-counts (S7):** listening on phone and web in the same wall-clock hour sums to two hours — two distinct sessions, each accruing real time. Rare; defensible (two streams consumed); documented rather than de-duplicated.
...
- **Server unit:** stats merge (idempotency, multi-session sum, no-clobber under the mutex, **`max()` upsert vs out-of-order/stale retry — S2**, sessionId/date validation incl. **past-floor + future-skew rejection and `days.length`/`seconds` bounds — S4/S5**); aggregation (completion math from parsed `duration` strings + divide-by-zero, **end-of-final-chapter `finished` predicate — S3**, per-series imported-count, empty=zeros not NaN); `byDay` summation across books + sessions; continue-listening sort + finished/noise-floor exclusion.
