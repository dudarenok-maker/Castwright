...
- **tts**: **`sidecar-blocked`** (new first layer — fires when the just-computed `sidecar` diagnosis is `status: 'fail'` with a cause **other than** `unreachable-transient`, i.e. `venv-missing` / `python-missing` / `supervisor-exhausted` / `supervisor-tripped`) → `no-engine-installed` (`!anyTtsEnginePresent`) → `weights-missing` (an installed engine's `detectQwenInstallStateOnDisk`-style probe reports weights missing) → **`cannot-confirm-engine`** (new — sidecar diagnosis is `status: 'fail', cause: 'unreachable-transient'`, i.e. no actionable sidecar cause but the sidecar still isn't reachable) → `package-broken` (only reachable once `sidecar.status === 'pass'`; sidecar-confirmed `kokoroPackageInstalled`/`qwenPackageInstalled === false`) → `pass`.
  ...
  - **The gate excludes `unreachable-transient` on purpose (round-2 review finding 3, `Significant`).** The original round-1 fix gated on *any* `sidecar.status === 'fail'`, which meant a merely-booting sidecar (no actionable cause, nothing to click) would suppress `no-engine-installed`/`weights-missing` — disk-only checks that are evaluable independent of sidecar reachability — and point the user at a sidecar card with no action, reproducing the very "diagnosis with no pathway" problem this spec exists to fix, just one layer up. Excluding `unreachable-transient` from the gate lets those two disk checks fire normally during a transient sidecar hiccup.
  - **`cannot-confirm-engine` exists so that exclusion can't reopen finding 2's false-pass.** `package-broken`'s only signal (`kokoroPackageInstalled`/`qwenPackageInstalled`) is `undefined`, not `false`, whenever the sidecar isn't reachable — so simply letting the chain fall through past a transient sidecar straight to `package-broken` would silently under-evaluate it, and falling through further to `pass` would resurface the exact false-pass finding 2 fixed. `cannot-confirm-engine` is a conservative `status: 'fail'` terminal (text-only, no action: "Waiting for the voice engine to respond to confirm this — try again shortly") for exactly the gap between "disk checks didn't find a problem" and "the live signal needed to rule one out isn't available yet" — matching the same "resolve uncertainty downward" philosophy `qwen-install-detect.ts`'s own doc comment already states for its on-disk probe.
...
- Merge `clearTripAndRespawn()` into a single `resetAndRespawn()` whose **first synchronous statements** (before anything `await`s) reset **both** `restart43Trip`/`restart43Timestamps` (existing) **and** `consecutiveFailures` (new) — only *then* does it call (and `await`) `spawnOnce()`. Keeps the existing "safe to call when not tripped" idempotency guarantee for the exhaustion case too. All existing call sites of `clearTripAndRespawn` (Plan 2's auto-revert route) are updated to call `resetAndRespawn()`.
- **Concurrent-call safety (corrected in round 2 — the original `respawnEpoch` design fixed the wrong race).** Round-1's fix guarded a `resetAndRespawn()` call landing *during* the plain-exhaustion backoff window. Round-2 review (finding 1, `Critical`+`Contradicted`) traced the actual control flow: both give-up paths (`onChildExit`, code-43 trip at line 306-317 and plain exhaustion at line 325-331) `return` *before* ever scheduling that backoff IIFE — the backoff that produced the child which just died already fired its own `spawnOnce()` at the *prior* exit. So there is no in-flight backoff continuation for a manual reset to race through the routes that can actually call `resetAndRespawn()` (both require `current() === null` **and** `exhaustedEvent()`/`tripEvent()` true) — the epoch mechanism defended an unreachable window. The genuinely reachable hazard is **two direct `resetAndRespawn()` calls** (a double-click on the fix button, or `POST /api/sidecar/restart`'s exhausted branch racing Plan 2's auto-revert route calling it too) — each would call `spawnOnce()` unconditionally, racing two children for one port. **This is what "first synchronous statements reset state before any `await`" above actually fixes**, not an epoch counter: Node's run-to-completion guarantee means whichever of two near-simultaneous callers' handler bodies starts executing first runs synchronously through `resetAndRespawn()`'s state-clear *before* the event loop can start the other caller's handler body (no `await` separates route entry from the clear) — so the second caller always observes the already-cleared state (`exhaustedEvent()`/`tripEvent()` both false) and falls through to the existing generic "nothing to reset" path, never reaching a second `spawnOnce()`. No epoch counter, promise-dedup cache, or new injectable is needed; the guarantee falls directly out of ordering the synchronous reset before the `await`, which the merge already required.
