...
2. **Constrain `h` to RFC1918** (loopback incl.; link-local excluded to match `enumerateLanUrls`) — Task 8. Covers **all** entry paths: scan/deep-link via `PairingQr.parse`, and the **manual/edited** path by routing `_pair()` through a new public `PairingQr.checked` factory (the plain constructor bypassed validation). The confirm screen **surfaces the host prominently** and makes it **read-only on deep-link opens**, so the banner can't diverge from what's redeemed.
