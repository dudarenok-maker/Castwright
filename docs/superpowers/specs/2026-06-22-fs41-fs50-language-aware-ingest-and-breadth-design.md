...
- **Follow-ups filed (none blocked the ship):** #1027 per-language sample books (voices are per-language designs → the demo pack needs a per-language sample), #1028 non-English minor-cast protection (the seam-3d `taggedSpeakerIds` gate-off folds minor speakers like Berrin/Ivo), #1029 chapter-assembly stall with the `SEG_SPK` drift gate on (8 GB box) — **closed**, #1030 recycle-storm single-owner guard — **closed**, srv-48/#1038 local-model option for persona generation (Gemini-only today).
- **#1034 / `fs-62` (translate the voice-design persona) — CLOSED won't-fix (2026-06-23).** Deep research + the shipped-ES result established that the persona/`instruct` should stay **English**: Qwen VoiceDesign's `instruct` is English/Chinese only, and spoken-language + accent ride a separate `language`/calibration-text channel (#1019) — not the persona. So **FR/DE are NOT gated by persona-i18n**; they proceed with English personas exactly as ES did. Rationale + citations: [`docs/research/2026-06-23-qwen-voicedesign-persona-language.md`](../../research/2026-06-23-qwen-voicedesign-persona-language.md).
