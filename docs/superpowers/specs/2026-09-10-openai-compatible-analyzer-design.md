...
- **Truncation.**
  ...
  - **Fail:** a `length` finish with no answer text and reasoning evidence raises `AnalyzerReasoningOverflowError`. It maps to `analyzer-reasoning-overflow` and never splits.
    ...
    - **Structured fixes:** `AnalysisFailure` (`failure-taxonomy.ts:400`), the SSE error payload and OpenAPI gain an optional `fixes: AnalysisFailureFix[]`, built by `reasoningOverflowFixes(ctx)`. Each fix is `{ label, settingKey?, endpointField?: { endpointId, field }, wikiPage? }`. `wikiPage` is a page name: `src/lib/wiki-links.ts` forbids `#anchor` fragments, so the frontend builds `WIKI_BASE/<page>`. The wiki link is its own entry: each engine's list ends with one `{ label: 'Read: <section title>', wikiPage }`, and no other fix carries `wikiPage`. `AnalysisFailureFix` and `reasoningOverflowFixes` live in `server/src/routes/failure-taxonomy.ts`, and `ctx` is `{ transport, model, endpointId? }`. No transport throws `AnalyzerReasoningOverflowError`: the runner's `mapFinish` does, called from `StageRunner.send`. Its constructor is `(transport, model, reasoningTokens, opts?: { endpointId?, reasoningLevel? })`. `endpointId` arrives in 3b and comes from `OpenAIAnalyzer`, never `OpenAITransport`. It is set on both exits: a thrown error, via `withEndpointId`, and an escalation overflow reported through `StageCall.onReasoningOverflow`, whose hook `OpenAIAnalyzer` wraps. `reasoningLevel` is added to the same object in 5a, never positional: `mapFinish`'s ctx gains it, and `StageRunner` passes the level it resolved for that request. Wave 5a adds `reasoningSetting?: { engine: 'gemini' | 'ollama'; model }`, because Gemini and Ollama reasoning levels live in the per-model `analyzerReasoningByEngine` map, which is neither a registry key nor an endpoint field.
    ...
    - **Wiki link entry:** each engine's fix list ends with one `{ label: 'Read: <section title>', wikiPage }` entry, and no other fix carries `wikiPage`.
      ...
      - **5b:** the payload fix adds `{ label: 'Read: Custom payload', wikiPage: 'OpenAI-Compatible-Analyzer-Endpoints' }`.
    - **Staging:**
