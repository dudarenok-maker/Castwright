...
**Add a failing test** to `setup-readiness.route.test.ts` (stub `selectTtsProvider` → a provider whose `synthesize` returns `{ pcm: Buffer.alloc(24000*2*0.3,0), sampleRate: 24000, mimeType: 'audio/L16' }`, and stub `encodePcmToAudio` → `Buffer.from([0xff])`; stub the analyzer probe). Assert: `POST /api/setup/smoke` → 200 `{ ok: true, url: <string>, analyzerOk: <bool> }`. Add a second case: when `synthesize` throws → 200 `{ ok: false, stage: 'synth', error }` (NOT 500). Use the test-injection seam — if `setup-readiness.ts` doesn't already allow injecting `selectTtsProvider`/`encodePcmToAudio`, add a small `__setSmokeDepsForTests` hook (mirror how other routes inject deps), OR `vi.mock` the `../tts/index.js` + `../tts/mp3.js` modules.
...
- `try { const provider = selectTtsProvider(modelKey); const { pcm, sampleRate } = await provider.synthesize({ text, voiceName, modelKey }); const mp3 = await encodePcmToAudio(pcm, sampleRate); write mp3 to a temp/public file; url = ... } catch (e) { return res.json({ ok: false, stage: 'synth'|'assemble', error: e.message }); }` (wrap synth and assemble so the failing stage is reported). For the URL: mirror `voice-sample.ts`'s public-file write + `publicUrl` (reuse its helper if exported; else write to the same dir).
...
- [ ] **Step 1: READ the current `step-finish.tsx`** (the placeholder block + Finish button). **Failing tests** (mock fetch / `api`): clicking "Run smoke test" calls `api.runSmokeTest` and, on `ok:true`, renders an `<audio>` player with the returned `url` + an analyzer-status line; on `ok:false`, renders the error + the failing stage. "Hear the demo book" calls `onTryDemoBook`. "Finish setup" calls `onFinish`. Keep `data-testid="smoke-test-placeholder"` on the (now-enabled) Run-smoke-test button so e2e can target it.
- [ ] **Step 2: Run → FAIL.**
...
- [ ] **Step 1:** `SetupWizard` + `SetupView` accept + forward `onTryDemoBook` to `StepFinish` (only StepFinish uses it). In `SetupRoute`, implement `onTryDemoBook` by reusing the EXISTING fs-22 demo flow from `BooksRoute.onTrySample` (`src/routes/index.tsx` — `api.loadSample('the-coalfall-commission')` → `api.getLibrary` → `libraryActions.hydrate` → `uiActions.openBook(...)`). Extract that handler into a shared helper if cleanest, or inline the same sequence. After it dispatches `openBook`, the user leaves `#/setup` for the normal pipeline (the demo lands in cast-confirm → confirm → generate → listen) — that's expected; the smoke "full run" rides the shipped pipeline.
