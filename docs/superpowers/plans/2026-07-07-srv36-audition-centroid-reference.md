...
- Total render attempts for a character, across the initial pool-fill AND any bimodal-safety top-up, must never exceed `AUDITION_POOL_TARGET_N + AUDITION_POOL_MARGIN` (= 8). This is a hard invariant, not a target — cost wins over reaching the target pool size when they conflict (degrades to the existing `too-short` → inconclusive outcome).
