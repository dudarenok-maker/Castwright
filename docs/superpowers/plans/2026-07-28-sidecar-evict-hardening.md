...
- **On the way OUT of a forward, the `_last_used` re-stamp must happen BEFORE the in-flight count drops** — the mirror of the rule above, and equally load-bearing. Decrementing first leaves a window where the count reads 0 while the timestamp is still the pre-forward value, which `maybe_free_idle*` (called with `ttl=0.0` from the admission path) will happily evict on. Put the final stamp in an inner `finally` **inside** the claim; a plain `with counter.claim(): yield` puts it after the decrement, because the context manager's decrement runs as soon as the block body ends.

  **This is not hypothetical.** Task 1 found three sites shipping the broken order on `main` today — `CoquiEngine.synthesize`, `WhisperEngine.transcribe`, `SpeakerEngine.embed` all decremented in a `finally` and re-stamped afterwards. Only `_base17_activity` and `design_voice` had it right. Do not assume existing code is a safe template for this; check the site you are touching.

- **A test for an ordering rule must destroy the sentinel from INSIDE the forward.** Zeroing `_last_used` before the call proves nothing: every one of these methods re-stamps on the way *in*, so the sentinel is gone before the forward starts and the assertion holds under both orderings. Measured — an early revision of Task 1's own regression test did exactly this and passed against a faithful revert. Hook the fake model (`_FakeTts.on_enter`, the fake `transcribe`, the fake `encode_batch`) so the zero is the last write before the forward returns, then assert the observed value at the decrement is non-zero.
