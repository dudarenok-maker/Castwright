...
- **Commit convention:** `<type>(<scope>): <subject>`. Scopes here: `server` (Node), `sidecar` (Python), `config`/`docs` as needed. End every commit body with the `Co-Authored-By` trailer the repo uses. Never `--no-verify`.
...
- Test: `server/src/tts/engine-vram-cost.test.ts` (add a `spk` case; create the file if absent)
...
Check whether `server/src/tts/engine-vram-cost.test.ts` exists. If it does, append the `describe` block below; if not, create it with this content:

```ts
import { describe, it, expect, afterEach } from 'vitest';
import { ENGINE_VRAM_COST, costForEngine } from './engine-vram-cost.js';
...
git commit -m "config(server): add spk VRAM weight + idle-TTL knobs for srv-47"
```
...
- Test: `server/tts-sidecar/tests/test_embed.py` (create)
...
Create `server/tts-sidecar/tests/test_embed.py`:

```python
"""SpeakerEngine / `/embed` coverage — the render-integrity ECAPA gate's cuda
path (srv-47).

The real `speechbrain` package isn't installed in CI / the dev venv. These
tests stub it via sys.modules so the load path executes without model weights,
then assert on the engine's device handling + idle eviction. Pins the srv-47
invariants:
  - cuda unavailable -> degrade to cpu at load (no crash),
  - cuda available but a non-poison load failure -> demote to cpu + reload,
  - a poison-matching load failure -> re-raised (so /embed can fence it),
  - maybe_free_idle frees after TTL on cuda, is a no-op on cpu, simple recency.
"""
...
Run: `npm run test:sidecar`
Expected: PASS for the `test_embed.py` device/eviction cases.

- [ ] **Step 5: Commit**

```bash
git add server/tts-sidecar/main.py server/tts-sidecar/tests/test_embed.py
...
- Test: `server/tts-sidecar/tests/test_embed.py` (extend)
...
Append to `server/tts-sidecar/tests/test_embed.py`:
...
- Test: `server/tts-sidecar/tests/test_embed.py` (extend — HTTP-level via `TestClient`)
...
def test_embed_load_poison_is_fenced(monkeypatch: pytest.MonkeyPatch):
    ...
    # Fresh engine on cuda; reset the process-poison flag so the route runs.
