...
- Modify: `server/.env.example` (add `GPU_WEIGHT_SPK` + `SPK_IDLE_TTL` doc lines)
...
In `server/.env.example`, after the `GPU_WEIGHT_ASR=1` line and its comment, add:

```
# VRAM token cost for a render-integrity ECAPA speaker-embed op. Only charged when SPK_DEVICE=cuda; the CPU-default path takes no token. Read live per-op via costForEngine(); changes take effect on the next embed without a restart. [live · high risk]
GPU_WEIGHT_SPK=1
```

After the `ASR_IDLE_TTL=120` line and its comment, add:
...
> **Regression guard:** the existing tests in this file (`test_embed_is_unit_norm_and_192d`, `test_self_cosine_is_one`, `test_embed_endpoint_raw_body`) MUST stay green. The Task-3 refactor preserves `embed()`'s signature and its `if self._model is None: raise` guard (it only adds a `_last_used` stamp), so they keep passing. Your stub-based tests use fresh `main.SpeakerEngine()` instances + `monkeypatch.setitem(sys.modules, ...)` (auto-reverted per test), so they don't disturb the real-`speechbrain` tests.
