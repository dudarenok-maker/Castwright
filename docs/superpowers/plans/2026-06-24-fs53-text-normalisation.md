...
export interface LangNormalizer {
  ...
  /** 12 nominative month names; genitiveDates used by date rendering if present. */
...
export const en: LangNormalizer = {
  cardinal, ordinal, year, decade,
  separators: { decimal: '.', thousands: ',' },
  decimalWord: 'point',
  currency: { '$': usd, '£': gbp, '€': eur },
  months: { nominative: ['January','February','March','April','May','June','July','August','September','October','November','December'] },
  symbols: { '%': 'percent', '&': 'and', '°': 'degrees', '#': 'number', '@': 'at', '×': 'times' },
  abbreviations: [
...
/** Spell a parsed number for speech: integer via cardinal, fraction digit-by-digit. */
export function speakNumber(n: number, norm: LangNormalizer): string {
  if (Number.isInteger(n)) return norm.cardinal(n);
  const [intPart, frac] = String(n).split('.');
  const digits = frac.split('').map((d) => norm.cardinal(Number(d))).join(' ');
  return `${norm.cardinal(Number(intPart))} ${norm.decimalWord} ${digits}`;
}
```
...
function expandPercentAndSymbols(s: string, norm: LangNormalizer): string {
  ...
  if (norm.symbols['%']) s = s.replace(new RegExp(`(${num})\\s?%`, 'g'), (_m, raw) =>
    `${speakNumber(parseLocaleNumber(raw, norm.separators), norm)} ${norm.symbols['%']}`);
  for (const [sym, word] of Object.entries(norm.symbols)) {
    if (sym === '%') continue;
    s = s.replace(new RegExp(sym.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g'), ` ${word} `);
  }
...
function expandYears(s: string, norm: LangNormalizer): string {
  return s.replace(/\b(\d{4})\b/g, (m, y, offset: number, full: string) => {
    ...
    const prev = full.slice(0, offset).trim().split(/\s+/).pop();
...
function expandNumbers(s: string, norm: LangNormalizer): string {
  return s.replace(new RegExp(numberToken(norm), 'g'), (raw) =>
    speakNumber(parseLocaleNumber(raw, norm.separators), norm));
}
...
function expandDates(s: string, norm: LangNormalizer): string {
  // Textual-month form: "January 3, 2026" / "3 January 2026".
  const months = norm.months.nominative.join('|');
  const md = new RegExp(`\\b(${months})\\s+(\\d{1,2})(?:st|nd|rd|th)?,?\\s+(\\d{4})\\b`, 'g');
  s = s.replace(md, (_m, mon, day, yr) => `${mon} ${norm.ordinal(Number(day))}, ${norm.year(Number(yr))}`);
  const dm = new RegExp(`\\b(\\d{1,2})\\s+(${months})\\s+(\\d{4})\\b`, 'g');
  s = s.replace(dm, (_m, day, mon, yr) => `${norm.ordinal(Number(day))} ${mon} ${norm.year(Number(yr))}`);
  return s;
}
```
...
// ...
const langCode = bookLanguage ? normaliseBookLanguage(bookLanguage) : undefined;
```
...
**Rules (from spec):** `dieciséis…veintinueve` contractions; `quinientos/setecientos/novecientos`; `cien` vs `ciento`; `y` joins tens–units only (`treinta y uno`, NOT `ciento y uno`, NOT inside 21–29); gender floor masculine; separators `.`=thousands `,`=decimal; decimal word `coma`; decade drops the century (`los noventa`); currency connector `con`, minor `céntimos`.
...
- Dates: day as **neuter ordinal**, month in **genitive** (`months.genitiveDates`).
...
**Rules:** `soixante-dix`/`quatre-vingts`/`quatre-vingt-dix`; `vingt`/`cent` pluralisation common-case; separators space=thousands `,`=decimal; decimal word `virgule`; decade `les années quatre-vingt-dix` (no century-drop); currency connector `et`, minor `centimes`. `supported:false` ⇒ dormant end-to-end.
...
- [ ] **Step 5: Run → PASS.**
- [ ] **Step 6: `__fixtures__/fr.txt`** — but DO NOT add `fr` to the `expandForSpeech` fixture loop (it would no-op). Instead drive fr fixtures through the **engine** or a temporarily-supported shim. Simplest: assert fr fixtures by calling the passes with a forced normalizer — skip the end-to-end loop for dormant languages and rely on `fr.test.ts` engine coverage. Document this in a comment in `fixtures.test.ts`.
...
**Rules:** unit-before-ten one-word compounding (`einundzwanzig`); `eins`→`ein` before a scale word (`einhundert`, `eintausend`); separators `.`=thousands `,`=decimal; decimal word `Komma`; decade `die Neunzigerjahre` (century dropped); currency connector `und`, minor `Cent`. Dormant (`supported:false`).
...
- [ ] **Step 5: Run → PASS.**
- [ ] **Step 6: `__fixtures__/de.txt`** with the **separator-guard** known-failure pinned: `Version 1.5 ist da. ⇒ Version eins Komma fünf ist da.` and `Es kostet 1.200,50 €. ⇒ Es kostet eintausendzweihundert Euro und fünfzig Cent.` Drive via the engine (dormant end-to-end), as in Task 6 Step 6.
- [ ] **Step 7: Run server normalize suite → PASS.**
...
- **Known integration risk to watch during execution:** the `expandPercentAndSymbols` generic symbol replace is greedy — verify it doesn't eat `&`/`#` inside words; restrict to standalone occurrences if a fixture regresses.
