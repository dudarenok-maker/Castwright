# AMD GPU Support — Phase 2 (Behavior Change + AMD Enablement) Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans. Steps use checkbox (`- [ ]`) syntax. **This plan ships behavior changes** — it is gated on the Phase 1 foundation being merged and on the Section-0 spike. Per the spec's delivery model, it may be **built** against the desk-pass findings but **must not be RELEASED** until the 🔴 on-AMD acceptance (Wave H) passes.
