...
**Type consistency:** `StageScore.byFamily` changes to `Record<string, FamilyBreakdown>` in Task 1 and is consumed by `aggStage` in Task 2 with the same `{correct, attributed, drift}` shape. `strength?: 'weak'` is defined in Task 5 (types.ts) and read in Task 6 (`span.speaker!.strength === 'weak'` in cross-examine.ts) — consistent. `isFillEligible` (Task 3) and `CONFIDENT_ANCHOR_MIN` (Task 4) are module-private, no cross-task signature coupling. `runEval` return type gains `runs` and `fixtures: FixtureAgg[]`; the only external consumer (`run-eval-cli.test.ts`) reads `.skipped`, still present.
