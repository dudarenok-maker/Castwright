...
**Later waves cite earlier waves by symbol, not by line.** Each PR's first task re-reads every cited `file:line` against `main` at the time it starts (all citations are verified at `main` `80be2f1d`). It also runs the `git grep` checks its entry criteria name. A symbol that moved is followed; a symbol that changed shape is reported to the coordinator before the task proceeds.

## Global Constraints

- **Wave 0 gates everything.**
  - Do not start wave 1 until #3139 and #3141 are merged. **Wave 0 is cleared (2026-09-13): wave 1 can start.**
    - #3139 was discharged by PR #3163, merged 2026-09-11.
    - #3141's fix, PR #3192 ("make Advanced Settings own analyzer endpoint and phase models"), merged on 2026-09-13; `origin/main` is now `80be2f1d`.
    - #3141 mattered because it is exactly the bug where Advanced Settings' analyzer-model overrides are silently overridden by Account-tab fields, and every wave here reads those saved per-phase models.
  - **The plans are verified at `80be2f1d`; re-read any citation against current `main` before editing,** locating each construct by symbol.
  - **#3192 moved symbols, not just lines.**
    - `getResolvedOllamaUrl` and `getResolvedOllamaModel` moved file, from `workspace/user-settings.ts` to `server/src/config/ollama-resolved.ts` (`:21`, `:35-39`). Import them from `config/ollama-resolved.js`.
    - #3192 also touched:
      - `analyzer/voice-style.ts`, `analyzer/ollama.ts`, and `analyzer/select-analyzer.ts` (a new per-run `phaseModel` tier; no env or settings reads);
      - `routes/analysis.ts` (per-run `requestedPhase0Model` / `requestedPhase1Model`);
      - `routes/user-settings.ts` (the `RETIRED_ANALYZER_FIELDS` 400);
      - `routes/models-inventory.ts`;
      - `workspace/user-settings.ts` (phase models and `ollamaUrl` migrated into `configOverrides` by `migrateLegacyAnalyzerModelFields`, and removed from the schema);
      - `src/lib/api.ts` (mock PUT rejection);
      - `src/components/model-settings-form.tsx` (read-only rows);
      - `src/views/advanced.test.tsx`, `src/lib/models.ts` and `openapi.yaml`.
...
- **Wiki upkeep (P33).**
  ...
  - The same guard asserts the "— N knobs across M groups" count in `Advanced-Settings.md:14` ("116 knobs across 12 groups" at `80be2f1d`). A PR that adds or removes a knob updates that count: 2b adds two knobs and 3d one.
...
- **Import cycles.** New analyzer modules import `StageCall` and other analyzer types from the leaf `server/src/analyzer/types.ts` (wave 1), never from `server/src/analyzer/index.ts`. Madge counts type-only imports as cycle edges (`npm run check:cycles`).
  - **The baseline is a committed list, and it moves.** `server/madge-cycles-allowlist.json` is checked as a list of cycles, not a count. PR #3192 (merged 2026-09-13, `80be2f1d`) left it **unchanged**: `git diff 46e62a34 80be2f1d -- server/madge-cycles-allowlist.json` is empty. It avoided a `config/resolver.ts` ↔ `workspace/user-settings.ts` cycle by adding the leaf `config/ollama-resolved.ts` instead. Still re-read the file on current `main` rather than assuming this plan's picture of it. Every leaf this plan introduces to dodge a cycle — `transports/allowlisted-fetch.ts`, `known-secrets-gate.ts`, `reasoning-level.ts` (5a), `resolveEndpointApiKey`'s structural parameter, and validating request controls in the PUT route instead of `writeUserSettings` — stays required: a new cycle is a deliberate allowlist row with a reason, never a silent edge.
...
| P4 | Reasoning deltas (`reasoning_content`, `reasoning`, non-empty `reasoning_details`) and Gemini thought-only chunks call `onChunk` with an unchanged byte count. | Keeps the analysis route's silence warning from firing during a long think (`analysis.ts:1179`, `:4421-4427`). |
...
| P23 | Until PR 3d, `openai:` ids are refused wherever a model id can be saved.
- **At `80be2f1d` the phase models are config knobs, not settings fields.** The general settings PUT already rejects the retired `ollamaUrl`, `analyzerPhase0Model`, `analyzerPhase1Model` and `analyzerPhase1MinLagChapters` outright (`RETIRED_ANALYZER_FIELDS`, `routes/user-settings.ts:42-47`, rejected at `:122-130` before any other validation).
- **Refused field:** `defaultAnalysisModel` only, checked after that retired-field block.
- **Refused knobs and env:** `analyzer.phase0.model`, `analyzer.phase1.model` and `analyzer.ollama.model`.
- **Resolver:** `getResolvedOllamaModel` (`config/ollama-resolved.ts:35-39`) ignores an `openai:` value from env or override, returning the knob default through `resolveKnob`, never a raw env read.

Selection Selection maps a stray one to `analyzer-endpoint-missing`, never a silent Ollama run, and every selection call site (analysis phase 0, subset, phase 1, annotate-emotion, instruct-annotation, script review) catches any analyzer error selection throws and reports its classified code (`analyzer-endpoint-missing`, `auth` for a key-origin mismatch, and so on), so no selection error ends a stream uncoded. Until 5a/5b, endpoint routes refuse a non-default `reasoning` and a non-empty `extraParams`. | The model and phase fields are plain strings, and values saved before wave 5's checks exist would bypass them permanently. |
...
- **Scope (re-verified by reading `80be2f1d`):** this covers **schema** failures on individual entries — the whole-file fallback at `user-settings.ts:579-582`. An unparseable `user-settings.json` is a different path and does **not** reject: `performUserSettingsRead` recovers it from its `.bak.N` backups, and when the file and every backup are unreadable it warns once and returns in-memory defaults with `corrupt: true` (`:535-554`). `readUserSettings` rejects only when `readFile`/`stat` itself fails or the legacy-settings migration fails. `dropInvalidEndpointEntries` sits **inside `performUserSettingsRead`** (`:528-587`), after the JSON parse and the legacy migrations (`migrateLegacyEagerLoadFields`, then #3192's `migrateLegacyAnalyzerModelFields`, `:570-571`) and before the whole-file schema parse (`:579`) — it extends that shipped function rather than replacing or wrapping the read path, and changes nothing about the malformed-file path. Dropping an entry therefore never sets `corruptSettingsFile`: that flag means the file itself was unreadable and was recovered, and a dropped entry is reported by its own warning naming the archive file instead. Before any later write can persist the drop, the entry is appended to `user-settings.invalid-endpoints.json`, and the warning names that file.
- **Concurrent reads:** the cold settings read is single-flight — satisfied by PR #3195's shipped `inFlightRead` (`:494`), so the plan adds no wrapper of its own.
...
- **Save validation (F5):** endpoint create/update, the key write and the settings PUT validate every endpoint field before writing (server 3b, UI 3d). In the settings PUT that check runs after #3192's retired-field rejection (`routes/user-settings.ts:122-130`) and before `writeUserSettings` (`:131`).
...
- **Keys:** dropped key entries are archived with their origin only, never the key. An OpenAI answer that has a `finish_reason` counts as complete even if the idle watchdog fires, or the socket drops, before `[DONE]`. Detect builds `props` relative to the base URL with a trailing `/v1` removed, keeping any reverse-proxy path prefix. | `readUserSettings` replaces the whole file with defaults when any field fails the **schema** (`user-settings.ts:579-582`), including the saved Gemini key. A malformed file takes a different path — backup recovery, else defaults with a corruption flag — so this decision is about schema failures on individual entries. A finished answer must not be retried away. `new URL('/props', base)` drops a proxy's path. |
| P20 | **Approved by the owner 2026-09-13** — stop new spend, with a loud, actionable warning (F7; see the sub-row below and P34). A reasoning-overflow failure ends the analysis run, as `GeminiContentBlockedError` does (`analysis.ts:4554`, `:7199`; stage 2 through `runPhase1Pool`'s terminal handler), and the job starts no new model calls after it. **Alternative considered, not chosen:** skip the chapter and continue, which keeps chapters that fit (an overflow need not recur on the next chapter) but can spend a full thinking budget on each chapter that doesn't. It also stops a script-review pass, as a content block does (`script-review.ts:924-986`).
- **In-flight chapters:** they finish and cache for resume, as the pools are designed to (`analysis.ts:5676-5679`). The job is not aborted, because aborting would discard work a resume must redo, including another phase's model in pipelined mode.
...
- **Escalation and non-story classification:** after an overflow, the job starts no further escalation windows (up to 120 per chapter, `registry.ts:1325-1352`) and no further non-story classification calls (`analysis.ts:5835-5837`). A single escalation call that overflows still returns `null`, but it also marks the job through `StageCall.onReasoningOverflow`.
...
- **Voice design:** P20 covers analysis runs and their passes. In a voice-design job, today's behaviour is kept — and on `80be2f1d` that behaviour is **per character on both persona paths**:
...
| P8 | `tpm: 0` in `analyzerRateLimitsByModel` means unlimited, matching the retired `rate.*.gemma*` knobs. Saved gemma overrides are copied into the map when settings load, without failing the load, by `migrateLegacyRateLimitOverrides(migrateLegacyAnalyzerModelFields(eagerLoadMigrated))`, which keeps #3192's migration (`user-settings.ts:570-571`). | Keeps saved overrides meaningful after the knobs are removed. |
...
| P14 | Pre-run checks, and the digest read they need, run only when a new job is created, after the rejoin path. A live-job re-check follows the read, with no `await` before registration (#3004). They use the same selection resolution as the job (`resolvePhaseModelSelection`), which follows `selectAnalyzerForPhase` at `80be2f1d` exactly (`select-analyzer.ts:86-124`): env → per-run `phaseModel` → `opts.model` → saved Advanced override → default. It is one shared resolver extracted from `selectAnalyzerForPhase`, never a reimplementation. `preflightTargets` and the run busy marks take the run's `requestedPhase0Model` / `requestedPhase1Model`, and a per-run phase pick reports source `run-pick`. So a run whose `phase1Model` names an endpoint is pre-checked and busy-marked for that endpoint, and an unsaved endpoint named in `phaseModel` is refused before start. The subset rejoin keeps main's `sameChapterSet` rule (a live job for a different chapter set answers `subset_in_progress`). A selection failure maps to a coded failure. | Checking before rejoin refuses a reload of a live job. A check that classifies ids differently from selection passes and then fails without a code. |
...
| | **Activation:** the target runs with its own limiter, concurrency, key-origin check and capability check; a capability refusal fails the call naming the target. A GPU-bound target (`local`, or an endpoint whose `gpu` is not `none`) takes the same in-flight and run busy marks a primary would, only once fallback activates; a switch after the job's release has run takes no mark. After a switch, later chunking uses the target's capacity, and phase-1 events carry the target engine. Up to K chapters (the pool width, default 2) can already be sized for the primary, so before routing any call to the target `FallbackAnalyzer` checks the prompt body's characters against the input budget the chunker would compute for that pass on the target's capacity (`resolveStage1ChunkCharBudget(targetCapacity, body)`, `resolveStage2ChunkCharBudget(targetCapacity, body)`, `chapterChunkBudget(targetCapacity, …)`): fraction × context at 2 chars/token, bounded by the per-request cap and the pass ceiling, so split halves keep room to answer. It never compares `estimateInputTokens` against raw `contextTokens`. The check covers every analyzer call a fallback can serve. Passes that split on `AnalyzerTruncatedError` (stage 1 chunks, stage 2, script review) get it thrown, split the chunk and retry on the target. When one cannot split further (max split depth, or an unsplittable span) and the reason is `'input-over-target-budget'`, it is converted to `AnalyzerTargetInputTooLargeError` before it escapes, so the copy never says "response truncated". Passes that cannot split (emotion annotation, instruct annotation, attribution escalation, non-story classification, whole-book stage 1) fail that call with `AnalyzerTargetInputTooLargeError` (→ `analyzer-request-rejected`, own copy naming the fallback target and its input limit; not an `AnalyzerTruncatedError`, not retried), never a silently truncated prompt. Non-story classification keeps its story default on this error but logs a warning naming the target. An attribution-escalation refusal is caught at the escalation call site and skips that window with a warning (the escalation contract in `analyzer/index.ts`); it does not fail the phase-1 chapter. Only a switch to a smaller-context target can trigger this, and #3084's configurable fallback introduces that case, so nothing regresses and a visible failure beats corrupted analysis. `onFallback` names both the primary and the target. If the target is also unreachable, the run fails naming both. | |
| | **Announced behaviour changes (3d):** "never silent" is now literal. On `80be2f1d` five `FallbackAnalyzer` methods fall back without calling `onFallback` (`runStage1`, `runEmotionChapter`, `runStage3Chapter`, `runAttributionEscalation`, `runNonStoryClassification`); from 3d all eight announce, through one private `switchTo`. The switch note renders the server's `fallbackReason` (`fallbackReasonFor`), which names both: `Ollama unreachable (<model>) — switched to <target>`, or `Analyzer endpoint unreachable (<endpoint name> · <model>) — switched to <target>`, replacing the note `Switched to Gemini — Ollama unreachable`. Script review's warm-fail copy "Or turn on Cloud fallback in Settings → analyzer to use Gemini when the local analyzer is unavailable." becomes "Or choose an analyzer fallback in Advanced Settings → Analyzer fallback." Each change is announced in the release notes, which also say Gemini is the default fallback for endpoint users, and (3d) that a fallback to a smaller model splits or clearly fails oversized requests instead of truncating them. | |
...
| | **Unchanged:** persona generation never falls back (`registry.ts:1175`). Before 3d, today's Ollama → Gemini rule under `allowCloudFallback` (`index.ts:215`) is untouched. | |
| | **Privacy help (3d):** in `src/data/help-topics.ts`, `is-my-data-private` (`:331-342`; its Gemini sentence `:338-341`) says only the optional Gemini analyzer can send chapter text off the machine. 3d rewrites it, `does-it-work-offline` (`:344-355`) and `picked-local-but-ran-on-gemini` (`:319-329`), so the copy names every path by which text can leave the machine: Gemini analysis; a remote analyzer endpoint; a fallback target once it activates; Gemini TTS when chosen (`server/src/tts/gemini.ts`); persona generation, which has its own engine (`analyzer.personaGeneration.engine`, `registry.ts:1171`, default `gemini`) and sends dialogue quotes; and cloud escalation (`analyzer.structure.escalation: 'cloud'`, `registry.ts:1325`). The offline topic keeps main's persona sentence (`:352-354`) and never re-adds "Voice design follows the same analyzer engine". `picked-local-but-ran-on-gemini`'s "turn off Cloud fallback in analyzer settings" points at Advanced Settings → Analyzer fallback instead, with a guard or test case. No single keep-everything-local switch is added; that stays out of scope, and the owner is told. | |
...
| | **Wiki link entry:** the wiki link is its own fix. `reasoningOverflowFixes` builds two arrays, `fixes` (actionable) and `reads` (`{ label: 'Read: <section title>', wikiPage }` entries), and returns `[...fixes, ...reads]`. Every `Read:` entry comes after every actionable fix, a list may carry more than one, and later waves push into the right array. No actionable fix carries `wikiPage`. `wikiPage` is a page name, never an `#anchor`: `src/lib/wiki-links.ts` is page-level only, and the frontend builds `WIKI_BASE/<page>`. Gemini and Ollama (2b): `{ label: 'Read: When a model thinks past its output limit', wikiPage: 'Analysis-and-the-Analyzer' }`, whose section lands in 2b. Endpoints (3d): the same label with `wikiPage: 'OpenAI-Compatible-Analyzer-Endpoints'` (P33). When its payload fix fires, 5b also pushes `{ label: 'Read: Custom payload', wikiPage: 'OpenAI-Compatible-Analyzer-Endpoints' }` into `reads`. The guard checks that each page file exists. | |
...
export class AnalyzerReasoningOverflowError extends Error {                    // 2b
  ...
  // Thrown by two runner sites, never by a transport: mapFinish, called from StageRunner.send as mapFinish(withThinkEvidence(result), { … }), and runFreeText's inline throw on the persona path (W4). 2b: the first three params.
  // opts.endpointId: 3b, from OpenAIAnalyzer (never OpenAITransport), on both exits: a thrown error via withEndpointId, and an escalation overflow via the StageCall.onReasoningOverflow hook OpenAIAnalyzer wraps. opts.reasoningLevel: 5a, added to the same object (never positional), from the level StageRunner resolved for that request; both throw sites pass it. Both feed reasoningOverflowFixes.
}
...
export interface AnalysisFailureFix {
  ...
  wikiPage?: string;                                      // a wiki page name, never an #anchor (src/lib/wiki-links.ts is page-level only). Set only on { label: 'Read: <section title>', wikiPage } entries, which reasoningOverflowFixes collects in `reads` and returns after every actionable fix ([...fixes, ...reads]); a list may carry several. No actionable fix carries it. The frontend builds WIKI_BASE/<page>; the guard checks the page file exists
...
export function reasoningOverflowFixes(ctx: { transport: TransportKind; model: string; endpointId?: string }): AnalysisFailureFix[];   // builds `fixes` (actionable) and `reads` (Read: entries), returns [...fixes, ...reads]; later waves push into the right array
...
export class StageRunner {
  ...
  runFreeText(input: { system?: string; prompt: string; signal?: AbortSignal; temperature?: number; ollama?: FreeTextOptions }): Promise<string>;   // 4; throws AnalyzerReasoningOverflowError inline on a length stop with reasoning evidence (the second throw site beside mapFinish); 5a passes reasoningLevel
}
...
- **`openai-transport.ts` (3b):** `OpenAITransport({ endpoint, apiKey, model, dispatcher?, now? })`.
  ...
  - **Settings load (P25):** `dropInvalidEndpointEntries` runs inside `performUserSettingsRead`, between the legacy migrations (`user-settings.ts:570-571`, #3192's `migrateLegacyAnalyzerModelFields` included; its write-back `.catch` closes at `:578`) and the whole-object `safeParse` (`:579`). The cold read's single flight is #3195's shipped `inFlightRead`, not new code. Every drop is listed through `listDroppedEndpointEntries` until acknowledged (P31), and endpoint writes validate first, refusing a malformed entry with 400 `{ error, code, issues }` (P25).
...
export async function listDroppedEndpointEntries(): Promise<DroppedEndpointEntrySummary[]>;       // 3b (P31): archived entries whose contentHash is not in the acknowledged sidecar, plus pending ones → GET `droppedEndpointEntries` (a sync twin, listDroppedEndpointEntriesSync, serves envDerived)
export async function acknowledgeDroppedEndpointEntries(archiveIds: string[]): Promise<void>;   // 3b (P31): resolves each id to its archive record's contentHash and adds that hash to the acknowledged sidecar; a null or unknown id is ignored
...
- **`droppedEndpointEntries`** (3b, P31): `DroppedEndpointEntrySummary[]`, read-only on GET, in `FORBIDDEN_KEYS`; an archived entry is cleared through `acknowledgeDroppedEndpointEntries`, which writes the record's `contentHash` to the acknowledged sidecar, and a pending entry (`archiveId: null`) stays listed until archived.
...
```ts
// server/src/analyzer/reasoning-level.ts (5a) — a leaf that imports nothing
export const REASONING_LEVELS = ['model-default', 'off', 'on', 'none', 'minimal', 'low', 'medium', 'high'] as const;
export type ReasoningLevel = (typeof REASONING_LEVELS)[number];
// errors.ts imports the ReasoningLevel type from this leaf, never from reasoning.ts: reasoning.ts imports errors.ts, and madge counts `import type`.
// server/src/analyzer/reasoning.ts (5a) — re-exports ReasoningLevel and REASONING_LEVELS from reasoning-level.ts
...
|---|---|---|---|
| 1a | characterisation tests, errors + types leaf, helper moves breaking the ollama↔gemini cycle | `refactor/server-3084-w1a-characterise` | wave 0 merged — **satisfied 2026-09-13** (PR #3192 at `80be2f1d`) |
...
| 3c / 3d | live structured output (Test action vs a real chapter); long silent prefill; same-card eviction; context overrun after a fallback switch to a smaller-context Ollama or llama.cpp target (context shift, stop or error; the pre-send guard leaves room to answer) |
