...
- **Wave 0 gates everything.**
  ...
  - **Wave 1's first task re-derives every citation against post-#3192 `main`** (`80be2f1d` or later), locating each construct by symbol. The plans stay pinned to `46e62a34`.
...
- **Import cycles.** New analyzer modules import `StageCall` and other analyzer types from the leaf `server/src/analyzer/types.ts` (wave 1), never from `server/src/analyzer/index.ts`. Madge counts type-only imports as cycle edges (`npm run check:cycles`).
  - **The baseline is a committed list, and it moves.** `server/madge-cycles-allowlist.json` is checked as a list of cycles, not a count. PR #3192 (merged 2026-09-13, `80be2f1d`) **added one** (`config/resolver.ts` ↔ `workspace/user-settings.ts`), so re-read the file on current `main` rather than assuming this plan's picture of it. Every leaf this plan introduces to dodge a cycle — `transports/allowlisted-fetch.ts`, `known-secrets-gate.ts`, `reasoning-level.ts` (5a), `resolveEndpointApiKey`'s structural parameter, and validating request controls in the PUT route instead of `writeUserSettings` — stays required: a new cycle is a deliberate allowlist row with a reason, never a silent edge.
