...
| | **Privacy help (3d):** the "Is my data private?" topic (`src/data/help-topics.ts:337-340`) says only the optional Gemini analyzer can send chapter text off the machine. 3d updates it, because a remote endpoint, and a remote fallback target, also do. | |
...
export class AnalyzerReasoningOverflowError extends Error {                    // 2b
  ...
  // 3b: gains endpointId?: string, set by OpenAITransport; 5a: gains reasoningLevel?: ReasoningLevel, the level the request sent. Both feed reasoningOverflowFixes.
}
...
export interface AnalysisFailureFix {
  ...
  wikiPage?: string;                                      // a wiki page name, never an #anchor (src/lib/wiki-links.ts is page-level only): the label names the section, the frontend builds WIKI_BASE/<page>, the guard checks the page file exists
...
export const GEMINI_REASONING_TABLE: ReadonlyArray<{ match: RegExp; control: 'thinkingLevel' | 'gemmaOnOff'; levels: ReasoningLevel[]; defaultLevel?: ReasoningLevel }>;   // P9 retired: 2.5 and unknown ids match no row → model default only. defaultLevel = the documented default; it gates overflow fixes only, never the wire
