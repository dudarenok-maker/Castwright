...
- **Windows safety (spec §7):** before unlinking the `.pt`, call the sidecar's `POST /qwen/evict-voice` and use rm-then-rename — copy the exact precedent at `qwen-voice.ts:664-678` ("rm-then-rename so a Windows rename … can't EPERM" + evict). A loaded `.pt` on Windows otherwise throws a sharing violation mid-erasure.
- Consumes: Tasks 3-4.
...
- Modify: `server/src/workspace/library-cast-scan.ts` (`LibraryCastCharacter`, ~:13-24 — type `overrideTtsVoices` with the widened slot so Task 8 can read `provenance`)
...
**Files:**
- Modify: `server/src/routes/voice-library.ts`; the cast-write path used by `PUT /api/voices/:voiceId/override` (`routes/voices.ts:527` — `applyOverrideToCastFiles` at ~:686 spreads the slot, so extra fields are preserved on write) is the pattern to copy for writing into `cast.json`
- Test: `server/src/routes/voice-library.test.ts`
...
**Files:**
- Modify: `server/src/routes/qwen-voice.ts` — the scope-agnostic core **already mostly exists**: `designQwenVoiceForCharacter` (~:316-471) + the fully-decoupled `postDesignAndCache` (~:331-409). The design handler itself is at ~:473-619 (NOT :187-260 — that's `ensureCharacterVoiceUuid`). The task is to generalise `designQwenVoiceForCharacter`'s character-coupled inputs (uuid source, sample text) behind parameters, moving the shared core to the new module. Note: the sidecar persists the `.pt`/`.json`, not Node — the core's job is request-shaping + preview-cache warming.
...
**Interfaces:**
- Produces:
  - `runVoiceDesign(opts: { storageKey: string; displayName: string; persona: string; languageCode?: string; preview?: boolean }): Promise<{ storageKey: string; previewUrl?: string }>` — scope-agnostic core; `preview: true` designs under `<storageKey>-preview` (the plan-161 pattern generalised) and ALWAYS warms the sample cache + returns the audition `url` exactly as the character flow does (`qwen-voice.ts:680-698` precedent) — this `previewUrl` is what the A/B modal plays (Task 15).
  ...
  - **`baseModel` source:** reuse the exact value/symbol `postDesignAndCache` already stamps into the sidecar-written `.json` (locate the `baseModel` field it records; same source feeds Task 4's staleness comparison — export a `currentQwenBaseModel()` helper from the core so both tasks name one symbol).
  ...
  - `POST /:voiceUuid/redesign` `{ persona }` → `runVoiceDesign({ …, preview: true })` → `{ previewUrl }`; `/redesign/promote` replaces the live `.pt` with the preview one using **evict + rm-then-rename** (`/qwen/evict-voice` + the `qwen-voice.ts:664-678` Windows-safe idiom), updates `persona` + `updatedAt`, purges cached samples (`purgeVoiceSamples`, Task 5); `/redesign/discard` evicts + deletes preview artifacts.
...
- Modify: `server/src/routes/voices.ts` (~:297-298) — **`aggregateVoices` rebuilds each override as a fresh `{ engine, name }` and silently STRIPS the new fields**; carry `libraryUuid` + `provenance` through into the derived `Voice` payload (Task 1 already added them to the openapi Voice slot) or every In-use badge renders blank
