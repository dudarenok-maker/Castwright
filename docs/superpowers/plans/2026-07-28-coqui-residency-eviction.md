...
- **`_synth_lock` is a non-reentrant `threading.Lock`.** Any method that already holds it MUST call `_unload_locked()`, never `unload()`. Violating this self-deadlocks the sidecar.
- **The TTL default must match in two places**, per the `srv-47 R2-A` invariant already pinned at `main.py:5179-5180`: `_COQUI_IDLE_TTL_DEFAULT` in `main.py` and `default:` on `sidecar.coquiIdleTtl` in `server/src/config/registry.ts`. Value: **30**.
- **Coqui-specific:** `maybe_free_idle` must route through `_unload_locked()` and must NOT null `self._tts` inline. Inline nulling skips `self._device = self._requested_device` (the #1730 gap-3 restore) and re-introduces a shipped bug. This is the single most important rule in this plan.
...
**Interfaces:**
- Consumes: nothing.
- Produces: `CoquiEngine._synth_lock: threading.Lock`, `CoquiEngine._synth_in_flight: int`, `CoquiEngine._last_used: float`, `CoquiEngine._unload_locked() -> None` (caller must hold `_synth_lock`). Task 2 consumes all four.
...
class _FakeTts:
    """Stands in for the loaded XTTS model. `tts()` blocks on an event so a
    test can hold a forward open and race unload() against it."""
    ...
    def tts(self, text, speaker, language):
        if self.block is not None:
            # Signal we're inside the forward, then wait to be released.
...
def _loaded_coqui(monkeypatch, fake_tts):
    ...
    neutered so no real XTTS is pulled."""
...
def test_unload_waits_for_an_in_flight_synth(monkeypatch):
    ...
    eng = _loaded_coqui(monkeypatch, _FakeTts(block=release))
    ...
    t.start()
    # Give the synth thread time to enter the forward and hold the lock.
    time.sleep(0.2)
...
Expected: `test_unload_waits_for_an_in_flight_synth` FAILS on the `assert not unloaded.wait(...)` line ("unload() did not wait for the synth") — because nothing serializes them today. `test_synthesize_tracks_in_flight_and_last_used` FAILS with `AttributeError: 'CoquiEngine' object has no attribute '_synth_in_flight'`.
...
- [ ] **Step 4: Split the teardown out of `unload()`**

Replace the `def unload(self) -> None:` header and its first line, keeping the entire existing body under the new private name:
...
```python
    def unload(self) -> None:
        ...
        Internal holders call `_unload_locked()` directly.
        """
        with self._synth_lock:
            self._unload_locked()

    def _unload_locked(self) -> None:
        """Body of `unload()`. CALLER MUST HOLD `_synth_lock`.

        Split out (#1894) so `maybe_free_idle` can reuse the FULL teardown
        under the lock it already holds, rather than nulling `_tts` inline the
        way QwenEngine's `maybe_free_idle_design` does. Qwen can null inline
        because `_design` carries no paired state; Coqui's teardown also
        restores `self._device = self._requested_device` (the #1730 gap-3
        fix), and skipping that would leave the next lazy /synthesize
        cold-loading onto the last admitted card, bypassing placement.
        """
        if self._tts is None:
            return
```

Everything from `torch_module = self._torch` (the old `:1334`) to the closing `log.info("Coqui model unloaded.")` stays exactly as it is, now inside `_unload_locked`.
...
```python
    ...
    def _synthesize_locked(
        ...
        Binds `self._tts` to a local up front: re-reading the attribute mid-
        forward is what made a racing unload() a crash rather than a wait.
        """
        tts = self._tts
...
**Interfaces:**
- Consumes: `_synth_lock`, `_synth_in_flight`, `_last_used`, `_unload_locked()` (Task 1).
...
Expected: 5 FAIL with `AttributeError: 'CoquiEngine' object has no attribute 'maybe_free_idle'`.

- [ ] **Step 3: Implement `maybe_free_idle`**

Insert directly after `_unload_locked` in `CoquiEngine`:
...
```python
    def maybe_free_idle(self, ttl_seconds: float) -> bool:
        ...
        it calls `_unload_locked()` rather than nulling `_tts` inline,
        ...
        if time.monotonic() - self._last_used <= ttl_seconds:
            return False
        # Re-validate under the lock: `synthesize` refreshes `_last_used` and
        # runs its forward while holding `_synth_lock`, so a check that still
...
Run: `server/tts-sidecar/.venv/Scripts/python.exe -m pytest server/tts-sidecar/tests/test_devices.py server/tts-sidecar/tests/test_placement.py -v`
Expected: the four new `test_devices.py` cases FAIL (`AttributeError: module 'tts_sidecar.main' has no attribute '_coqui_idle_ttl'`, and no Coqui branch so `freed == 0`); `test_starved_qwen_admits_after_coqui_is_evicted` PASSES already (it injects its own `evict`, so it is a contract test for the signature, not for the branch).
...
In `_idle_evict`, after the `SPK` block and before `return freed`:
...
In `docs/wiki/Advanced-Settings.md`, §9 "GPU arbitration, memory & lifecycle" (starts at `:260`), add a row to that section's knob table:

```
| Coqui (XTTS) idle TTL (s) | Seconds of Coqui inactivity before a VRAM-starved operation may reclaim the resident XTTS model (~3 GB). No background watchdog — fires only when another operation would otherwise fail. Values below 5 fall back to the default. | 30 | integer, min 0 | restart sidecar | high |
```

Match the column order of the neighbouring rows in that section — if §9's table differs from the `| Knob | What it does | Default | Range | Apply | Risk |` shape used elsewhere, follow §9's.
...
The header currently justifies excluding a resident Qwen base. Extend that reasoning to Coqui by replacing the sentence beginning "A resident Qwen base is excluded on purpose" with:
...
```
   ...
   taken. Coqui is excluded for the same reason since #1894 — the sidecar's
   admission path reclaims an idle XTTS before it ever reports noCapacity.
...
Expected: all pass. `capacity-retry.test.ts` is included because it exercises the `NoCapacityError` message path; if it asserts on a Coqui blocker string, update that assertion too.
...
In `docs/testing/onbox-acceptance-register.md`, insert **A20** directly after A19's block (i.e. after the `---` at `:315`, before `## Group B`), and update the Group A count in the `## At a glance` section at `:46` from 19 to 20. Match A19's shape exactly:
...
Append to `docs/release-notes-next.md` under its GPU/performance section:
...
PR body must contain `Closes #1894`, a `## Summary` and a `## Test plan` section, and a link to the spec.
