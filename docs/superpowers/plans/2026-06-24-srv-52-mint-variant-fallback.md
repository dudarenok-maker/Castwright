...
> its preview: …"`, (b) the `console.log('[qwen-voice] … cached …')` line
> (`qwen-voice.ts:427-430`), and (c) the `maybeSampleSidecarEngine('qwen:design')`
> telemetry call — all exist in the current inline block (`qwen-voice.ts:418-433`)
> and an existing test asserts on `maybeSampleSidecarEngineMock`
> (`qwen-voice.test.ts:226`). The single `releaseGpu()` stays in the OUTER
> `finally`, never in this helper.
...
it('includes a fallback count in the completion toast', () => {
  ...
  // Read toasts off real store state (the pattern used elsewhere in this file):
...
- [ ] **Step 3a: Extend the local `CastDesignRootState` type (REQUIRED — else `tsc` fails)**

The inline `CastDesignRootState` at `cast-design-stream-middleware.ts:39-41` types
`active` as `{ bookId: string; state: string; kind?: string } | null` — it has NO
`fallbacks`, so reading `.active.fallbacks` is a compile error that the pre-commit
gate blocks. Add the field there first:

```ts
  castDesign: { active: { bookId: string; state: string; kind?: string; fallbacks?: { characterId: string; emotion: string }[] } | null };
```

- [ ] **Step 3b: Add the toast part**
...
> Read `fellBack` BEFORE `settle` (settle doesn't touch `fallbacks`, but reading first is robust). The `.active.fallbacks` access only typechecks because Step 3a added `fallbacks?` to the inline `CastDesignRootState` — do Step 3a first.
