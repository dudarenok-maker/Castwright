...
- [ ] **Step 1a — visual baselines: NO pill regen needed (verified 2026-06-22).** The new "Qwen 1.7B" pill is **hidden in e2e mock mode** (no real sidecar → `qwen_base17_loaded` false), so it appears in NO snapshot; chromium `test:e2e:visual` passed 19/19, and the local battery's 2 visual fails were diff-confirmed UNRELATED to the pill: `analysing` (mobile) = whole-page font-hinting/offset jitter (known Windows flake, clears on Linux CI); `library` (tablet) = book-cover-thumbnail rendering nondeterminism with the top-bar (pill region) MATCHED. Both are **pre-existing, not from this branch** — do NOT regen baselines for this PR on their account. Confirm they're green (or pre-existing-red) on the Ubuntu CI run; if `library` is red on Linux too, file it as a separate pre-existing stale-baseline item (not part of fs-55).
