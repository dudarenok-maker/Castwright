...
10. **Eliminate the variable-input-shape host-memory leak** ([`side-11`](BACKLOG.md) — promoted to **Should** 2026-05-31) — **← the next thing to play with for end-to-end runs.** NOT an RTF lever: it kills the periodic process-recycle (plan 143) that interrupts long books and drops the in-flight chapter (`srv-17c`). Try `torch.backends.mkldnn.enabled=False`, fixed-shape batch padding (re-tune plan 128 bucketing), or an upstream `qwen_tts` fix; repro with a variable-length bench loop watching `/debug/memory` (RSS climbs while CUDA stays flat = this leak).
