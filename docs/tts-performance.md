...
| Var | Default | Effect |
|---|---|---|
| `QWEN_BATCH_SIZE` | `4` | Qwen sentences packed into one batched `generate_voice_clone` forward (plan 113). Larger = more throughput but more VRAM; **`1` disables batching** (the per-call kill-switch). Qwen-only — Coqui/Kokoro/Gemini are always one-per-call. When `QWEN_BATCH_TOKEN_BUDGET` is set, this becomes the **hard width cap**. |
| `QWEN_BATCH_TOKEN_BUDGET` | `0` (off) | Token-budget packing (plan 136): when `>0`, batch width is variable — the packer fills each batch while `width × maxLen ≤ budget` (units = normalised chars) and `width ≤ QWEN_BATCH_SIZE`, so short/dialogue batches pack wide (toward the cap) and long sentences stay narrow. `width ≈ budget/maxLen`, so `2400/100 ≈ 24` on typical prose. `0`/unset = exact fixed-width `QWEN_BATCH_SIZE` slicing (kill-switch). Reads straight from `process.env` (same gotcha as `QWEN_BATCH_SIZE`). Output-preserving. |
...
1. **Length-bucketing batches** (plan 128) — group similar-length sentences before each `/synthesize-batch` so a batch decodes to a tighter max-length. A batch decodes to its LONGEST item, so a mixed-length batch wastes compute padding the short ones; bucketing cuts that. **MECHANISM SHIPPED 2026-05-29** (`synthesise-chapter.ts` sort, `QWEN_BATCH_BUCKET` default ON, output-preserving). ~10–30% expected on high-variance chapters; **TODO: record the A/B row** — `bench-tts.py --engine qwen --voice <id> --batch 16 --bucket 0` vs `--bucket 1`, then flip plan 128 to `stable`.
