...
> **Benchmarking gotchas (cost hours once):** (1) `getResolvedSidecarUrl()` (`server/src/workspace/user-settings.ts`) resolves, in order: an explicitly-set, non-factory-default `sidecarUrl` in **`user-settings.json`**; else a non-default `LOCAL_TTS_URL` env value; else `http://localhost:LOCAL_TTS_PORT` (default `9000`) — so a **factory-default** `sidecarUrl` in the settings file (the common case, since onboarding writes it) does NOT shadow `LOCAL_TTS_URL`, and setting `LOCAL_TTS_URL` to a genuinely different, private-host URL **does** point the full pipeline's *outbound requests* at a custom/fixed sidecar without touching the settings file (#2632). It only fails to take effect if `user-settings.json` also carries an explicit, non-default `sidecarUrl` — check that first if a `LOCAL_TTS_URL` override doesn't seem to land. **This does NOT stop this checkout from also spawning and supervising its own sidecar on `LOCAL_TTS_PORT`** — `spawnSidecar` takes its port from `LOCAL_TTS_PORT` only, never from `LOCAL_TTS_URL` (`spawn-sidecar.ts`), so setting `LOCAL_TTS_URL` alone still loads a second model on this box and walks straight into gotcha (3) below. To point at another sidecar WITHOUT spawning a second one, either set `LOCAL_TTS_PORT` to that sidecar's port (so this checkout's own probe adopts it instead of spawning) or turn `autoStartSidecar` off in Account settings first. (2) A shell-exported value always wins over `server/.env` for **every** server knob, `QWEN_BATCH_SIZE` included — `process.loadEnvFile` (`server/src/load-env.ts`) never overwrites an already-present `process.env` entry, so this isn't a `QWEN_BATCH_SIZE`-specific quirk to watch for; it's the same precedence every `.env`-sourced knob gets (measured directly, #2632 N45). (3) VRAM contention silently inflates RTF 3–5× — free the GPU first (the original 4.12 figure was a contention artifact).
...
**Preload — cold-start vs. resident VRAM:** `PRELOAD_KOKORO` (off, fs-60; warms on demand on first synth — opt in for ~1 GB eager), `PRELOAD_QWEN` (off; warms on first `/load`/synth), `PRELOAD_COQUI` / `PRELOAD_COQUI_MODEL` (off / `xtts_v2`).
...
11. ~~**Move the Code2Wav codec decode to GPU**~~ ([`side-25`](BACKLOG.md), [#1374](https://github.com/dudarenok-maker/Castwright/issues/1374), `docs/features/archive/243-side25-qwen-codec-gpu.md`) — **SHIPPED inert-by-default 2026-07-06; on-box acceptance on the 8 GB RTX 4070 Laptop FAILED at production batch size, reverted.** `codec=cpu` baseline: RTF **0.63** (median), VRAM peak **5234 MB**, at `QWEN_BATCH_SIZE=32`. `codec=auto` + `QWEN_CODEC_CHUNK_SIZE=100`: VRAM peak **7934 MB** dedicated + **~10.9 GB** spilled into the Windows shared-memory GPU fallback, then the CUDA context fully poisoned (0/2 batch calls succeeded). A genuine 8 GB capacity limit at this batch size, not a code defect — matches the risk the design spec's Out of scope section flagged in advance. Don't flip `QWEN_CODEC_DEVICE` on an 8 GB card at batch=32 without a much smaller batch size or progress on the deferred batch-size-cap/bf16/sub-batch-grouping follow-ups ([#1396](https://github.com/dudarenok-maker/Castwright/issues/1396)). See `docs/wiki/Troubleshooting.md`'s "GPU out of memory (VRAM)" section.
