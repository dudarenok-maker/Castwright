...
- **Open: eliminate the leak itself** (so recycling isn't needed) — `side-11` / [plan 153](features/153-sidecar-variable-shape-host-leak.md). Candidate 1 LANDED (default OFF, awaiting a live A/B): env-gated `SIDECAR_DISABLE_MKLDNN=1` sets `torch.backends.mkldnn.enabled=False` to kill the suspected CPU per-shape workspace. Instrument LANDED: `bench-tts.py --mem-sample` drives a seeded variable-shape batch loop, samples `/debug/memory` (now exposing `committed_mb`) after each batch, and prints the committed-private slope — reboot, run flag-OFF then flag-ON, PASS iff the ON slope ≈ flat. If MKLDNN isn't the lever, fall to fixed-shape batch padding; `PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True` already shipped (plan 144) but targets the *separate* VRAM-frag OOM, not this host leak.
...
10. **Eliminate the variable-input-shape host-memory leak** ([`side-11`](BACKLOG.md) — promoted to **Should** 2026-05-31; [plan 153](features/153-sidecar-variable-shape-host-leak.md)) — **← the next thing to play with for end-to-end runs.** NOT an RTF lever: it kills the periodic process-recycle (plan 143) that interrupts long books and drops the in-flight chapter (`srv-17c`). **Candidate 1 landed** (env-gated `SIDECAR_DISABLE_MKLDNN`, default OFF) **+ the leak-slope instrument** (`bench-tts.py --mem-sample`, samples `/debug/memory`'s new `committed_mb`). **Next: the live A/B** — reboot, run the bench flag-OFF then flag-ON, PASS iff the committed slope flattens; then flip the default ON. If MKLDNN isn't the lever, fall to fixed-shape batch padding (re-tune plan 128 bucketing + a python-side pad).
