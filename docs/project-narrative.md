...
_A project narrative — updated 19 May 2026_
...
The shape of the project today, on the day v1.3.0 cuts: **a working local audiobook pipeline, end-to-end, on the machine I own — packaged.** Tagged release zip, cross-OS verify, one-click install path. Ingest a manuscript, analyse it on a local LLM, cast it, synthesise it on a local GPU, listen to it, hand the chapters off to whichever audiobook app the listener already uses — and now also share a 30-second clip, mint a streaming-link URL, keep editorial notes per book, audition voices without committing, run two tabs in lockstep across the same workspace. Every stage works on a real book — a friend's novel, about 80k words, twenty-something characters, the canonical regression test I use because the failure modes are the interesting ones. Not every stage is _good_ yet. But every stage works, and a clean Windows / macOS / Linux box can install and run the whole thing from a release zip without me on the other end of a Slack DM.
...
**An 8 GB GPU that holds either the analyzer or the synthesiser, but not both — and a default that eats none of that budget.** Kokoro v1 became the default TTS engine in May, eager-loaded inside the sidecar at ~1 GB VRAM and ~1 s cold-start, sitting permanently resident alongside the analyzer Ollama on an 8 GB card. Coqui XTTS v2 stays button-driven for users who want zero-shot voice cloning: a Load / Stop pill in the application's top bar loads it on demand and evicts the analyzer, with an inline banner so you know what happened. The previous Load / Stop story still exists for that path; what changed is that the _default_ path now spends nothing on lifecycle UX because nothing needs unloading. Both sides expose load/unload as first-class HTTP, so the UI doesn't have to know which side of the GPU is hot. The mid-market-hardware claim now holds without the user having to think about it.
...
**What v1.3.0 actually added on top.** The release that's cutting today is the biggest single step the project has taken since the analysis pipeline was rebuilt in phases. Listen-surface overhaul: speed picker (0.75×–2×, persisted per book), user-placed markers (note / rerecord), sleep timer with end-of-chapter mode, true RMS waveform peaks computed at chapter encode, listening-progress resume bookmarks, collapsible per-book editorial notes, a one-click button that slices a 30-second clip of any chapter as MP3 to share, and a slugged share URL that mints a single-link download for the whole book M4B. Voice-side: a per-candidate Play sample affordance inside the profile drawer so you can audition multiple voices without committing, plus same-book Compare lifted across to the global Voices tab. Cast-side: revision history timeline as a read-only log of accept/reject events per chapter. Cross-tab: the analysis and generation pills now sync via `BroadcastChannel`, so two tabs on the same workspace stay live without re-hitting the catch-up endpoint. Theme: dark mode locked stable after a three-pass contrast bug bundle. Onboarding: the deployer's first install is now end-to-end inside the app — Account → Models → Install Ollama → Pull qwen3.5:4b → analyze, no terminal. And the structural decomposition of the listen view (1136 → 319 lines, behaviour-neutral) makes the next round of feature work land in three different files instead of fighting for the same orchestrator.

**What's _not_ good enough yet to put in front of someone else.** Voice differentiation across twenty-plus characters in a single book is still the open question. The model produces a distinct voice for any one character on demand; whether the cast collectively reads as twenty distinct people, or as five flavours of four, is something I can't measure from inside the application. Drift-detection cutoffs are still placeholders pending a labelled set. The qwen3.5:4b malformed-JSON rate is acceptable after the schema-format + divergent-retry + quote-escape-repair pass, but I haven't measured it cleanly; the parked experiment to swap to qwen3.5:9b once the KV-cache math says it fits resident in 8 GB at 16K context still hasn't run. XTTS v2 and Kokoro v1 are the running TTS engines; I haven't evaluated F5-TTS or OpenVoice v2 head-to-head on this manuscript.
