...
| P-21 | Analyzer engine setting | Admin → Model Manager → Server configuration → Analyzer engine (or `~/.castwright/user-settings.json`). Record it; the analyzer competes for VRAM. | |
| P-22 | TTS engine setting (persisted account default) | Admin → Model Manager → Defaults for new books → Voice model. Record the **model key** (e.g. `qwen3-tts-0.6b`). | |
...
| **KL-j(1)** | ~~After a revoke that raced an in-flight derive, `consent.revokedAt` is missing~~ — should no longer reproduce. | **Fixed in source (Wave 3c, Task 14) and proven by the #1826 chain's Step 1 regression tests (`server/src/tts/clone-voice-resolver.revoke-race.test.ts`) — verify the file is present on the SHA under test.** `workspace/voice-library.ts`'s `updateEntry(uuid, mutate)` now holds a per-uuid promise-chain lock (`withEntryLock`) across the ENTIRE fresh-read + mutate + write span, not just the write, and every read-modify-write call site — the revoke route's `revokedAt` stamp, the resolver's post-derive `statusStampMutate`, PATCH, redesign/promote, and `purgeCloneArtifacts`'s master-clip clear — now routes through it. The commit (`74bff8b3`) explicitly states this closes "the millisecond window the resolver's own doc comment flagged as a known follow-up" — i.e. exactly this row. Cross-process locking (two separate server processes sharing one workspace) is still explicitly out of scope, same carve-out as before, but that's not this sheet's D-01 scenario (one server process, two concurrent chapter workers). If C-01 still reproduces a clobbered `revokedAt` on your SHA, that is now a **regression**, not expected — see C-01's own updated note. | [#1826](https://github.com/dudarenok-maker/Castwright/issues/1826) | ☐ |
| **KL-j(2)** | ~~The two-worker corner: B's `.pt` survives a revoke that raced worker A's derive.~~ — should no longer reproduce, for the same reason as KL-j(1). | Same per-uuid `updateEntry` lock (Task 14) serialises worker A's and worker B's status-stamp writes for the same voiceUuid — proven by the #1826 chain's Step 1 regression tests (`server/src/tts/clone-voice-resolver.revoke-race.test.ts`) — so whichever runs second re-reads the FIRST one's already-written state (including a landed `revokedAt`) before deciding whether to write or re-purge, so B can no longer observe a stale un-revoked snapshot. Watch for this in **D-01**'s repeat-with-repair step; a surviving `.pt` there is now a **regression**, not expected — same caveat as KL-j(1) about verifying the test file is on the SHA under test and about the cross-process carve-out. | [#1826](https://github.com/dudarenok-maker/Castwright/issues/1826) | ☐ |
