...
*Dated run record (2026-08-21). References to #2534 as a blocker in this document are from a prior state; that blocker has been resolved by PR #2576 (which re-pinned `ONNXRUNTIME_GPU_CONSTRAINT` to `>=1.26,<1.27`). The GPU-provider re-check was re-run against the fixed pin (wave-4 step 8, same procedure as A37/A38 rows) and still fails, but on a new, distinct root cause — the missing `nvidia-cudnn-cu12` dependency. The row stays STILL OWED on the GPU-provider check basis.*
...
*Log wording superseded by commit c556f51c; the phrase "GPU Kokoro is currently working" was later removed (the function cannot verify GPU Kokoro's actual status) and replaced with "The GPU build's files currently own the namespace" — see server/src/tts/ort-ensure-marker.test.ts:102-105 for the correction.*
