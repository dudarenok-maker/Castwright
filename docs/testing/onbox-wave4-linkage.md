...
> **Row IDs below are as-of wave 4 and have since renumbered.** Register IDs
> were positional at the time: PR #2626 discharged rows and shifted the
> survivors, so `A39`, `A40`, `A41`, `B3`, `B4` and `E11` in the table below no
> longer name the rows they named when this map was written. They are
> deliberately left as written — this is a dated snapshot of the wiring, and
> rewriting it would falsify the record. **Resolve any row here by its
> subject, not its number**, against the current register. (Under the
> pre-#2599 positional-ID rule; IDs are stable and never reused from
> 2026-08-27.)
