...
- [ ] A40: **must** use a bootstrapped copy of the sidecar venv, never
  ...
  flag it.
- [ ] A41: a copy of the venv, deliberately clobbered (`pip install
  --force-reinstall onnxruntime` over an existing `onnxruntime-gpu`) — row's
  own words permit a copy (L2272-2273); this step is destructive by design,
  never run it against the live venv.
