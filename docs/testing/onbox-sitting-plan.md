...
**Row count:** 4 + 9 + 7 + 9 + 10 + 4 + 7 + 1 = **51** (was 49 — see
2026-08-20 correction below).
...
A27, A29, A39, A40, A41, A42, B1, B2, B3, B4, C1, C2, C3, C4, E7, E8,
E11, G1, G2.

**Row count:** 6 (group A, excluding A16, A33, A43 — moved to §2.1
`onbox-sitting-cloning-identity.md` 2026-08-20) + 4 (B) + 4 (C) + 3
(E7/E8/E11) + 2 (G) = **19** (was 21 before the 2026-08-20 correction above).
...
**51** (operator) + **19** (wave-3) + 4 (blocked) = **74**. Every register row
appears exactly once. (Before the 2026-08-20 correction above, this read 49 +
21 + 4 — A33/A43 moving from the wave-3 set to the operator set changes the
first two terms by ∓2 each, leaving the total unchanged.)
---
...
blocked = 74** — see §7 for the current totals.
...
- **Total estimated operator minutes (runnable packs):** 110 + 155 + 155 + 155 +
  220 + 165 + 150 = **1,110 minutes (~18.5 hours)** (was 1,060 before the
  2026-08-20 correction added A33/A43's ≈50 minutes to the cloning-identity
  pack — see §2.1).
...
- **Wave-3 agent-runnable:** 19 rows (was 21 — A33/A43 moved to the operator
  set 2026-08-20, see §2.1/§2.2), no GPU, run off-box by the wave-3 pass —
  not counted in operator minutes.

**Grand reconciliation:** 51 operator + 19 wave-3 + 4 blocked = **74 rows**, the
