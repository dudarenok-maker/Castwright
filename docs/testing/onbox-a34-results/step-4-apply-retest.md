...
(`server/src/routes/analysis.ts:3803-3845`, `requestedFresh` branch): it
explicitly deletes four artifacts — `cast.json`, the reuse-carryover,
`manuscript-edits.json`, and clears the cast-merge/dedup-suggestions journal
— before re-analysing from nothing — "fresh run regenerates ids from
...
**This recovers only the two files the repair script touches** (`cast.json`
and `cast-id-history.json`). Of the other two artifacts the `fresh:true`
detour deleted: the cast-merge/dedup-suggestions journal's `fold`/`dedup`
entries ARE regenerated by attempt 6's own ordinary re-analysis (both
`writeFoldJournal`/`writeDedupJournal` replace-all their own entry kind on
every run) — only a *manual* journal entry, had one existed for this book
before the detour, would be genuinely lost, and none is recorded as having
existed. The reuse-carryover and `manuscript-edits.json` (any operator-typed
manuscript patches for this book) are NOT regenerated by an ordinary
re-analysis and are NOT restored by the repair script — that lineage is
genuinely gone from the real workspace's copy of *Заказ Коалфолла*, and no
operator-authored manuscript edits are recorded as having existed for it
either, but the loss itself is real and disclosed here rather than glossed
over. None of this has any bearing on this row's own criterion (the
`cast.json` id), which the repair script does fully restore.
