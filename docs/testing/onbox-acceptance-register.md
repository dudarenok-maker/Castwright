...
- Stop the manually-started sidecar process (e.g. Ctrl+C in the terminal running `python main.py`). Restore `SIDECAR_NEVER_ADOPT` to its prior state (unset, or `'0'`). Confirm the server's supervisor detects the sidecar has disappeared and respawns an owned sidecar, which surfaces as ready on the next render or health check.
