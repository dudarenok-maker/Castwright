...
- Note: the deadline timer (`LISTENER_PID_DEADLINE_MS = 5000`) tests the stale-replace code path (`spawn-sidecar.ts:694`), which requires an unfit-but-ours sidecar. A foreign listener triggers the not-ours refusal path instead (`spawn-sidecar.ts:681`) and never reaches the deadline code. The unit tests (`spawn-sidecar.test.ts`) fully verify the deadline wiring and the handler that logs `[sidecar] probe for the PID on :…timed out — …` when it fires; no additional on-box observation of the deadline timeout is owed here. With the foreign listener setup, confirm only that the refusal counter accretes monotonically (bullet 3) and the supervisor eventually surfaces as exhausted (bullet 4).
- Stop the foreign listener, confirm the sidecar respawns and surfaces as ready within one backoff window (~15s for the final backoff).
