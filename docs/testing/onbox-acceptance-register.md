...
`PUT /api/config` now rejects a `qa.asr.device` / `qa.asr.computeType` save
that would leave this pair mismatched, checked against the resulting
effective config (not just the incoming patch), so the UI can no longer
produce this state. A hand-edited `server/.env` still bypasses save-time
validation by design and can still reach this combination — that residue is
explicitly out of scope for #2180 (belongs with #2131's sidecar-side
surfacing work instead).
