...
- Record the listener-enumeration probe's actual wall-clock latency from the logs. On a contended box the probe may slow but must complete within the 5000ms deadline window. If the deadline does fire (rare without artificial contention), confirm it appears in logs as the handler completing the timeout and the next attempt starting fresh—not as a stuck probe blocking the backoff timer.
