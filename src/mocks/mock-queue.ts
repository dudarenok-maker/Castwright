...
function readInitial(): QueueFile {
  ...
  const entries = Array.isArray(g.__mockQueueInitial) ? g.__mockQueueInitial.map((e, i) => ({ ...e, order: i })) : [];
...
export function mockQueueRequest(path: string, init?: { method?: string; body?: string }): MockResponse {
  ...
  if (rest === '/enqueue') {
    ...
    for (const inp of incoming) {
      if (seen.has(inp.id)) return resp({ error: `queue.enqueue: duplicate entry id "${inp.id}"` }, 409);
    }
  ...
  if (rest === '/pause') {
    state.paused = init?.body ? Boolean((JSON.parse(init.body) as { paused?: boolean }).paused) : false;
    return snapshot();
  }
  ...
  if (method === 'DELETE') {
    ...
    if (target?.status === 'in_progress') {
      return resp({ error: `queue.cancel: entry "${id}" is in_progress; pause the queue first` }, 409);
    }
