...
describe('generationStreamMiddleware — enqueue-on-work (override replacement)', () => {
  it('auto-enqueues the viewed book’s non-excluded queued/in_progress chapters', async () => {
  ...
  it('does NOT auto-enqueue when the queue is globally paused', async () => {
    const { store } = makeStore();
    store.dispatch(uiSlice.actions.openBook({ id: 'b1', status: 'generating' }));
  ...
  it('does NOT auto-enqueue while a local analysis is alive on the same book (reverse-analyzer guard)', async () => {
    const { store } = makeStore();
    store.dispatch(uiSlice.actions.openBook({ id: 'b1', status: 'generating' }));
    store.dispatch(
  ...
  it('DOES auto-enqueue when the analysis is remote (gemini — no GPU contention)', async () => {
    const { store } = makeStore();
    store.dispatch(uiSlice.actions.openBook({ id: 'b1', status: 'generating' }));
    store.dispatch(
      analysisActions.setActiveStream({
...
describe('generationStreamMiddleware — halt + stubs', () => {
  ...
  it('enqueues a pending-revision stub on a character regen', () => {
    ...
    store.dispatch(
      chaptersSlice.actions.regenerateCharacter({ characterId: 'Marlow', chapterIds: [3] }),
    );
    const pending = store.getState().revisions.pending;
    expect(pending.some((p) => p.chapterId === 3 && p.characterId === 'Marlow')).toBe(true);
  });
});
