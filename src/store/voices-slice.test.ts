...
describe('voicesSlice — setOverride', () => {
  it('writes the override onto the matching voice', () => {
    const start = voicesSlice.reducer(undefined, voicesActions.hydrate({ voices: [voice('v_Brann'), voice('v_other')] }));
    const next = voicesSlice.reducer(start, voicesActions.setOverride({
      voiceId: 'v_Brann',
      override: { engine: 'coqui', name: 'Asya Anara' },
    }));
    expect(next.voices.find(v => v.id === 'v_Brann')!.overrideTtsVoice).toEqual({ engine: 'coqui', name: 'Asya Anara' });
  ...
  it('clears the override when given null', () => {
    const start = voicesSlice.reducer(undefined, voicesActions.hydrate({
      voices: [voice('v_Brann', { overrideTtsVoice: { engine: 'coqui', name: 'Asya Anara' } })],
    }));
    const next = voicesSlice.reducer(start, voicesActions.setOverride({ voiceId: 'v_Brann', override: null }));
