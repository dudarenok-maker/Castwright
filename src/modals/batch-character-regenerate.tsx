...
interface Props {
  ...
  chapters: Chapter[];
  onClose: () => void;
  onConfirm: (args: { characterIds: string[]; chapterIds: number[]; reason: string; note: string }) => void;
}

export function BatchCharacterRegenerateModal({ characterIds, characters, chapters, onClose, onConfirm }: Props) {
  const selectedChars = characterIds.map(id => characters.find(c => c.id === id)).filter(Boolean) as Character[];
