...
import {
  IconClose, IconArrowLeft, IconWifi, IconFolder, IconChevR, IconUpload,
  IconAirdrop, IconMobile, IconQrCode, IconShare, IconPlus, IconCheck,
  IconSignal, IconWifiBars, IconBattery,
} from '../lib/icons';
...
export function AppHandoffModal({ app, onClose, onComplete }: Props) {
  ...
  const next = () => {
    if (isLast) { onComplete?.(app); onClose(); }
    else setStep(step + 1);
  };
    ...
    <>
      ...
      <div className="fixed inset-0 z-50 grid place-items-center p-6 pointer-events-none">
        <div className="bg-white rounded-3xl shadow-float w-full max-w-2xl pointer-events-auto fade-in overflow-hidden">
          <div className="px-6 py-4 flex items-center gap-3 border-b border-ink/10"
          ...
          <div className="px-6 pt-5 flex items-center gap-2">
            {steps.map((_, i) => (
              <div key={i} className="flex-1 h-1 rounded-full bg-ink/[0.06] overflow-hidden">
                <div className={`h-full rounded-full transition-all ${i < step ? 'bg-ink' : i === step ? 'bg-gradient-progress' : 'bg-transparent'}`}
...
interface IllustrationProps { kind: string; app: ListenerApp; }
function HandoffIllustration({ kind, app }: IllustrationProps) {
    ...
    return (
      <div className="aspect-[2/1] rounded-2xl bg-canvas border border-ink/10 grid grid-cols-[1fr_auto_1fr] items-center gap-4 p-5">
        ...
        <div className="text-center">
          <span className="w-12 h-12 rounded-full grid place-items-center text-white mx-auto" style={{ background: grad }}><IconAirdrop className="w-6 h-6"/></span>
  ...
  if (kind === 'ios-share-sheet') return <PhoneFrame><IOSShareSheet app={app}/></PhoneFrame>;
  if (kind === 'android-files')   return <PhoneFrame platform="android"><AndroidFiles app={app}/></PhoneFrame>;
  return <div className="aspect-[2/1] rounded-2xl bg-canvas border border-ink/10"/>;
}

function PhoneFrame({ platform = 'ios', children }: { platform?: 'ios' | 'android'; children: ReactNode }) {
  return (
...
interface ShareItem { id: string; glyph: string; color?: string; gradient?: string; target?: boolean; }
        ...
        <div className="flex items-center gap-2 overflow-x-auto pt-1 border-t border-ink/5">
          {appRow.map(item => (
            <span key={item.id} className={`flex flex-col items-center gap-1 shrink-0 ${item.target ? 'relative' : ''}`}>
              <span className={`w-9 h-9 rounded-lg grid place-items-center text-white text-[10px] font-bold ${item.target ? 'ring-2 ring-offset-1 ring-peach' : ''}`}
                    style={{ background: item.gradient || item.color }}>{item.glyph}</span>
