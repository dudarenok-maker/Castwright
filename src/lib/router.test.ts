...
describe('stageToHash', () => {
  ...
  it('analysing with bookId → #/books/:id/analysing', () => {
    expect(stageToHash({ kind: 'analysing', bookId: 'ns', manuscriptId: null }))
      .toBe('#/books/ns/analysing');
  });

  it('analysing without bookId falls back to #/new', () => {
    expect(stageToHash({ kind: 'analysing', bookId: undefined, manuscriptId: null }))
      .toBe('#/new');
  });
  ...
  it('ready with default chapter=3 omits chapter query param', () => {
    const stage: Stage = {
      kind: 'ready', bookId: 'ns', view: 'manuscript',
      currentChapterId: 3, openProfileId: null,
    };
  ...
  it('ready with non-default chapter includes chapter query param', () => {
    const stage: Stage = {
      kind: 'ready', bookId: 'ns', view: 'cast',
      currentChapterId: 5, openProfileId: null,
    };
  ...
  it('ready with openProfileId includes profile query param', () => {
    const stage: Stage = {
      kind: 'ready', bookId: 'ns', view: 'cast',
      currentChapterId: 3, openProfileId: 'halloran',
    };
  ...
  it('ready with both chapter and profile combines query params', () => {
    const stage: Stage = {
      kind: 'ready', bookId: 'ns', view: 'generate',
      currentChapterId: 7, openProfileId: 'eliza',
    };
...
describe('stageEqual', () => {
  ...
  it('ready with differing view is not equal', () => {
    const a: Stage = { kind: 'ready', bookId: 'ns', view: 'cast', currentChapterId: 3, openProfileId: null };
    const b: Stage = { kind: 'ready', bookId: 'ns', view: 'manuscript', currentChapterId: 3, openProfileId: null };
    expect(stageEqual(a, b)).toBe(false);
  });

  it('ready with differing currentChapterId is not equal', () => {
    const a: Stage = { kind: 'ready', bookId: 'ns', view: 'cast', currentChapterId: 3, openProfileId: null };
    const b: Stage = { kind: 'ready', bookId: 'ns', view: 'cast', currentChapterId: 7, openProfileId: null };
    expect(stageEqual(a, b)).toBe(false);
  });

  it('ready with differing openProfileId is not equal', () => {
    const a: Stage = { kind: 'ready', bookId: 'ns', view: 'cast', currentChapterId: 3, openProfileId: null };
    const b: Stage = { kind: 'ready', bookId: 'ns', view: 'cast', currentChapterId: 3, openProfileId: 'halloran' };
    expect(stageEqual(a, b)).toBe(false);
  });

  it('identical ready stages are equal', () => {
    const stage: Stage = { kind: 'ready', bookId: 'ns', view: 'cast', currentChapterId: 3, openProfileId: 'halloran' };
