...
describe('variantVoiceIdFor', () => {
  it('returns the designed variant voiceId for an emotion', () => {
    const c = {
      ...baseChar,
      overrideTtsVoices: {
        qwen: { name: 'qwen-sweeney', variants: { angry: { name: 'qwen-sweeney-angry' } } },
      },
    } as unknown as Character;
    expect(variantVoiceIdFor(c, 'angry')).toBe('qwen-sweeney-angry');
...
describe('playEmotionVariantSample', () => {
  it('plays the variant scope when the emotion variant exists', async () => {
    ...
    expect(args.voiceId).toBe('sweeney__angry');
    expect(args.voice.overrideTtsVoices.qwen.name).toBe('qwen-sweeney-angry');
  ...
  it('falls back to the base scope + voice when the emotion variant is missing', async () => {
    ...
    expect(args.voiceId).toBe('sweeney');
