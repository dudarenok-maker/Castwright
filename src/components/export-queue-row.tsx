...
import {
  IconCheckCircle, IconSpinner, IconWarning, IconClock,
  IconCopy, IconDownload, IconRefresh, IconTrash,
} from '../lib/icons';
    ...
    <div className="grid grid-cols-[44px_1fr_120px_120px_140px_120px] items-center gap-4 px-5 py-3.5 text-sm hover:bg-ink/[0.02] transition-colors">
      ...
      <span className="flex items-center justify-end gap-1">
        {item.status === 'done' && (
          item.url ? (
            onCopyLink
              ? <button onClick={() => onCopyLink(item)} title="Copy link" className={interactiveClass}><IconCopy className="w-4 h-4"/></button>
              : <button disabled title="Copy link — coming soon" className={disabledClass}><IconCopy className="w-4 h-4"/></button>
          ) : (
            onDownload
              ? <button onClick={() => onDownload(item)} title="Download" className={interactiveClass}><IconDownload className="w-4 h-4"/></button>
              : <button disabled title="Download — coming soon" className={disabledClass}><IconDownload className="w-4 h-4"/></button>
          )
        )}
        {item.status === 'failed' && (
          onRetry
            ? <button onClick={() => onRetry(item)} title="Retry" className={interactiveClass}><IconRefresh className="w-4 h-4"/></button>
            : <button disabled title="Retry — coming soon" className={disabledClass}><IconRefresh className="w-4 h-4"/></button>
        )}
        {onRemove
          ? <button onClick={() => onRemove(item)} title="Remove" className={interactiveClass}><IconTrash className="w-4 h-4"/></button>
          : <button disabled title="Remove — coming soon" className={disabledClass}><IconTrash className="w-4 h-4"/></button>
        }
      </span>
    </div>
  );
