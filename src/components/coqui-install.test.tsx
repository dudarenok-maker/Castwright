...
describe('CoquiInstall — controlled idle states', () => {
  it('renders ready as installed', () => {
    render(<CoquiInstall status={{ state: 'ready', packageBroken: false, packageFault: 'ok' }} />);
  ...
  it('renders the install card (with value/difference copy) when not installed', () => {
    render(<CoquiInstall status={{ state: 'not-installed', packageBroken: false, packageFault: 'ok' }} />);
  ...
  it('renders weights-missing distinctly (not "not installed")', () => {
    render(<CoquiInstall status={{ state: 'weights-missing', packageBroken: false, packageFault: 'ok' }} />);
  ...
  it('renders package-broken as a repair state', () => {
    render(<CoquiInstall status={{ state: 'ready', packageBroken: true, packageFault: 'broken' }} />);
  ...
  it('a live-confirmed "missing" fault renders Install, not Repair, even when state is still "ready"', () => {
    render(<CoquiInstall status={{ state: 'ready', packageBroken: true, packageFault: 'missing' }} />);
    expect(screen.getByTestId('coqui-install-missing')).toBeInTheDocument();
    expect(screen.getByText(/Coqui package is missing/i)).toBeInTheDocument();
    expect(screen.getByRole('button', { name: /install coqui xtts v2/i })).toBeInTheDocument();
