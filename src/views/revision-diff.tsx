...
import {
  IconArrowLeft, IconAB, IconClose, IconClock, IconPlay, IconPause,
  IconChecks, IconReject, IconCheck, IconSpinner,
} from '../lib/icons';
...
export function RevisionDiffPlayer({ revision, bookId, chapter, character, onClose, onAccept, onReject }: Props) {
  const [selected, setSelected] = useState<Record<number, 'A' | 'B'>>(() => {
    const m: Record<number, 'A' | 'B'> = {};
    revision.segments.forEach(s => { if (s.id != null) m[s.id] = s.changed ? 'B' : 'A'; });
    return m;
  });
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    if (hasPreviousAudio) {
      api.getChapterAudioPrevious({ bookId, chapterId: chapter.id, duration: chapter.duration })
        .then(res => { if (!cancelled) setAudioA(res); })
        .catch(err => {
    ...
    if (playable) {
      api.getChapterAudio({ bookId, chapterId: chapter.id, duration: chapter.duration })
        .then(res => { if (!cancelled) setAudioB(res); })
        .catch(err => {
  ...
  useEffect(() => {
    ...
    (async () => {
      for (const seg of changedSegments) {
        ...
        try { await ab.playA({ segmentId: seg.id ?? undefined, start, end }); } catch { /* ignore */ }
        ...
        try { await ab.playB({ segmentId: seg.id ?? undefined, start: startB, end: endB }); } catch { /* ignore */ }
        await waitForVersionEnd();
      }
  ...
  const acceptedNew = revision.segments.filter(s => s.changed && s.id != null && selected[s.id] === 'B').length;

  const acceptAllNew = () =>
    setSelected(Object.fromEntries(revision.segments.flatMap(s => s.id != null ? [[s.id, s.changed ? 'B' : 'A']] : [])));
  const rejectAll = () =>
    setSelected(Object.fromEntries(revision.segments.flatMap(s => s.id != null ? [[s.id, 'A']] : [])));

  function handlePlay(version: AbVersion, opts: { segmentId?: number; start?: number; end?: number } = {}) {
      ...
      <div className="max-w-[1400px] mx-auto px-6 py-8 grid grid-cols-1 lg:grid-cols-[1fr_360px] gap-8">
        <main>
          <section className="grid grid-cols-2 gap-4 mb-6">
            <ABCard label="A · Current"  sub={hasPreviousAudio ? 'Already in your audiobook' : 'Original audio not preserved — review by metadata only'}
              duration={revision.oldDuration} variant="current"
              available={hasPreviousAudio}
              isPlaying={ab.playing === 'A' && ab.segmentId === null}
              onPlay={() => handlePlay('A')}/>
            <ABCard label="B · New draft" sub={playable ? (revision.triggeredBy ?? '') : 'Rendering new take…'}
              duration={revision.newDuration} variant="new" character={character}
              available={playable}
            ...
            <section>
              ...
              <div className="space-y-2">
                {revision.segments.map(seg => {
                  const segMetaA = seg.id != null ? audioA?.segments?.find(s => s.sentenceId === seg.id) : undefined;
                  const segMetaB = seg.id != null ? audioB?.segments?.find(s => s.sentenceId === seg.id) : undefined;
                  return (
                    <SegmentDiffRow
                      key={seg.id}
                      seg={seg}
                      ...
                      bDisabled={!playable}
                      onPlayA={() => handlePlay('A', { segmentId: seg.id ?? undefined, start: segMetaA?.start, end: segMetaA?.end })}
                      onPlayB={() => handlePlay('B', { segmentId: seg.id ?? undefined, start: segMetaB?.start, end: segMetaB?.end })}
                    />
                  );
                })}
              </div>
        ...
        <aside className="self-start sticky top-24 space-y-4">
          <div className="bg-white rounded-3xl border border-ink/10 p-5 shadow-card">
            ...
            <p className="text-4xl font-bold text-ink tabular-nums leading-none">{Math.round((revision.confidence ?? 0) * 100)}<span className="text-xl text-ink/50">%</span></p>
...
function ABCard({ label, sub, duration, variant, character, available, isPlaying, error, onPlay }: ABCardProps) {
  ...
  return (
    <div className={`rounded-3xl border p-5 transition-all ${isNew ? 'border-peach bg-peach/[0.06]' : 'border-ink/10 bg-white'} shadow-card`}>
      ...
      {error && (
        <p className="mt-1 text-[11px] text-red-600/90 font-medium">⚠ {error}</p>
      )}
      {c && isNew && character && (
...
function SegmentDiffRow({ seg, charColor, selectedVersion, onSelect, isPlayingA, isPlayingB, aDisabled, bDisabled, onPlayA, onPlayB }: SegmentRowProps) {
        ...
        <div className="flex-1 min-w-0">
          ...
          {seg.narratorOnly ? (
            ...
            <div className="grid grid-cols-2 gap-2">
              <button onClick={(e) => { e.stopPropagation(); onSelect('A'); onPlayA(); }}
                      disabled={aDisabled}
              ...
              <button onClick={(e) => { e.stopPropagation(); onSelect('B'); onPlayB(); }}
                      disabled={bDisabled}
                ...
                <span className="flex-1 min-w-0">
                  ...
                  <span className="block text-[11px] tabular-nums text-ink/60">{seg.newDuration}</span>
                </span>
