...
import {
  ...
  IconChecks, IconReject, IconCheck,
} from '../lib/icons';
...
export function RevisionDiffPlayer({ revision, chapter, character, onClose, onAccept, onReject }: Props) {
  ...
  const [playing, setPlaying] = useState<{ segId: number; version: 'A' | 'B' } | null>(null);
            ...
            <div className="space-y-2">
              {revision.segments.map(seg => (
                <SegmentDiffRow
                  ...
                  playing={playing}
                  onPlay={(version) => seg.id != null && setPlaying({ segId: seg.id, version })}
                />
              ))}
            </div>
          </section>
...
interface ABCardProps { label: string; sub: string; duration?: string; variant: 'current' | 'new'; character?: Character; }
function ABCard({ label, sub, duration, variant, character }: ABCardProps) {
...
interface SegmentRowProps {
  ...
  onSelect: (v: 'A' | 'B') => void;
  playing: { segId: number; version: 'A' | 'B' } | null;
  onPlay: (version: 'A' | 'B') => void;
}

function SegmentDiffRow({ seg, charColor, selectedVersion, onSelect, playing, onPlay }: SegmentRowProps) {
