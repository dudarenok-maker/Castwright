        ...
        {Object.entries(groups).map(([date, items]) => (
          <div key={date}>
            ...
            <div className="bg-white rounded-3xl border border-ink/10 shadow-card overflow-hidden">
              <div data-testid={`changelog-section-scroll-${date}`}
            ...
            {isActivityEmpty ? (
              <>
                ...
                <p className="mt-2 text-sm text-ink/60 max-w-md mx-auto">
                  Regenerations, voice tunes, cast confirms, and boundary edits will appear here as you work — newest first.
                </p>
              </>
            ) : (
              <>
                <h3 className="text-base font-bold text-ink">No {activeFilterLabel.toLowerCase()} events yet</h3>
                <p className="mt-2 text-sm text-ink/60 max-w-md mx-auto">
                  Nothing in this category has been logged so far. Switch back to <button onClick={() => setFilter('all')} className="font-semibold text-ink underline-offset-2 hover:underline">All</button> to see everything that has happened.
                </p>
              </>
            )}
          </div>
...
function ChangeLogEntry({ event }: { event: ChangeLogEvent }) {
  const t = LOG_TYPES[event.type] || { icon: <IconHistory className="w-3 h-3"/>, color: '#6B6663', label: event.type };
  return (
    <div className="grid grid-cols-[auto_1fr_auto] gap-3 px-4 py-2.5 hover:bg-ink/[0.02] transition-colors">
      <span className="w-7 h-7 rounded-full grid place-items-center text-white shrink-0 mt-0.5" style={{ background: t.color }}>
        {t.icon}
      </span>
      <div className="min-w-0">
        <div className="flex items-baseline gap-2 mb-0.5 flex-wrap">
          ...
          {event.bookTitle && (
            <span className="text-[10px] uppercase tracking-wider text-ink/45 font-semibold">· {event.bookTitle}</span>
          )}
          {event.chapterId && (
            <span className="text-[10px] text-ink/45 font-mono">· CH {String(event.chapterId).padStart(2, '0')}</span>
          )}
        </div>
