...
- **A GPU is optional but strongly recommended** (Kokoro on CPU is ~10× slower than on a modest GPU):
  - Windows / Linux: an **NVIDIA GPU + recent drivers** (CUDA). PyTorch installs automatically with the sidecar requirements — the default PyPI wheel is CUDA-bundled, so GPU works out of the box; for a **specific CUDA toolkit** (e.g. CUDA 13.0) see "PyTorch / CUDA" below.
...
> **PyTorch / CUDA.** `torch` (`>=2.6`) is an explicit sidecar requirement (recent `coqui-tts` no longer pulls it transitively) and installs with `pip install -r requirements.txt` — used by the Coqui XTTS and Qwen engines (Kokoro uses ONNX Runtime, no torch). On Windows/Linux x86_64 the default is the CUDA-bundled wheel; on macOS the CPU/MPS build. To pick a **specific** CUDA build, pre-install torch before the requirements — e.g. **CUDA 13.0**: `pip install torch torchaudio --index-url https://download.pytorch.org/whl/cu130` (swap `cu130` for another CUDA index from pytorch.org/get-started, or use `…/whl/cpu` to force CPU-only).
